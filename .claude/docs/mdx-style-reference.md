...
- **LLM surface**: every page is served as Markdown (append `.md`/`.mdx` to the URL, or `Accept: text/markdown`); the root `llms.txt` is hand-maintained at `fern/llms.txt` and served verbatim via `agents.llms-txt` in docs.yml; product-level `/docs/<product>/llms.txt` indexes are auto-generated (one description + URL per page; `description` frontmatter feeds it, `subtitle` is the fallback). `llms-full.txt` is deprecated upstream. A per-page agent directive is auto-prepended (configurable under the `agents` key in docs.yml; ours points agents at signalwire.com/docs/llms.txt).
