...
To start sharing the screen, you need to call the [RoomSession.startScreenShare()](/sdks/relay-browser-sdk/video/room-session#startscreenshare) method. This will make the browser prompt your user with the browser's screen share dialog like the one shown below.
...
[RoomSession.startScreenShare()](/sdks/relay-browser-sdk/video/room-session#startscreenshare) asynchronously returns a [RoomSessionScreenShare](/sdks/relay-browser-sdk/video/room-session-screenshare) object.  
To stop the video feed, simply call the [RoomSessionScreenShare.leave()](/sdks/relay-browser-sdk/video/room-session-screenshare#leave) method.
