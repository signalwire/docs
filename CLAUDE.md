...
`fern/llms.txt` is the root `https://signalwire.com/docs/llms.txt`, served verbatim via `agents.llms-txt` in `fern/docs.yml`. When a page linked there moves, update the link. The product-level `/docs/<product>/llms.txt` indexes are still generated by Fern from page `description` frontmatter. The `## Products` section is a manual copy of Fern's generated product list (index link, subtitle, five sample pages each) and needs a refresh when product subtitles or nav change.
