...
| `token` | `string` | <span class="required-arg">required</span> | Json Web Token retrieved using Rest API. See [Generate a JWT](/sdks/relay-browser-sdk/v2#generate-a-jwt) for more information. |
...
> **Note:** the client will attach the streams to the proper element but will not change the `style` attribute. You can decide if you would like to display or hide the `HTMLMediaElement` following the application logic.<br/>Use the [callUpdate notification](/sdks/relay-browser-sdk/v2/notification#callupdate) to detect call state changes and update the UI accordingly.
