...
export class MdxAstBuilder {
  ...
  list(items, ordered = false) {
    const listItems = items.map((item) =>
      u("listItem", [
        typeof item === "string" ? this.paragraph([this.parseInlineContent(item)]) : item,
      ]),
    );
    ...
    if (linkMatches) {
      ...
      linkMatches.forEach((match) => {
        ...
        if (index > lastIndex) {
          ...
          if (textBefore.startsWith("`") && textBefore.endsWith("`")) {
            parts.push(this.inlineCode(textBefore.slice(1, -1)));
          } else {
            parts.push(this.text(textBefore));
          }
        }
      ...
      if (lastIndex < content.length) {
        ...
        if (textAfter.startsWith("`") && textAfter.endsWith("`")) {
          parts.push(this.inlineCode(textAfter.slice(1, -1)));
        } else {
          parts.push(this.text(textAfter));
        }
      }
