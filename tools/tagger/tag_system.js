...
try {
  ...
  try {
    ...
    console.error("Could not read template");
    throw e;
  }
} catch (error) {
...
function main() {
  ...
  allFiles.forEach((filePath, index) => {
    ...
    process.stdout.write(
      `[${index + 1}/${allFiles.length}] Processing ${relativePath}... `,
    );
    ...
    if (result.updated) {
      ...
      result.reason === "No matching path configuration found" ||
      result.reason === "No tags configured for this path"
    ) {
