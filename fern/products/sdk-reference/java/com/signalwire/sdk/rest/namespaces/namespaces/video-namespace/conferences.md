---
slug: "/reference/java/com/signalwire/sdk/rest/namespaces/namespaces/video-namespace/conferences"
title: "conferences"
sdk_label: "Java SDK"
icon: "java"
lustri:
  auto_generated: true
  kind: "method"
  language: "java"
  qualified_name: "com.signalwire.sdk.rest.namespaces.VideoNamespace.conferences"
  parent: "com.signalwire.sdk.rest.namespaces.VideoNamespace"
  module: "com.signalwire.sdk.rest.namespaces.VideoNamespace"
  source_url: "https://github.com/signalwire/signalwire-java/blob/main/src/main/java/com/signalwire/sdk/rest/namespaces/VideoNamespace.java"
  visibility: "public"
---
# `conferences`

## Signature

```java
public com.signalwire.sdk.rest.namespaces.VideoNamespace.VideoConferences conferences()
```

## Returns

`com.signalwire.sdk.rest.namespaces.VideoNamespace.VideoConferences`

## Source

[`src/main/java/com/signalwire/sdk/rest/namespaces/VideoNamespace.java`](https://github.com/signalwire/signalwire-java/blob/main/src/main/java/com/signalwire/sdk/rest/namespaces/VideoNamespace.java)

Line 48.
