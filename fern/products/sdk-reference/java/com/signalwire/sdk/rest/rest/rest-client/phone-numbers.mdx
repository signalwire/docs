---
auto_generated: true
kind: "method"
language: "java"
qualified_name: "com.signalwire.sdk.rest.RestClient.phoneNumbers"
parent: "com.signalwire.sdk.rest.RestClient"
module: "com.signalwire.sdk.rest.RestClient"
slug: "/sdk-reference/java/com/signalwire/sdk/rest/rest/rest-client/phone-numbers"
source_url: "https://github.com/signalwire/signalwire-java/blob/main/src/main/java/com/signalwire/sdk/rest/RestClient.java"
visibility: "public"
title: "phoneNumbers"
sdk_label: "Java SDK"
icon: "java"
---
# `phoneNumbers`

## Signature

```java
public com.signalwire.sdk.rest.namespaces.PhoneNumbersNamespace phoneNumbers()
```

## Returns

`com.signalwire.sdk.rest.namespaces.PhoneNumbersNamespace`

## Source

[`src/main/java/com/signalwire/sdk/rest/RestClient.java`](https://github.com/signalwire/signalwire-java/blob/main/src/main/java/com/signalwire/sdk/rest/RestClient.java)

Line 152.
