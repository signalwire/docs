...
slug: "/sdk-reference/php/signal-wire/security/security/webhook-validator/validate-webhook-signature"
...
```php
static public function validateWebhookSignature(
    string $signingKey,
    string $signature,
    string $url,
    string $rawBody
...
<ParamField path="rawBody" type="string" required>
  The raw request body bytes as a UTF-8 string, BEFORE
...
`bool` — True if the signature matches either Scheme A (hex JSON) or
