---
auto_generated: true
kind: "module"
language: "rust"
qualified_name: "signalwire::rest::namespaces::project"
parent: "signalwire::rest::namespaces"
module: "rest.namespaces"
slug: "/sdk-reference/rust/signalwire/rest/namespaces/project"
source_url: "https://github.com/signalwire/signalwire-rust/blob/main/src/rest/namespaces/project.rs"
visibility: "public"
title: "project"
sdk_label: "Rust SDK"
icon: "rust"
---
# `project`

## Classes


<CardGroup cols={2}>
  <Card title="Project" href="/sdk-reference/rust/signalwire/rest/namespaces/project/project">
    Project API namespace — exposes the API token sub-resource.
  </Card>
  <Card title="ProjectTokens" href="/sdk-reference/rust/signalwire/rest/namespaces/project/project-tokens">
    Project API token management.
  </Card>
</CardGroup>

## Source

[`src/rest/namespaces/project.rs`](https://github.com/signalwire/signalwire-rust/blob/main/src/rest/namespaces/project.rs)

Line 1.
