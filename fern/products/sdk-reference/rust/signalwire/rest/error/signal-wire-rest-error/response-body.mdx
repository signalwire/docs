---
auto_generated: true
kind: "method"
language: "rust"
qualified_name: "signalwire::rest::error::SignalWireRestError::response_body"
parent: "signalwire::rest::error::SignalWireRestError"
module: "rest.error"
slug: "/sdk-reference/rust/signalwire/rest/error/signal-wire-rest-error/response-body"
source_url: "https://github.com/signalwire/signalwire-rust/blob/main/src/rest/error.rs"
visibility: "public"
title: "response_body"
sdk_label: "Rust SDK"
icon: "rust"
---
# `response_body`

## Signature

```rust
fn response_body(&self) -> &str
```

## Returns

`&str`

## Source

[`src/rest/error.rs`](https://github.com/signalwire/signalwire-rust/blob/main/src/rest/error.rs)

Line 28.
