---
auto_generated: true
kind: "method"
language: "rust"
qualified_name: "signalwire::contexts::context_builder::Step::set_valid_steps"
parent: "signalwire::contexts::context_builder::Step"
module: "contexts.context_builder"
slug: "/sdk-reference/rust/signalwire/contexts/context-builder/step/set-valid-steps"
source_url: "https://github.com/signalwire/signalwire-rust/blob/main/src/contexts/context_builder.rs"
visibility: "public"
title: "set_valid_steps"
sdk_label: "Rust SDK"
icon: "rust"
---
# `set_valid_steps`

## Signature

```rust
fn set_valid_steps(&mut self, steps: Vec<&str>) -> &mut Self
```

## Parameters

<ParamField path="steps" type="Vec<&str>" required></ParamField>

## Returns

`&mut Self`

## Source

[`src/contexts/context_builder.rs`](https://github.com/signalwire/signalwire-rust/blob/main/src/contexts/context_builder.rs)

Line 254.
