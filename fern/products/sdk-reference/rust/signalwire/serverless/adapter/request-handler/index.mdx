---
auto_generated: true
kind: "interface"
language: "rust"
qualified_name: "signalwire::serverless::adapter::RequestHandler"
parent: "signalwire::serverless::adapter"
module: "serverless.adapter"
slug: "/sdk-reference/rust/signalwire/serverless/adapter/request-handler"
source_url: "https://github.com/signalwire/signalwire-rust/blob/main/src/serverless/adapter.rs"
visibility: "public"
title: "RequestHandler"
sdk_label: "Rust SDK"
icon: "rust"
---
# `RequestHandler`

Trait that the agent/service must implement so the adapter can
forward requests to it.

## Signature

```rust
trait RequestHandler
```

## Methods


<CardGroup cols={2}>
  <Card title="handle_request" href="/sdk-reference/rust/signalwire/serverless/adapter/request-handler/handle-request">
    Handle an HTTP request, returning (status_code, headers, body).
  </Card>
</CardGroup>

## Source

[`src/serverless/adapter.rs`](https://github.com/signalwire/signalwire-rust/blob/main/src/serverless/adapter.rs)

Line 28.
