...
<CardGroup cols={2}>
  <Card title="clone" href="/reference/rust/signalwire/security/session-manager/session-manager/clone"></Card>
  <Card title="create_session" href="/reference/rust/signalwire/security/session-manager/session-manager/create-session">
    Create or confirm a session, returning the call ID.
  </Card>
  <Card title="create_token" href="/reference/rust/signalwire/security/session-manager/session-manager/create-token">
    Generate an HMAC-SHA256 signed token for a given function and call.
  </Card>
  <Card title="new" href="/reference/rust/signalwire/security/session-manager/session-manager/new">
    Create a new session manager with a random 32-byte secret.
  </Card>
  <Card title="token_expiry_secs" href="/reference/rust/signalwire/security/session-manager/session-manager/token-expiry-secs">
    Get the configured token expiry in seconds.
  </Card>
  <Card title="validate_token" href="/reference/rust/signalwire/security/session-manager/session-manager/validate-token">
    Validate a token against the expected function name and call ID.
  </Card>
  <Card title="with_defaults" href="/reference/rust/signalwire/security/session-manager/session-manager/with-defaults">
