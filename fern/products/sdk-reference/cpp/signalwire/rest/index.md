---
slug: "/reference/cpp/signalwire/rest"
title: "rest"
sdk_label: "C++ SDK"
icon: "cpp"
lustri:
  auto_generated: true
  kind: "module"
  language: "cpp"
  qualified_name: "signalwire::rest"
  parent: "signalwire"
  module: "signalwire"
  source_url: "https://github.com/signalwire/signalwire-cpp/blob/main/include/signalwire/rest/http_client.hpp"
---
# `rest`

## Signature

```cpp
namespace signalwire::rest
```

## Functions

- [`make_headers`](/reference/cpp/signalwire/rest/make-headers)
- [`parse_url`](/reference/cpp/signalwire/rest/parse-url)
- [`to_wire_string`](/reference/cpp/signalwire/rest/to-wire-string) — Serialize a PhoneCallHandler to its wire string.

## Type Aliases

- [`json`](/reference/cpp/signalwire/rest/json)

## Classes

- [`CrudResource`](/reference/cpp/signalwire/rest/crud-resource) — Generic CRUD resource for REST API namespaces.
- [`HttpClient`](/reference/cpp/signalwire/rest/http-client) — HTTP client with Basic Auth support using cpp-httplib.
- [`PaginatedIterator`](/reference/cpp/signalwire/rest/paginated-iterator) — Iterates items across paginated API responses. Mirrors signalwire-python's signalwire.rest.\_pagination.PaginatedIterator: fetches the configured path with the configured params, walks the data\_key array, then follows links.next (parsing its query string for the next page's params) until the response carries no links.next. Iteration is lazy the constructor records inputs but performs no HTTP. The first has\_next() / next() call performs the first fetch. Cursor query params are extracted by parsing links.next like Python's urllib.parse.urlparse + parse\_qs.
- [`RestClient`](/reference/cpp/signalwire/rest/rest-client) — Top-level SignalWire REST client with all API namespaces.
- [`SignalWireRestError`](/reference/cpp/signalwire/rest/signal-wire-rest-error) — Error thrown on non-2xx REST API responses.

## Enums

- [`PhoneCallHandler`](/reference/cpp/signalwire/rest/phone-call-handler) — Enumeration of call\_handler values accepted by phone\_numbers.update. Setting a phone number's call\_handler + the handler-specific companion field routes inbound calls and auto-materializes the matching Fabric resource on the server. The typed set\_\* helpers on PhoneNumbersNamespace wrap the low-level update call with the right combination — prefer those over constructing the wire body by hand. Named PhoneCallHandler (not CallHandler) to avoid colliding with signalwire::relay::InboundCallHandler — the callback type used by the RELAY client for inbound-call events.

## Modules

- [`@151225112016371363115121076011247003035241121154`](/reference/cpp/signalwire/rest/151225112016371363115121076011247003035241121154)

## Source

[`include/signalwire/rest/http_client.hpp`](https://github.com/signalwire/signalwire-cpp/blob/main/include/signalwire/rest/http_client.hpp)

Line 12.
