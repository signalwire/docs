---
slug: "/reference/cpp/signalwire/security/session-manager"
title: "SessionManager"
sdk_label: "C++ SDK"
icon: "cpp"
lustri:
  auto_generated: true
  kind: "class"
  language: "cpp"
  qualified_name: "signalwire::security::SessionManager"
  parent: "signalwire::security"
  module: "signalwire.security"
  source_url: "https://github.com/signalwire/signalwire-cpp/blob/main/include/signalwire/security/session_manager.hpp"
  visibility: "public"
---
# `SessionManager`

Manages HMAC-SHA256 based session tokens for secure SWAIG tool calls.
Token format: base64(functionName:callID:expiryTimestamp) + "." + hex(hmac\_signature)

## Signature

```cpp
class signalwire::security::SessionManager
```

## Methods

- [`create_token`](/reference/cpp/signalwire/security/session-manager/create-token) — Create a signed token for a function call
- [`SessionManager`](/reference/cpp/signalwire/security/session-manager/session-manager) — Construct with a random 32-byte secret.
- [`timing_safe_compare`](/reference/cpp/signalwire/security/session-manager/timing-safe-compare) — Timing-safe comparison of two byte sequences.
- [`validate_token`](/reference/cpp/signalwire/security/session-manager/validate-token) — Validate a token

## Source

[`include/signalwire/security/session_manager.hpp`](https://github.com/signalwire/signalwire-cpp/blob/main/include/signalwire/security/session_manager.hpp)

Line 14.
