---
auto_generated: true
kind: "class"
language: "cpp"
qualified_name: "signalwire::skills::WikipediaSearchSkill"
parent: "signalwire::skills"
module: "signalwire.skills"
slug: "/sdk-reference/cpp/signalwire/skills/wikipedia-search-skill"
source_url: "https://github.com/signalwire/signalwire-cpp/blob/main/src/skills/builtin/wikipedia_search.cpp"
visibility: "public"
title: "WikipediaSearchSkill"
sdk_label: "C++ SDK"
icon: "cpp"
---
# `WikipediaSearchSkill`

Wikipedia search skill — issues a real GET against the Wikipedia API /w/api.php endpoint with action=query&list=search, parses the query.search[] results, and returns titles + snippets. Matches the Python WikipediaSearchSkill upstream call shape.
WIKIPEDIA_BASE_URL env var overrides the API root (used by audit_skills_dispatch.py to point the skill at its loopback fixture).

## Signature

```cpp
class signalwire::skills::WikipediaSearchSkill : signalwire::skills::SkillBase
```

## Methods


<CardGroup cols={2}>
  <Card title="get_prompt_sections" href="/sdk-reference/cpp/signalwire/skills/wikipedia-search-skill/get-prompt-sections">
    Get prompt sections to inject into agent.
  </Card>
  <Card title="register_tools" href="/sdk-reference/cpp/signalwire/skills/wikipedia-search-skill/register-tools">
    Register tools with the agent. Returns tool definitions.
  </Card>
  <Card title="setup" href="/sdk-reference/cpp/signalwire/skills/wikipedia-search-skill/setup">
    Initialize the skill with given params. Return true on success.
  </Card>
  <Card title="skill_description" href="/sdk-reference/cpp/signalwire/skills/wikipedia-search-skill/skill-description"></Card>
  <Card title="skill_name" href="/sdk-reference/cpp/signalwire/skills/wikipedia-search-skill/skill-name"></Card>
</CardGroup>

## Source

[`src/skills/builtin/wikipedia_search.cpp`](https://github.com/signalwire/signalwire-cpp/blob/main/src/skills/builtin/wikipedia_search.cpp)

Line 20.
