---
slug: "/reference/python/signalwire/cli/simulation/data-overrides/apply-overrides"
title: "apply_overrides"
sdk_label: "Python SDK"
icon: "python"
lustri:
  auto_generated: true
  kind: "function"
  language: "python"
  qualified_name: "signalwire.cli.simulation.data_overrides.apply_overrides"
  parent: "signalwire.cli.simulation.data_overrides"
  module: "signalwire.cli.simulation.data_overrides"
  source_url: "https://github.com/signalwire/signalwire-python/blob/main//src/signalwire/signalwire/cli/simulation/data_overrides.py"
---
# `apply_overrides`

Apply override values to data using dot notation paths

## Signature

```python
apply_overrides(
    data: Dict[str, Any],
    overrides: List[str],
    json_overrides: List[str]
) -> Dict[str, Any]
```

## Parameters

| Name             | Type             | Required | Default | Description                        |
| ---------------- | ---------------- | -------- | ------- | ---------------------------------- |
| `data`           | `Dict[str, Any]` | yes      | —       | Data dictionary to modify          |
| `overrides`      | `List[str]`      | yes      | —       | List of "path=value" strings       |
| `json_overrides` | `List[str]`      | yes      | —       | List of "path=json\_value" strings |

## Returns

`Dict[str, Any]` — Modified data dictionary

## Source

[`/src/signalwire/signalwire/cli/simulation/data_overrides.py`](https://github.com/signalwire/signalwire-python/blob/main//src/signalwire/signalwire/cli/simulation/data_overrides.py)

Line 80.
