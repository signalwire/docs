---
auto_generated: true
kind: "function"
language: "python"
qualified_name: "signalwire.core.security.webhook_validator.validate_request"
parent: "signalwire.core.security.webhook_validator"
module: "signalwire.core.security.webhook_validator"
slug: "/sdk-reference/python/signalwire/core/security/webhook-validator/validate-request"
source_url: "https://github.com/signalwire/signalwire-python/blob/main/signalwire/signalwire/core/security/webhook_validator.py"
title: "validate_request"
sdk_label: "Python SDK"
icon: "python"
---
# `validate_request`

Legacy ``@signalwire/compatibility-api`` drop-in entry point.

If ``params_or_raw_body`` is a string, delegates to
:func:`validate_webhook_signature` (Scheme A then Scheme B with parsed form).

If it's a mapping or a list of (key, value) tuples, treats it as
pre-parsed form params and runs Scheme B directly (with URL port
normalization and optional bodySHA256 fallback).

Args:

```
signing_key: Customer's Signing Key. Missing raises ``ValueError``.
signature: Header value. Missing / empty returns False.
url: Full URL SignalWire POSTed to.
params_or_raw_body: ``str`` raw body OR pre-parsed form params.
```

Returns:

```
True on match, False otherwise.
```

**Raises:**
- ValueError: when ``signing_key`` is missing.
- TypeError: when ``params_or_raw_body`` is neither a string nor a
mapping/list (e.g. a parsed JSON dict-like that's been rejected).

## Signature

```python
validate_request(
    signing_key: str,
    signature: str,
    url: str,
    params_or_raw_body: Union[str, Mapping[str, Any], List[Tuple[str, Any]], None]
) -> bool
```

## Parameters

<ParamField path="signing_key" type="str" required></ParamField>
<ParamField path="signature" type="str" required></ParamField>
<ParamField path="url" type="str" required></ParamField>
<ParamField path="params_or_raw_body" type="Union[str, Mapping[str, Any], List[Tuple[str, Any]], None]" required></ParamField>

## Returns

`bool`

## Source

[`signalwire/signalwire/core/security/webhook_validator.py`](https://github.com/signalwire/signalwire-python/blob/main/signalwire/signalwire/core/security/webhook_validator.py)

Line 268.
