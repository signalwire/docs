...
The Python implementation (agent_base.py line 612) creates a sip_routing_callback that extracts the SIP username from the body, checks it against _sip_usernames, and returns None in both the matched and unmatched case — letting the normal routing continue. It then calls register_routing_callback to register the callback, and optionally calls auto_map_sip_usernames.
