...
Mirrors Python's RelayClient.dial(devices, *, tag=None, max_duration=None, dial_timeout=None). The calling.dial RPC response only contains \{"code": "200", "message": "Dialing"} — no call_id. The real call_id and node_id arrive via subsequent calling.call.dial events keyed by tag. This method waits for that event so the returned Call always has valid identifiers.
