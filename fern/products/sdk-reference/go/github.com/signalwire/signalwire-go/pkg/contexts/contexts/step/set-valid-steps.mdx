---
auto_generated: true
kind: "method"
language: "go"
qualified_name: "github.com/signalwire/signalwire-go/pkg/contexts.Step.SetValidSteps"
parent: "github.com/signalwire/signalwire-go/pkg/contexts.Step"
module: "github.com.signalwire.signalwire-go.pkg.contexts"
slug: "/sdk-reference/go/github.com/signalwire/signalwire-go/pkg/contexts/contexts/step/set-valid-steps"
source_url: "https://github.com/signalwire/signalwire-go/blob/main/pkg/contexts/contexts.go"
title: "SetValidSteps"
sdk_label: "Go SDK"
icon: "golang"
---
# `SetValidSteps`

SetValidSteps sets which steps can be navigated to from this step.

## Signature

```go
func (*Step) SetValidSteps(steps []string) *Step
```

## Parameters

<ParamField path="steps" type="[]string" required></ParamField>

## Returns

`*Step`

## Source

[`pkg/contexts/contexts.go`](https://github.com/signalwire/signalwire-go/blob/main/pkg/contexts/contexts.go)

Line 245.
