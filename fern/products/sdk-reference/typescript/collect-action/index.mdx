---
auto_generated: true
kind: "class"
language: "typescript"
qualified_name: "CollectAction"
slug: "/sdk-reference/typescript/collect-action"
source_url: "https://github.com/signalwire/signalwire-typescript/blob/main/src/relay/Action.ts"
title: "CollectAction"
sdk_label: "TypeScript SDK"
icon: "typescript"
---
# `CollectAction`

Async handle for a `play_and_collect` action (combined playback + input collection).

## Signature

```typescript
class CollectAction extends Action
```

## Properties

<ParamField path="call" type="CallLike">
  Reference to the owning call.
</ParamField>
<ParamField path="completed" type="boolean">
  Whether the action has reached a terminal state.
</ParamField>
<ParamField path="controlId" type="string">
  Unique control ID used by the server to route events back to this action.
</ParamField>
<ParamField path="isDone" type="get isDone(): boolean">
  True once the action has reached a terminal state.
</ParamField>
<ParamField path="result" type="RelayEvent | null">
  Final event once the action terminates, or `null` while still running.
</ParamField>

## Methods


<CardGroup cols={2}>
  <Card title="_checkEvent" href="/sdk-reference/typescript/collect-action/check-event">
    play_and_collect shares a control_id across play and collect phases. Only resolve on collect events, not play events.
  </Card>
  <Card title="constructor" href="/sdk-reference/typescript/collect-action/constructor"></Card>
  <Card title="startInputTimers" href="/sdk-reference/typescript/collect-action/start-input-timers">
    Start the collect input timers (useful when `initial_timeout` should be reset after an async side effect completes).
  </Card>
  <Card title="stop" href="/sdk-reference/typescript/collect-action/stop">
    Stop the play_and_collect operation.
  </Card>
  <Card title="volume" href="/sdk-reference/typescript/collect-action/volume">
    Adjust playback volume of the prompt audio mid-collect.
  </Card>
  <Card title="wait" href="/sdk-reference/typescript/collect-action/wait">
    Wait for the action to complete.
  </Card>
</CardGroup>

## Source

[`src/relay/Action.ts`](https://github.com/signalwire/signalwire-typescript/blob/main/src/relay/Action.ts)

Line 277.
