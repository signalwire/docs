...
- [`constructor`](/reference/typescript/relay/relay-event/echo-event/constructor)
- [`fromPayload`](/reference/typescript/relay/relay-event/echo-event/from-payload) — Factory that builds a typed event from a raw `signalwire.event` payload. Subclasses override this to populate their specialised fields; the base implementation returns a minimal `RelayEvent` used as the fallback for unrecognised event types.
