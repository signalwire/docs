...
This plays well with [AgentBase](/docs/sdk-reference/reference/typescript/agent-base) methods like `promptAddSection()` and
...
- \- \[PomSection]\(/docs/sdk-reference/reference/typescript/pom-builder/pom-section)
  \- \[AgentBase.promptAddSection]\(/docs/sdk-reference/reference/typescript/agent-base#prompt-add-section)

## Properties

<ParamField path="pom" type="get pom(): PromptObjectModel" required>
  Returns the underlying [PromptObjectModel](/docs/sdk-reference/reference/typescript/pom/prompt-object-model) for the builder. Mirrors Python's `PomBuilder.pom` attribute (the wrapped low-level model). Returns a fresh `PromptObjectModel` populated from the builder's current sections; mutations on the returned instance do not propagate back to this builder.
</ParamField>

## Classes

<CardGroup cols={2}>
  <Card title="PomSection" href="/docs/sdk-reference/reference/typescript/pom-builder/pom-section">
