...
Construct a [RestClient](/reference/typescript/rest/rest-client) instance.
...
| -------- | ------------------------- | -------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `args`   | `string[]`                | no       | —       | Positional credentials (compat shim for ports that pass project/token/host positionally). Usually empty in TS.                               |
| `kwargs` | `Record<string, unknown>` | no       | —       | Keyword-style credentials. When omitted, reads `SIGNALWIRE_PROJECT_ID`, `SIGNALWIRE_API_TOKEN`, and `SIGNALWIRE_SPACE` from the environment. |
