...
slug: "/reference/typescript/action"
...
An Action is returned from the async variants on [Call](/reference/typescript/call) (e.g. `call.playAsync`).
It resolves when the server emits a terminal event for its `controlId`. Use
[Action.wait](/reference/typescript/action/wait) to await completion, or register an `onCompleted` callback.
...
<CardGroup cols={2}>
  <Card title="constructor" href="/reference/typescript/action/constructor"></Card>
  <Card title="wait" href="/reference/typescript/action/wait">
