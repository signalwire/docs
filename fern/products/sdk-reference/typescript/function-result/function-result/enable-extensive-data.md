---
slug: "/reference/typescript/function-result/function-result/enable-extensive-data"
title: "enableExtensiveData"
sdk_label: "TypeScript SDK"
icon: "typescript"
lustri:
  auto_generated: true
  kind: "method"
  language: "typescript"
  qualified_name: "FunctionResult.FunctionResult.enableExtensiveData"
  parent: "FunctionResult.FunctionResult"
  module: "FunctionResult"
  source_url: "https://github.com/signalwire/signalwire-typescript/blob/main/src/FunctionResult.ts"
---
# `enableExtensiveData`

Enable or disable extensive data reporting in function calls.

## Signature

```typescript
enableExtensiveData(enabled: boolean = true): this
```

## Parameters

| Name      | Type      | Required | Default | Description                       |
| --------- | --------- | -------- | ------- | --------------------------------- |
| `enabled` | `boolean` | yes      | `true`  | Whether to enable extensive data. |

## Returns

`this` — This instance for chaining.

## Source

[`src/FunctionResult.ts`](https://github.com/signalwire/signalwire-typescript/blob/main/src/FunctionResult.ts)

Line 463.
