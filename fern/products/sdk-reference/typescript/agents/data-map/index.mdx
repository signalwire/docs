...
- \- \[FunctionResult]\(/docs/sdk-reference/reference/typescript/agents/function-result) — response shape used in \`.output()\` / \`.expression()\`
  \- \[createSimpleApiTool]\(/docs/sdk-reference/reference/typescript/agents/data-map#create-simple-api-tool) — one-liner helper for REST API tools
  \- \[AgentBase.defineTool]\(/docs/sdk-reference/reference/typescript/agents/agent-base#define-tool) — alternative for tools that run in your process
...
Alias for [purpose](/docs/sdk-reference/reference/typescript/agents/data-map#purpose); sets the LLM-facing tool description.

This string is read by the model to decide WHEN to call this tool.
See [purpose](/docs/sdk-reference/reference/typescript/agents/data-map#purpose) for bad-vs-good examples.
