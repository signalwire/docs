...
<ParamField path="debugMode" type="boolean" required>
  When true, [debugToken](/docs/sdk-reference/reference/typescript/session-manager#debug-token) decodes token internals. When false (default), it returns `{ error: "debug mode not enabled" }`.
</ParamField>
...
Alias for [generateToken](/docs/sdk-reference/reference/typescript/session-manager#generate-token).
...
Requires [debugMode](/docs/sdk-reference/reference/typescript/session-manager#debug-mode) to be `true`. When disabled, returns
...
Alias for [validateToken](/docs/sdk-reference/reference/typescript/session-manager#validate-token) with reordered parameters.
