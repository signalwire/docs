...
parent: "livewire"
module: "livewire"
slug: "/sdk-reference/typescript/livewire/define-agent"
...
execution by [runApp](/sdk-reference/typescript/livewire/run-app). Pass-through — no side effects.
...
<Indent>
  <ParamField path="agent.entry" type="(ctx: JobContext) => Promise<void>" required>
    Main callback invoked with a [JobContext](/sdk-reference/typescript/livewire/job-context) when
the agent runs.
  </ParamField>
  <ParamField path="agent.prewarm" type="(proc: JobProcess) => any">
    Optional prewarm callback invoked with a
[JobProcess](/sdk-reference/typescript/livewire/job-process) before `entry`.
  </ParamField>
</Indent>
