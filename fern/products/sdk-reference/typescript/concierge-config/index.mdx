---
auto_generated: true
kind: "interface"
language: "typescript"
qualified_name: "ConciergeConfig"
slug: "/sdk-reference/typescript/concierge-config"
source_url: "https://github.com/signalwire/signalwire-typescript/blob/main/src/prefabs/ConciergeAgent.ts"
title: "ConciergeConfig"
sdk_label: "TypeScript SDK"
icon: "typescript"
---
# `ConciergeConfig`

Configuration for the [ConciergeAgent](/sdk-reference/typescript/concierge-agent).

## Signature

```typescript
interface ConciergeConfig
```

## Properties

<ParamField path="agentOptions" type="Partial<AgentOptions>">
  Additional AgentBase options forwarded to `super()`.
</ParamField>
<ParamField path="amenities" type="Record<string, Record<string, string>>">
  Dictionary of amenities, each mapping to a dict of detail keys/values.
</ParamField>
<ParamField path="hoursOfOperation" type="Record<string, string>">
  Optional hours of operation by category (e.g. `{ default: "9 AM - 5 PM" }`).
</ParamField>
<ParamField path="name" type="string">
  Agent display name (defaults to `"concierge"`).
</ParamField>
<ParamField path="route" type="string">
  HTTP route for this agent (defaults to `"/concierge"`).
</ParamField>
<ParamField path="services" type="string[]">
  List of services offered by the venue (required).
</ParamField>
<ParamField path="specialInstructions" type="string[]">
  Optional special instructions appended to the agent's instruction bullets.
</ParamField>
<ParamField path="venueName" type="string">
  Name of the venue or business (required).
</ParamField>
<ParamField path="welcomeMessage" type="string">
  Optional custom welcome message spoken as a non-bargeable static greeting.
</ParamField>

## Source

[`src/prefabs/ConciergeAgent.ts`](https://github.com/signalwire/signalwire-typescript/blob/main/src/prefabs/ConciergeAgent.ts)

Line 16.
