...
kind: "function"
language: "typescript"
qualified_name: "createTypedHandlerWrapper"
slug: "/sdk-reference/typescript/create-typed-handler-wrapper"
...
<ParamField path="paramNames" type="string[]" required>
  Ordered list of parameter names extracted from `fn`,
produced by [inferSchema](/sdk-reference/typescript/infer-schema).
</ParamField>
<ParamField path="hasRawData" type="boolean" required>
  When `true`, the raw-data record is appended as the
final positional argument to mirror the old-style handler shape.
</ParamField>

## Returns

`SwaigHandler` — A [SwaigHandler](/sdk-reference/typescript/swaig-handler) suitable for registration with
[AgentBase.defineTool](/sdk-reference/typescript/agent-base/define-tool).
