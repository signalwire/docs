---
auto_generated: true
kind: "class"
language: "ruby"
qualified_name: "SignalWire::AgentBase::AgentSecurityHeadersMiddleware"
parent: "SignalWire::AgentBase"
module: "SignalWire.AgentBase"
slug: "/sdk-reference/ruby/signal-wire/agent-base/agent-security-headers-middleware"
source_url: "https://github.com/signalwire/signalwire-ruby/blob/main/lib/signalwire/agent/agent_base.rb"
visibility: "public"
title: "AgentSecurityHeadersMiddleware"
sdk_label: "Ruby SDK"
icon: "ruby"
---
# `AgentSecurityHeadersMiddleware`

==================================================================
Rack Middleware
==================================================================

## Signature

```ruby
class AgentSecurityHeadersMiddleware < Object
```

## Constants

<ParamField path="HEADERS" type="HEADERS = {   'x-content-type-options' => 'nosniff',   'x-frame-options'        => 'DENY',"></ParamField>

## Methods


<CardGroup cols={2}>
  <Card title="call" href="/sdk-reference/ruby/signal-wire/agent-base/agent-security-headers-middleware/call"></Card>
  <Card title="initialize" href="/sdk-reference/ruby/signal-wire/agent-base/agent-security-headers-middleware/initialize"></Card>
</CardGroup>

## Source

[`lib/signalwire/agent/agent_base.rb`](https://github.com/signalwire/signalwire-ruby/blob/main/lib/signalwire/agent/agent_base.rb)

Line 2062.
