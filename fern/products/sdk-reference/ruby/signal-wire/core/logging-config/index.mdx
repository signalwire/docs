---
auto_generated: true
kind: "module"
language: "ruby"
qualified_name: "SignalWire::Core::LoggingConfig"
parent: "SignalWire::Core"
module: "SignalWire.Core"
slug: "/sdk-reference/ruby/signal-wire/core/logging-config"
source_url: "https://github.com/signalwire/signalwire-ruby/blob/main/lib/signalwire/core/logging_config.rb"
visibility: "public"
title: "LoggingConfig"
sdk_label: "Ruby SDK"
icon: "ruby"
---
# `LoggingConfig`

## Signature

```ruby
module LoggingConfig
```

## Methods


<CardGroup cols={2}>
  <Card title="get_execution_mode" href="/sdk-reference/ruby/signal-wire/core/logging-config/get-execution-mode">
    Detect the SDK's deployment environment based on well-known environment variables.
  </Card>
</CardGroup>

## Source

[`lib/signalwire/core/logging_config.rb`](https://github.com/signalwire/signalwire-ruby/blob/main/lib/signalwire/core/logging_config.rb)

Line 20.
