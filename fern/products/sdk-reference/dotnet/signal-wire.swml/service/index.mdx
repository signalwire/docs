...
Transport selection mirrors Python's `SecurityConfig` /
uvicorn `ssl_certfile`/`ssl_keyfile` path:

&lt;ul>&lt;li>Plain HTTP uses System.Net.HttpListener (BCL, no extra deps).&lt;/li>&lt;li>HTTPS (when `SWML_SSL_ENABLED` is truthy and
  `SWML_SSL_CERT_PATH`/`SWML_SSL_KEY_PATH` point at a valid
