---
slug: "/reference/dotnet/signal-wire.agent"
title: "SignalWire.Agent"
sdk_label: ".NET SDK"
icon: "dotnet"
lustri:
  auto_generated: true
  kind: "module"
  language: "dotnet"
  qualified_name: "SignalWire.Agent"
---
# `SignalWire.Agent`

## Classes

<CardGroup cols={2}>
  <Card title="AgentBase" href="/docs/sdk-reference/reference/dotnet/signal-wire.agent/agent-base">
    AI agent built on &lt;xref href="SignalWire.SWML.Service" data-throw-if-not-resolved="false">&lt;/xref>. Provides prompt management, SWAIG tool dispatch, context switching, skill stubs, and a 5-phase SWML rendering pipeline. All configuration methods return `this` for fluent chaining.
  </Card>

  <Card title="AgentOptions" href="/docs/sdk-reference/reference/dotnet/signal-wire.agent/agent-options">
    Configuration options for an AI agent, extending the base SWML service options.
  </Card>
</CardGroup>
