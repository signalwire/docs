---
slug: "/reference/dotnet/core/security/webhook-validation-middleware"
...
HTTP middleware that validates the `X-SignalWire-Signature` header
...
Pull `X-SignalWire-Signature` from request headers, or the
`X-Twilio-Signature` alias for cXML/Compat callers. Header
...
&lt;ol>&lt;li>`SWML_PROXY_URL_BASE` env var (joined with path + query).&lt;/li>&lt;li>`X-Forwarded-Proto` / `X-Forwarded-Host` headers
    when `trustProxy=true`.&lt;/li>&lt;li>`http://hostFallback:portFallback&#123;path&#125;` as a last-resort
...
`SignalWire.SWML.Service.Run` already does this, so AgentBase
...
<ParamField path="trustProxy" type="bool" default="false">
  When true, honor `X-Forwarded-Proto` / `X-Forwarded-Host` when reconstructing the URL. Default false because proxy headers are spoofable; opt in only when you control the proxy chain.
</ParamField>
