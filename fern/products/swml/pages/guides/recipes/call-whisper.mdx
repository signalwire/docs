---
id: a5787bf4-cc0d-4f62-8304-8a8b3543ed05
title: Call whisper
slug: /guides/call-whisper
...
[SWML script resources](swml).
 One will utilize the [`connect`](swml/reference/connect)
method to connect the caller to the recipient, while the other will use the [`play`](swml/reference/play) method to play the Call Whisper message.
...
[`play`](swml/reference/play) method and using the `${call.from}` variable to get the caller's phone number.
...
Inside the [`connect`](swml/reference/connect) method, you can use the
...
After setting up your [SWML](swml) script resources, you can now use them to perform a Call Whisper.
