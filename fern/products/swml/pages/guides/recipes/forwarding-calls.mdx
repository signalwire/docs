...
In [Making and receiving phone calls](/swml/guides/recipes/making-and-receiving-calls) we learned how to use SWML
...
We used the [`connect`](/swml/reference/connect) method to call the number you would like the call to be forwarded to.
Notice how we used its `from` parameter to ensure that the number of the original caller
(stored in the [`call.from` variable](/swml/reference/variables#field-from))
