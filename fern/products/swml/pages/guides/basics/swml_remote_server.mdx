...
In the [Quickstart](/swml/quickstart) guide, we learned how to use SWML Scripts in the Dashboard to handle incoming calls serverlessly.
...
You can [`execute`](/swml/methods/execute) or [`transfer`](/swml/methods/transfer) the control flow out of one SWML to another. In such case, in addition to the `Call` object,
