...
The [`data_map`](swml/guides/data-map) object is a crucial component of
[SWAIG Functions](swml/reference/ai/swaig/functions) in the
[`ai`](swml/reference/ai) SWML method.
...
[Expressions](swml/reference/ai/swaig/functions/data-map#expressions)
...
Multiple expressions can be stated inside the [`data_map.expressions`](swml/reference/ai/swaig/functions/data-map#expressions) field.
...
[Webhooks](swml/reference/ai/swaig/functions/data-map#webhooks)
...
- **URL Construction:** The `url` field specifies the endpoint to which the HTTP request should be made.
  - In the example, `https://icanhazdadjoke.com/search?term={lc:args.type}` constructs the URL with a query parameter `term` based on the `args.type` value.
    ...
    - `args.target` is a  [`SWAIG.function`](swml/reference/ai/swaig/functions) argument that contains the target value passed by the user when interacting with the AI agent.
...
[Outputs](swml/reference/ai/swaig/functions/data-map#output) define the responses and actions that should be taken when an expression matches or a webhook request completes.
...
[Actions](swml/reference/ai/swaig/functions/data-map#output#actions) in `data_map` define what should happen when the `swaig.function` executes and
...
For complete technical specifications, types, and properties, see the [SWAIG Variables Reference](swml/reference/ai/swaig/functions/data-map).

For general SWML variable scopes like `call`, `params`, and `vars`, see the [Variables Reference](swml/reference/variables).
...
[SWAIG function](swml/reference/ai/swaig/functions) arguments are processed as
...
through the SWML [`set_global_data`](swml/reference/ai/swaig/functions/data-map#output)
or [`set_meta_data`](swml/guides/set-meta-data) actions.
...
SWML variables (`vars`) store data created by a SWML [`request`](swml/reference/request)
or [`set`](swml/reference/set) method.
