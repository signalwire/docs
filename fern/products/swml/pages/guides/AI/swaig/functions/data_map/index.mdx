...
<Callout type="tip" title="no server required">
...
<Callout type="tip" title="output levels">
...
```
<Callout type="warning" title="When passing a SWML object that includes a `connect` (or other method that transfers the call) in `output.action[]`, your script must be JSON, and `"transfer": true,` must be included before the SWML in the same action object. Refer to the example above.">
...
<Callout type="tip" title="Replace Placeholders">
