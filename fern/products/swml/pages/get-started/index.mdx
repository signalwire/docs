...
<ParamField path="sections.main" type="array" required={true} toc={true}>
  The entry-point section. Execution starts here when the document loads.
</ParamField>

<ParamField path="sections.<name>" type="array" toc={true}>
  Any additional named section, invokable via `execute`, `transfer`, or `goto` from elsewhere
  in the document.
</ParamField>
