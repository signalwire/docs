...
<Indent>
  <ParamField path="main" type="object[]" required>
    ...
    Accepts an array of [SWML methods](/swml/reference).
  </ParamField>
  <ParamField path="*" type="object[]" optional>
    Additional user-defined sections, invoked from `main` using [program flow control methods](/swml/guides/goto-execute-transfer-disambiguation).
    
    User-defined sections can use any text string as the property. Like `main`, user-defined sections accept an array of [SWML methods](/swml/reference).
  </ParamField>
</Indent>
...
- **[`execute`](/swml/reference/execute)**: Calls a section like a function and returns to the current section
- **[`transfer`](/swml/reference/transfer)**: Transfers control permanently to another section
...
When a SWML script is hosted on an external URL (set in phone number settings, or invoked via [`execute`](/swml/reference/execute) or [`transfer`](/swml/reference/transfer)),
...
Several SWML methods also use this same request format when fetching SWML from a URL (e.g., [`join_conference.wait_url`](/swml/reference/join-conference#properties), [`enter_queue.wait_url`](/swml/reference/enter-queue#properties), [`connect.confirm`](/swml/reference/connect#properties)).

### Request body

<ParamField path="call" type="object">
  Information about the current call, including `call_id`, `from`, `to`, `type`, `headers`, and more. See the [Variables reference](/swml/reference/variables) for all `call` properties.
</ParamField>


<ParamField path="vars" type="object">
  Script variables set by prior SWML execution (via [`set`](/swml/reference/set) or method outputs). Empty on a fresh incoming call.
</ParamField>


<ParamField path="params" type="object">
  Parameters explicitly passed by [`execute`](/swml/reference/execute) or [`transfer`](/swml/reference/transfer). Empty on a fresh incoming call.
</ParamField>
...
For the full variable reference and a detailed JSON example, see [Variables](/swml/reference/variables#example-variables-in-json-format).
...
For an extensive breakdown of this example, see our [IVR Creation Guide](/swml/guides/ivr).
...
## Next steps
  ...
  <Cards>
    <Card title="Quickstart" href="/swml/guides">
      Quickly create and deploy SWML through the SignalWire Dashboard
    </Card>
    <Card title="Methods reference" href="/swml/reference">
      Explore all available SWML methods
    </Card>
    <Card title="AI guides" href="/swml/guides">
