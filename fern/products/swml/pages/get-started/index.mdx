...
For a comprehensive breakdown, see our [SWML deployment guide](/swml/guides/deployment).
...
<ParamField toc={true} path="version" type="string" optional>
  Always use `1.0.0` (the only supported version).
</ParamField>

<ParamField toc={true} path="sections" type="object" required>
  Contains all script sections.
</ParamField>
<Indent>
  <ParamField toc={true} path="main" type="object[]" required>
    Required starting section where execution always begins.
    Accepts an array of [SWML methods](/swml/methods).
  </ParamField>
  <ParamField path="*" type="object[]" optional>
    Additional user-defined sections, invoked from `main` using [program flow control methods](/swml/guides/methods/goto_execute_transfer_disambiguation).
    
    User-defined sections can use any text string as the property. Like `main`, user-defined sections accept an array of [SWML methods](/swml/methods).
  </ParamField>
</Indent>
...
When a SWML script is hosted on an external URL (set in phone number settings, or invoked via [`execute`](/swml/methods/execute) or [`transfer`](/swml/methods/transfer)),
...
Several SWML methods also use this same request format when fetching SWML from a URL (e.g., [`join_conference.wait_url`](/swml/methods/join_conference#properties), [`enter_queue.wait_url`](/swml/methods/enter_queue#properties), [`connect.confirm`](/swml/methods/connect#properties)).

### Request body

<ParamField path="call" type="object">
  Information about the current call, including `call_id`, `from`, `to`, `type`, `headers`, and more. See the [Variables reference](/swml/variables) for all `call` properties.
</ParamField>


<ParamField path="vars" type="object">
  Script variables set by prior SWML execution (via [`set`](/swml/methods/set) or method outputs). Empty on a fresh incoming call.
</ParamField>


<ParamField path="params" type="object">
  Parameters explicitly passed by [`execute`](/swml/methods/execute) or [`transfer`](/swml/methods/transfer). Empty on a fresh incoming call.
</ParamField>
...
For the full variable reference and a detailed JSON example, see [Variables](/swml/variables#example-variables-in-json-format).
For server setup instructions, see the [Deployment guide](/swml/guides/deployment).
...
## Next steps
  ...
  <Cards>
    <Card title="Quickstart" href="/swml/quickstart">
    ...
    <Card title="AI guides" href="/swml/guides/ai">
