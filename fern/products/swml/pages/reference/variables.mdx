...
<ParamField path="params" type="object" toc={true}>
  Parameters that are user-defined and set by the calling [`execute`](/docs/swml/reference/calling/execute) or [`transfer`](/docs/swml/reference/calling/transfer) method.
...
<ParamField path="vars" type="object" toc={true}>
  ...
  **Scope:** Global within a single call session. Variables persist across all sections and through `execute` calls. However, connecting to a new leg of a call resets the `vars` object to an empty state.
  ...
  **Scope:** Message-specific and read-only. There is no persistent state between inbound messages.
  ...
  <Note>
    ...
    */}
    ...
    `segments`, and `timestamp`), see the SWML inbound message webhook reference.
  </Note>
</ParamField>
...
{/* TODO: Restore the inline link below once the SWML messaging schema PR merges and
    ...
    [SWML inbound message webhook](/docs/apis/rest/swml-webhook/webhooks/inbound-message-webhook)
*/}
See the SWML inbound message webhook page for the full field reference.
