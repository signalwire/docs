...
<ParamField path="message" type="object" toc={true}>
  Information about the inbound message that triggered the SWML document.

  **Scope:** Message-specific and read-only. Each inbound message produces its own `message` object.
  There is no persistent state between inbound messages.
</ParamField>
  ...
  <Indent>
    ...
    <ParamField path="message.direction" type="string" toc={true}>
      Always `inbound` for messages handled by an SWML messaging script.
    </ParamField>

    <ParamField path="message.type" type="string" toc={true}>
      The message channel type. Possible values: `sms`, `mms`.
    </ParamField>
    ...
    <ParamField path="message.media" type="object[]" toc={true}>
      Array of MMS media attachments. Empty for SMS-only messages.
    </ParamField>
