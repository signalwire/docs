...
<Warning title="Calling SWML only">
  Expressions (`${...}`) are a [Calling SWML](/docs/swml/reference/calling) feature.
  [Messaging SWML](/docs/swml/reference/messaging) supports only pure variable substitution via
  `%{path.to.value}` — no JavaScript, no method calls, no operators. For dynamic behavior in a
  messaging document, use [`switch`](/docs/swml/reference/messaging/switch) for branching and
  [`request`](/docs/swml/reference/messaging/request) for fetching computed values from your server.
</Warning>
