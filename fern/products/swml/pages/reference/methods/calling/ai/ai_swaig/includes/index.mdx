---
id: bf4cb7f8-d106-4b0a-b30b-00261a566af8
slug: /reference/calling/ai/swaig/includes
...
[webhook-response]: /docs/swml/reference/calling/ai/swaig/functions#reply
[webhook-request]: /docs/swml/reference/calling/ai/swaig/functions#request
...
<ParamField path="includes[].url" type="string" required={true} toc={true}>
  ...
  Credentials embedded in the URL take precedence over `auth_user` and `auth_password`.
</ParamField>

<ParamField path="includes[].functions" type="string[]" required={true} toc={true}>
  An array of the function names to be included.
</ParamField>

<ParamField path="includes[].auth_user" type="string" toc={true}>
  Username for HTTP basic authentication on the signature request, as an alternative to embedding
  credentials in `url`.
</ParamField>

<ParamField path="includes[].auth_password" type="string" toc={true}>
  Password for HTTP basic authentication on the signature request.
</ParamField>

<ParamField path="includes[].meta_data" type="object" toc={true}>
...
Functions can trigger various behaviors: speaking text, sending SMS, playing audio, transferring calls, and more. See the [actions documentation](/docs/swml/reference/calling/ai/swaig/functions#list-of-valid-actions) for all available options.
</Tip>
