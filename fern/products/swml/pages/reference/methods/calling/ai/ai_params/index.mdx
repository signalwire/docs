...
<ParamField path="params.enable_turn_detection" type="string" default="both" toc={true}>
  ...
  Every mode except `off` is tuned by [`turn_detection_min_length`](#paramsturn_detection_min_length) and [`turn_detection_timeout`](#paramsturn_detection_timeout). The parameter is also accepted under the name `turn_detection`.
</ParamField>
...
<ParamField path="params.turn_detection_min_length" type="integer" default="0" toc={true}>
  ...
  The acoustic signal overrides this: when it reads the caller as finished, the turn ends whatever the length, so short complete replies like "Yes." still work. That leaves this a filter on the punctuation path alone, and it has no effect when [`enable_turn_detection`](#paramsenable_turn_detection) is `off`.
</ParamField>
