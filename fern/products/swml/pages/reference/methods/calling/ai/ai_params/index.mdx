...
<ParamField path="params.utility_model" type="string" default="Value of ai_model parameter" toc={true}>
  The AI model used for lightweight background tasks such as redaction (`redact_prompt`) and transcription cleanup (`auto_correct`). Allowed values are `gpt-4o-mini`, `gpt-4.1-mini`, and `gpt-4.1-nano`. Choose a small, fast model — these tasks run while the caller is waiting for a response.
</ParamField>
