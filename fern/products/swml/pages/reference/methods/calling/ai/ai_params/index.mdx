...
<ParamField path="params.languages_enabled" type="boolean" default="false" toc={true}>
  Turns on the multi-language handling that goes with a [`languages`][ai-languages] array, so the agent can move between the languages you listed.

  This does not enable [`multilingual`][ai-multilingual], which configures its own languages and is switched on by being set.
</ParamField>
