...
Set `debug_webhook_url` to get a live feed of what the agent is doing while the call is still
running. Use it to act mid-call, such as paging a supervisor or flagging a handoff. For analysis
after the call, use the
...
the rest, since the set grows over time. An event with no data arrives as `null`, so test whether the
property is present rather than what it holds.
