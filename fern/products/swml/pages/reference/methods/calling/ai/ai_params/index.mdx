...
<ParamField path="params.languages_enabled" type="boolean" default="false" toc={true}>
...
<ParamField path="params.turn_filler_every_n" type="integer" default="1" toc={true}>
  Play a backchannel on every Nth opportunity rather than all of them. The count advances only on opportunities that would otherwise have produced one, and the first of each wait always plays, so this widens the spacing during a long wait without delaying the first response.
</ParamField>
...
<ParamField path="params.turn_filler_sources" type="string" default="spell,number" toc={true}>
  Which deliberate waits earn a backchannel from [`languages[].turn_fillers`](/docs/swml/reference/calling/ai/languages#languagesturn_fillers), as a comma-separated list of exact tokens. A wait happens when the agent holds off answering because the caller is not finished, and the token names why.
