...
<ParamField path="params.redact_prompt" type="string" toc={true}>
  A plain-language description of content to redact from everything the platform records or transmits about the call — logs, events, webhook payloads, the call timeline, and the post-conversation `call_log` and `raw_call_log`. For example: `"credit card numbers, social security numbers, and full names"`. A non-empty value enables redaction, and redacted content appears as `----`; omit the parameter to disable redaction.

  The caller still hears the content in full, and **the AI model still receives the real text each turn** — redaction protects what is recorded and transmitted, not what the model processes. Redaction is best-effort.
</ParamField>
...
<ParamField path="params.utility_model" type="string" default="Value of ai_model parameter" toc={true}>
  The model used for lightweight utility passes such as redaction (`redact_prompt`) and input cleanup (`auto_correct`). Allowed values are `gpt-4o-mini`, `gpt-4.1-mini`, and `gpt-4.1-nano`. Use a small, fast model — these passes run during the conversation turn.
</ParamField>
...
<ParamField path="params.auto_correct" type="boolean" default="false" toc={true}>
  Cleans up the transcription of caller speech before each AI turn — converting spoken numbers to digits, formatting addresses and phone numbers, and fixing obvious mishearings — without changing the meaning. Mutually exclusive with `enable_text_normalization`, which is on by default: set `enable_text_normalization` to `false` to use `auto_correct`, otherwise `auto_correct` is ignored. When combined with `redact_prompt`, cleanup and redaction run in a single pass, avoiding an extra model round-trip per turn.
</ParamField>
