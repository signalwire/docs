...
Every request carries `call_info` plus exactly one more key, and that key's name is the event. There
is no `type` field to switch on: read the key that is not `call_info`. New event names are added
over time, so ignore any you do not recognize.
