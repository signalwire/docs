...
<ParamField path="params.enable_text_normalization" type="string" default="both" toc={true}>
  Controls NeMo-based text normalization between written and spoken forms. Converts spoken-form ASR input to written form before the LLM (Inverse Text Normalization), and/or written-form LLM output to spoken form before TTS (Text Normalization). Enabled in both directions by default. See [accepted values](#text-normalization-values) below.
</ParamField>
...
|---|---|---|
| `heard` | ASR → LLM | Applies Inverse Text Normalization (ITN) to speech-recognition output before the LLM (spoken form → written form, e.g. `twenty three dollars` → `$23`). |
| `spoken` | LLM → TTS | Applies Text Normalization (TN) to LLM output before TTS (written form → spoken form, e.g. `$23` → `twenty three dollars`). |
| `both` | Both | Applies ITN and TN. This is the default. |
| `false` / `off` / `none` | — | Disables text normalization. |
| `true` / `on` | Both | Aliases for `both`. |

Normalization grammars load lazily for the active conversation language and reload when the language changes.

<Note>
If normalization grammars are not available for the active language, that direction is skipped, a `calling.error` event is emitted, and the conversation continues. Some languages support only one direction; for those, `both` applies whichever direction is available.
</Note>
