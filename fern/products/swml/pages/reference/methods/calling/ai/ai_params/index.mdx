...
Setting `debug_webhook_url` streams events to your endpoint as they happen during the call, rather
than once at the end. Use it when something has to happen mid-call, such as paging a supervisor or
flagging a handoff. For after-the-fact analysis, use the
[post_prompt_url callback](/docs/swml/reference/calling/ai#post_prompt_url-callback) instead.

Every request carries `call_info` alongside one or more event properties, and the property name is
the event. There is no `type` field to switch on. Most requests carry a single event, but some carry
several that describe one moment together: a [`data_map`](/docs/swml/reference/calling/ai/swaig/functions/data-map)
webhook evaluation sends `webhook`, `input`, `output`, `error_keys`, and `match` in one body. Handle
each property you recognize and ignore the rest.

An event that carried no data arrives as `null` rather than being left out, so check whether the
property is present rather than whether it is truthy. New events are added over time.
