...
<ParamField path="languages[].code" type="string" required={true} toc={true}>
...
<Info> If a different STT model was selected using the [`openai_asr_engine` parameter](/docs/swml/reference/calling/ai/params#paramsopenai_asr_engine), you must select a code supported by that engine.</Info>
</ParamField>
...
<ParamField path="languages[].emotion" type="string" default="None" toc={true}>
  Enables automatic emotion for the set TTS engine. This allows the AI to express emotions when speaking. A global emotion or specific emotions for certain topics can be set within the prompt of the AI.
*Valid values:* `auto`
...
<ParamField path="languages[].speed" type="string" default="None" toc={true}>
