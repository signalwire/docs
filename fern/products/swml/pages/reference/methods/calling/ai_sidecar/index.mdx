...
<ParamField path="ai_sidecar.prompt" type="string | object" toc={true}>
  ...
  May be a plain string or a Prompt Object Model (POM). SignalWire automatically adds built-in instructions for the sidecar's role, so your prompt only needs to describe the coaching behavior.
...
took in total. Any callback produced while answering an [`ai_sidecar.ask`](/docs/apis/rest/calls/call-commands) also
...
regular [`ai`](/docs/swml/reference/calling/ai) method's
[SWAIG webhook](/docs/swml/reference/calling/ai/swaig/functions#tool-webhook), and it groups the
