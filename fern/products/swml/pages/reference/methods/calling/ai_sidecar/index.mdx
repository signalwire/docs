...
hide_title: false
slug: /reference/calling/ai_sidecar
title: ai_sidecar
description: Attach a real-time AI observer that streams agent-facing advice events during a live call.
max-toc-depth: 3
---

[params]: /docs/swml/reference/calling/ai_sidecar/params
[prompt]: /docs/swml/reference/calling/ai_sidecar/prompt
[SWAIG]: /docs/swml/reference/calling/ai_sidecar/swaig
[act_on_channel]: /docs/swml/reference/calling/ai_sidecar/params#paramsact_on_channel
...
<ParamField path="ai_sidecar.url" type="string" toc={true}>
  ...
  When unset, no webhook callbacks are sent.
...
As it observes the call, the sidecar reports each step of its activity as a **webhook callback** — an HTTP `POST` to the `url` you set on the verb, with the callback body wrapped under `sidecar_event`. Callbacks are sent only when `url` is set.
