...
<ParamField path="argument" type="object" toc={true}>
  The arguments the model passed, in the standard SWAIG argument shape: `parsed` (an array of the parsed
  argument objects), `raw` (the raw JSON string the model produced), and `substituted` (the raw string after
  variable substitution).
</ParamField>

<ParamField path="call_id" type="string" toc={true}>
  ID of the call the sidecar is attached to.
</ParamField>
...
<ParamField path="channel_data" type="object" toc={true}>
...
<Note>
This is the **sidecar** tool-webhook shape. It is intentionally narrower than the regular
[`ai`](/docs/swml/reference/calling/ai) method's SWAIG webhook — the sidecar does not send
`app_name`, `version`, `content_type`, `meta_data`, `argument_desc`, `project_id`/`space_id`,
