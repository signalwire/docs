...
<ParamField path="connect.from_name" type="string" toc={true}>
  The caller ID name shown to the person you're calling, displayed alongside the `from` number (sometimes called CNAM). Applies to SIP calls only — it has no effect on calls to phone numbers. Can be overridden on each [destination](#properties-for-destination).
</ParamField>
...
<ParamField path="connect.confirm_timeout" type="integer" toc={true}>
...
<ParamField path="connect.encryption" type="string" toc={true}>
