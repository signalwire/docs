...
Return a JSON object with a `response` key and an optional `action` key. The `response` gives the agent
its next prompt input, and `action` carries SWML-compatible objects that perform new dialplan actions.
