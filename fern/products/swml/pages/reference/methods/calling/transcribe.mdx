...
When you set `status_url`, SignalWire POSTs a transcript event to it when the call's transcription finishes: `calling.transcript.completed` with the transcribed `text` on success, or `calling.transcript.failed` if the call could not be transcribed (in which case `text` is omitted).
