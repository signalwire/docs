...
When you set `status_url`, SignalWire POSTs a `calling.transcript.completed` event to it once the call's transcription is ready. The payload carries the transcribed `text`.

<WebhookPayloadSnippet webhook="transcribeStatusCallback" />

See the [Transcript completed callback](/docs/apis/rest/calls/webhooks/transcribe-status-callback) webhook page for the full field reference.
