...
After the bridge completes (when the agent or caller hangs up), execution continues with the SWML script specified in `execute_after_queue`.
...
<ParamField path="enter_queue.execute_after_queue" type="string" toc={true}>
  SWML to execute when the queue interaction ends (when the agent or caller hangs up). This defines what should happen after the call is connected to an agent and the bridge ends.
...
<ParamField path="enter_queue.wait_time" type="integer" default="3600" toc={true}>
