...
title: enter_queue
slug: reference/calling/enter-queue
...
After the bridge completes (when the agent or caller hangs up), execution continues with the SWML script specified in `execute_after_queue`, or the call hangs up if none was given.

## **Properties**

<ParamField path="enter_queue" type="object" required={true} toc={true}>
  An object that accepts the following properties.
</ParamField>

<Indent>

<ParamField path="enter_queue.queue_name" type="string" required={true} toc={true}>
...
<ParamField path="enter_queue.execute_after_queue" type="string" toc={true}>
  SWML to execute after the bridge with the agent ends. When omitted, the call hangs up when the bridge ends.
  If the call leaves the queue without being bridged (`wait_time` expires or the caller hangs up), this field is not used and the script continues with the next instruction; check `queue_result`.
...
<ParamField path="enter_queue.status_url" type="string" toc={true}>
...
<ParamField path="enter_queue.wait_url" type="string" toc={true}>
...
<ParamField path="enter_queue.wait_time" type="integer" default="180" toc={true}>
...
<ParamField path="params.id" type="string" toc={true}>
...
<ParamField path="params.position" type="integer" toc={true}>
...
<ParamField path="params.avg_time" type="integer" toc={true}>
  Average wait time in the queue (in seconds).
</ParamField>


<ParamField path="params.enqueue_ts" type="integer" toc={true}>
  Unix timestamp in microseconds when the caller entered the queue. Set to `0` in `dequeue` events.
</ParamField>


<ParamField path="params.dequeue_ts" type="integer" toc={true}>
...
<ParamField path="params.leave_ts" type="integer" toc={true}>
