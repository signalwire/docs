...
<ParamField toc={true} path="expressions[].string" type="string" required={true}>
  The actual input or value from the user or system.
</ParamField>


<ParamField toc={true} path="expressions[].pattern" type="string" required={true}>
  A regular expression pattern to validate or match the string.
</ParamField>


<ParamField toc={true} path="expressions[].output" type="object" required={true}>
