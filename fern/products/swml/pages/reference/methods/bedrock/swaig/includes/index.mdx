...
<ParamField toc={true} path="SWAIG.includes" type="object[]">
  An array of objects that contain the [`includes parameters`][includes-parameters].
</ParamField>

## **includes parameters**

<ParamField toc={true} path="includes[].url" type="string" required={true}>
...
<ParamField toc={true} path="includes[].function" type="string[]" required={true}>
  An array of the function names to be included.
</ParamField>

<ParamField toc={true} path="includes[].meta_data" type="object">
...
```mermaid
sequenceDiagram
    participant Platform as SignalWire Platform
(SWML + SWAIG + AI)
    participant Server as Your Server
    participant User as User

    Note over Platform,Server: Initialization Phase
    Platform->>Server: POST /swaig
{"action": "get_signature", "functions": ["get_weather"]}
    ...
    Platform->>Server: POST /swaig
{"function": "get_weather", "argument": {"parsed": [{"city": "New York"}]}}
