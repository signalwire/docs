...
slug: /reference/amazon-bedrock/swaig
title: SWAIG
...
<ParamField path="defaults.web_hook_url" type="string">
  The default URL to send status callbacks and reports to for all SWAIG functions. If `defaults` is not set, `web_hook_url` may be set in each [SWAIG function](/swml/reference/amazon-bedrock/swaig/functions) object. Authentication can also be set in the url in the format of `username:password@url`.
</ParamField>
</Indent>
...
<ParamField path="SWAIG.functions" type="object[]">
  ...
  See [`functions`](/swml/reference/amazon-bedrock/swaig/functions) for additional details.
</ParamField>
...
<ParamField path="SWAIG.includes" type="object[]">
  ...
  See [`includes`](/swml/reference/amazon-bedrock/swaig/includes) for additional details.
</ParamField>
