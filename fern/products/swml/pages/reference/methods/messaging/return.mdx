...
Both bare `- return` and `- return: <value>` forms are supported.

## **Properties**

<ParamField path="return" type="any" required={true} toc={true}>
  The value to return. Accepts any type. Use `null` (or omit a value with the bare form) to return no
  value.
</ParamField>
...
### Bare return (exit a section with no value)
