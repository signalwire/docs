...
Record call in the background. Unlike the [`record` method](swml/reference/record), the `record_call` method
will start the recording and continue executing the SWML script while allowing the recording to happen in the background.
To stop call recordings started with `record_call`, use the [`stop_call_record`](swml/reference/stop-record-call) method.
...
<ParamField path="record_call.control_id" type="string" default="Auto-generated, saved to record_control_id variable">
  Identifier for this recording, to use with [`stop_record_call`](swml/reference/stop-record-call)
</ParamField>
