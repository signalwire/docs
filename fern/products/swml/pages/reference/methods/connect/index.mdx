...
<ParamField path="connect" type="object" required={true}>
  Connects the current call to a destination — a phone number, SIP URI, Call Fabric Resource, queue, or WebSocket stream. Supports single, serial, parallel, and serial-parallel dialing strategies.
</ParamField>


## **Properties**

A destination is required. Use `to` to dial a single destination, `serial` to try destinations one at a time, `parallel` to dial multiple destinations at once, or `serial_parallel` to combine both strategies. Only one can be used per connect.
