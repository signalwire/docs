...
<ParamField toc={true} path="connect" type="object" required={true}>
...
<ParamField toc={true} path="connect.answer_on_bridge" type="boolean" default="false">
  Delay answer until the B-leg answers.
</ParamField>


<ParamField toc={true} path="connect.call_state_events" type="string[]" default="['ended']">
...
<ParamField toc={true} path="connect.call_state_url" type="string">
...
<ParamField toc={true} path="connect.codecs" type="string" default="Based on SignalWire settings">
  Comma-separated string of codecs to offer. Has no effect on calls to phone numbers.
</ParamField>


<ParamField toc={true} path="connect.confirm" type="string | object[]">
...
<ParamField toc={true} path="connect.confirm_timeout" type="integer" default="Inherits from timeout">
  The amount of time, in seconds, to wait for the `confirm` script to execute.
</ParamField>


<ParamField toc={true} path="connect.encryption" type="string" default="optional">
  The encryption method to use for the call.
</ParamField>

**Possible values:** `mandatory`, `optional`, `forbidden`.

<ParamField toc={true} path="connect.from" type="string" default="Calling party's caller ID number">
...
<ParamField toc={true} path="connect.headers" type="object[]">
  Custom SIP headers to add to INVITE. Has no effect on calls to phone numbers. The object accepts [headers properties](./headers.mdx#properties).
  
  See [`headers`](./headers.mdx) for details.
</ParamField>


<ParamField toc={true} path="connect.max_duration" type="integer" default="14400 seconds (4 hours)">
  Maximum duration, in seconds, allowed for the call.
</ParamField>


<ParamField toc={true} path="connect.result" type="object | object[]">
  Action to take based on the result of the call. This will run once the peer leg of the call has ended.
  
  Will use the [`switch properties`](./switch#properties) when the `return_value` is a object, and will use the [`cond properties`](./cond#properties) method when the `return_value` is an array.
</ParamField>

 See [Variables](#variables) for details.

<ParamField toc={true} path="connect.ringback" type="string[]" default="Plays audio from the provider">
  Array of `play` URIs to play as ringback tone.
</ParamField>


<ParamField toc={true} path="connect.session_timeout" type="integer" default="Based on SignalWire settings">
...
<ParamField toc={true} path="connect.status_url" type="string">
  HTTP or HTTPS URL to deliver connect status events. These events report the overall status of the connect operation (connecting, connected, failed, disconnected). See [Connect Status Callbacks](#connect-status-callbacks) for event details.
</ParamField>


<ParamField toc={true} path="connect.timeout" type="integer" default="60 seconds">
  Maximum time, in seconds, to wait for an answer.
</ParamField>


<ParamField toc={true} path="connect.transfer_after_bridge" type="string">
...
<ParamField toc={true} path="connect.to" type="string">
...
<ParamField toc={true} path="connect.serial" type="object[]">
  Array of [`destination`](#properties-for-destination) objects to dial in order.
</ParamField>


<ParamField toc={true} path="connect.parallel" type="object[]">
  Array of [`destination`](#properties-for-destination) objects to dial simultaneously.
</ParamField>


<ParamField toc={true} path="connect.serial_parallel" type="object[][]">
  Array of arrays. Inner arrays contain [`destination`](#properties-for-destination) objects to dial simultaneously. Outer array attempts each parallel group in order.
</ParamField>


## **Properties for `destination`**

<ParamField toc={true} path="connect.to" type="string" required={true}>
...
<ParamField toc={true} path="connect.confirm_timeout" type="integer">
...
<ParamField toc={true} path="connect.encryption" type="string">
...
## **Stream destination properties**

When `to` starts with `stream:wss://`, the destination connects to a bidirectional WebSocket audio stream instead of dialing a phone or SIP endpoint.
These additional properties are available on stream destinations.

<ParamField toc={true} path="connect.authorization_bearer_token" type="string">
  Bearer token sent as an `Authorization` header during the WebSocket handshake.
</ParamField>


<ParamField toc={true} path="connect.codec" type="string" default="PCMU">
...
<ParamField toc={true} path="connect.custom_parameters" type="object">
  Custom key-value pairs sent in the WebSocket start message.
</ParamField>


<ParamField toc={true} path="connect.name" type="string">
  Stream name identifier.
</ParamField>


<ParamField toc={true} path="connect.realtime" type="boolean" default="false">
  Enable realtime mode for bidirectional audio.
</ParamField>


<ParamField toc={true} path="connect.status_url" type="string">
  Webhook URL (HTTPS) for stream status notifications. See [Stream Status Callbacks](#stream-status-callbacks) for event details.
</ParamField>


<ParamField toc={true} path="connect.status_url_method" type="string" default="POST">
...
<ParamField path="params.device.type" type="string">
  The type of device (e.g., `phone`, `sip`).
</ParamField>


<ParamField path="params.device.params.from_number" type="string">
  The originating phone number.
</ParamField>


<ParamField path="params.device.params.to_number" type="string">
