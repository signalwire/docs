...
<ParamField path="start.from_lang" type="string" required={true}>
  ...
  See [supported voices & languages](platform/voice/tts).
</ParamField>
...
You can send this action via the [Call Commands REST API](apis/calling/calls/call-commands),
or include it in a SWML section executed via [`transfer`](swml/reference/transfer) or [`execute`](swml/reference/execute) during a call.
...
**Live call:** Actions sent to active calls via the [Call Commands REST API](apis/calling/calls/call-commands) or SWML sections executed via [`transfer`](swml/reference/transfer) or [`execute`](swml/reference/execute) during a call.
