...
Use the [return](/swml/reference/return) statement to pass any return values or objects back to the current document.
...
- A URL (http or https) - URL pointing to the document to execute. An HTTP POST request will be sent to the URL. The `params` object is passed, along with the variables and the [`Call`](/swml/reference/variables#variables-list) object. Authentication can also be set in the url in the format of `username:password@url`.
...
You can follow our [Testing webhooks with ngrok](/platform/basics/how-to-test-with-ngrok) guide to learn how.
...
The `call` object is described in detail in the [introduction](/swml/reference/variables#variables-list). All variables created
