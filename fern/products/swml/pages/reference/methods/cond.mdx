...
<ParamField toc={true} path="cond" type="object[]" required={true}>
...
### **Parameters for `when-then` conditions**

<ParamField toc={true} path="cond[].when" type="string" required={true}>
  The JavaScript condition to act on
</ParamField>


<ParamField toc={true} path="cond[].then" type="object[]" required={true}>
  Sequence of [`SWML Methods`](./index.mdx) to execute when the condition evaluates to `true`
</ParamField>


### **Parameters for `else` condition**

<ParamField toc={true} path="cond[].else" type="object[]">
