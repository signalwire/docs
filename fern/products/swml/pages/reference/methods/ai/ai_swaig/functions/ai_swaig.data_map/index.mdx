...
<ParamField path="webhooks[].input_args_as_params" type="boolean" default="false">
  A boolean to determine if the input [parameters](/swml/methods/ai/swaig/functions/parameters) should be passed as parameters.
</ParamField>

<ParamField path="webhooks[].required_args" type="string | string[]">
  A string or array of strings that represent the [parameters](/swml/methods/ai/swaig/functions/parameters) that are required to make the webhook request.
</ParamField>
...
The `url` field supports `%{args.<name>}` substitution, where `<name>` matches a property defined in the function's [`parameters`](/swml/methods/ai/swaig/functions/parameters) schema.
...
- [Executing SWML from a SWAIG function](/swml/guides/ai/executing_swml)
