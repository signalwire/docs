...
`functions[].data_map` defines how a [`SWAIG function`](swml/reference/ai/swaig/functions) should process and respond to the user's input data.
...
<ParamField path="webhooks[].input_args_as_params" type="boolean" default="false">
  A boolean to determine if the input [parameters](swml/reference/ai/swaig/functions/parameters) should be passed as parameters.
</ParamField>

<ParamField path="webhooks[].required_args" type="string | string[]">
  A string or array of strings that represent the [parameters](swml/reference/ai/swaig/functions/parameters) that are required to make the webhook request.
</ParamField>
...
<ParamField path="args.*" type="any">
  Individual argument value. The key matches an argument name from the function's [`parameters`](swml/reference/ai/swaig/functions/parameters) schema.
</ParamField>
...
- [Executing SWML from a SWAIG function](swml/guides/executing-swml)
