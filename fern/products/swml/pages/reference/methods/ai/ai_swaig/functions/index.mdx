...
**functions[].active** (type: boolean) Default: `true`
  Whether the function is active.

**functions[].data_map** (type: object)
...
**functions[].skip_fillers** (type: boolean) Default: `false`
...
**functions[].meta_data** (type: object)
...
**functions[].meta_data_token** (type: string) Default: `Set by SignalWire`
  Scoping token for `meta_data`. If not supplied, metadata will be scoped to function's `web_hook_url`.

**functions[].wait_file** (type: string)
...
**functions[].purpose** (type: string) *deprecated*
  Deprecated. Use `description` instead.

**functions[].argument** (type: object) *deprecated*
