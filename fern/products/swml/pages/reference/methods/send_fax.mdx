...
**send_fax** (type: object) *required*
  An object that accepts the [`send_fax parameters`](#send_fax-parameters).

## **send_fax parameters**

**send_fax.document** (type: string) *required*
  URL to the PDF document to fax

**send_fax.header_info** (type: string)
  Text to add to the fax header

**send_fax.identity** (type: string) Default: `Calling party's caller ID number`
  Station identity to report

**send_fax.status_url** (type: string)
