...
Some codes are warnings and do not stop the document. When a document raises several while
loading, each fires its own `calling.script.warning` event, and `script_error` holds the first in
document order.
...
<ParamField path="relay_script_voice_unresolved" type="string" toc={true}>
  The requested text-to-speech voice could not be resolved, so playback substitutes a different
  voice. The document keeps running. A voice named in a `play` or `prompt` is reported as the
  document loads; a voice set from a variable is reported at playback, naming the substitute.
</ParamField>
