...
<ParamField path="relay_script_voice_unresolved" type="string" toc={true}>
  The requested text-to-speech voice could not be resolved, so a different voice is substituted.
  The document keeps running, and this is reported only as a `calling.script.warning` event, not in
  `script_error`. A voice named literally in a `play` or `prompt` is reported as the document loads,
  before the call connects. A voice or language set from a variable, and the voice used by `ai`, are
  reported at playback instead, naming the substitute. Each voice is reported once.
</ParamField>
