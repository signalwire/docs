...
[variables]: /call-flow-builder/variables
...
<ParamField path="Enable Whisper" type="boolean">
  This executes [SWML](/swml) when the call is answered. The SWML will be executed before connecting the call.<br /><br />**Handle Via:** `External URL` that is hosted off the SignalWire platform, or a [`SWML`](/swml) script [resource](/platform/call-fabric/resources).
</ParamField>
...
![A Call Flow that uses the Forward to Phone node to forward the call while preserving the original caller ID.](/images/call-flow/nodes/forward-from-original-example.webp)
...
![A Call Flow that uses the Forward to Phone node to forward the call to a sip endpoint.](/images/call-flow/nodes/forward-sip.webp)
...
![Forward to multiple SIP endpoints and phone numbers sequentially using the Forward to Phone node.](/images/call-flow/nodes/forward-mix.webp)
