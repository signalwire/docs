...
For loop protection, route **Gather Input**'s **No Input** output through an apology TTS, then **Set Variables** (`count = %{count} + 1`) and **Conditions** (`%{count} > 2`). The true branch goes to **Hang Up Call**; **Else** loops through a “didn't hear a response” TTS and back to **Gather Input**.
