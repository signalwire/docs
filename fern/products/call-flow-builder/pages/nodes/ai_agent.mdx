---
id: 5edb6ae0-abd0-4526-bde4-b2f0265e5e9c
title: AI Agent
description: Call Flow Builder node to connect to an AI Agent
slug: /reference/ai-agent
# image: "/assets/images/call-flow/nodes/ai_agent_node.webp"
---
This node is used to connect to a SignalWire conversational [AI agent](/platform/ai/no-code-agents).
The AI agent can be used to handle natural language processing, sentiment analysis, make and handle API calls, and other conversational tasks.

<Tip>
Learn more about AI Agents with our comprehensive [Getting Started](/platform/ai/no-code-agents) guide!
</Tip>

## **Node Settings**

<ParamField path="AI Agent Name" type="string">
  The name of the AI agent to connect to. The dropdown menu will populate with any [AI Agent Resource](/platform/call-fabric/ai-agents) that have been created in your SignalWire portal.
</ParamField>

## **Example**

In this example, we will create a simple call flow that connects to an AI agent after the call is answered.

<Frame caption="AI Agent node." ogImage>

![AI Agent node.](/assets/images/call-flow/nodes/ai_agent_node.webp)

</Frame>
