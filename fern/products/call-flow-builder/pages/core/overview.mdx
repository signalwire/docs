...
#### Add a node
When you first open a new Call Flow, you will see a canvas with a single [node](/call-flow-builder/reference):
the [Handle Call](/call-flow-builder/reference/handle-call) node.
...
Starting with the [Handle Call](/call-flow-builder/reference/handle-call) node, you can connect a node by
...
For example, the [Play Audio or TTS](/call-flow-builder/reference/play-audio-or-tts) node allows you to select an audio file or enter text to be played to the caller.
