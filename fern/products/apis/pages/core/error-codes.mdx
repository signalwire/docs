...
The AI chat endpoint is the exception: it speaks JSON-RPC, returns numeric codes, and reports failures under HTTP 200. See [AI chat errors](#ai-chat-errors).
...
The [AI chat endpoint](/docs/apis/rest/ai-chat/chat-methods) speaks JSON-RPC 2.0 rather than the REST
shapes above, so its failures look different in two ways that matter.

**They arrive under HTTP 200.** The service commits the status before a turn's outcome is known, so
the status code tells you nothing about whether the method succeeded. Check for an `error` key:

```json
{ "jsonrpc": "2.0", "error": { "code": -32001, "message": "Conversation not found" }, "id": "req-1" }
```

**The code is numeric.** `-32600` and below are the standard JSON-RPC codes; the rest are specific to
AI chat. Messages are deliberately coarse and don't describe internals — when diagnosing, the detail
you need is usually in your own agent's logs or in the SWML you served.

| Code | Meaning |
|---|---|
| `-32001` | No conversation with that id exists in your project |
| `-32002` | Your `config_url` could not be fetched. Check that it's reachable from the public internet |
...
| `-32007` | A turn is already running on this conversation. Wait for it rather than retrying, which fails the same way |
...
A `502` with `{"error": "AI chat service is unavailable"}` is the one failure that isn't JSON-RPC at
all: SignalWire could not reach the chat service, and the body carries no code.
