...
<ParamField path="already_a_member" type="Error" toc={true}>

- The invited email address already belongs to a member of the Space. Returned by `POST /api/space/members`.
</ParamField>
...
<ParamField path="amount_below_minimum" type="Error" toc={true}>

- `amount_in_microdollars` is below the minimum of 5,000,000 microdollars (5.00 USD). Returned by `POST /api/space/balance/top_ups`.
</ParamField>

<ParamField path="amount_not_a_whole_number" type="Error" toc={true}>

- `amount_in_microdollars` must be a whole number. Returned by `POST /api/space/balance/top_ups`.
</ParamField>

<ParamField path="amount_not_whole_cents" type="Error" toc={true}>

- `amount_in_microdollars` must be a whole number of cents, so a multiple of 10,000. Returned by `POST /api/space/balance/top_ups`.
</ParamField>
...
<ParamField path="cannot_change_the_owners_role" type="Error" toc={true}>

- The Space owner's role can't be changed. Returned by `PATCH /api/space/members/{id}`.
</ParamField>
...
<ParamField path="cannot_remove_the_owner" type="Error" toc={true}>

- The Space owner can't be removed from the Space. Returned by `DELETE /api/space/members/{id}`.
</ParamField>

<ParamField path="cannot_remove_yourself" type="Error" toc={true}>

- You can't remove your own membership. Returned by `DELETE /api/space/members/{id}` when the target is the calling token holder.
</ParamField>
...
<ParamField path="contact_email_address_invalid" type="Error" toc={true}>

- An address in `contact_email` is not a valid email address. The message names the address. Returned by `PUT /api/space/billing_profile`.
</ParamField>

<ParamField path="contact_email_missing" type="Error" toc={true}>

- `contact_email` must contain at least one address. Returned by `PUT /api/space/billing_profile`.
</ParamField>

<ParamField path="contact_email_must_be_array" type="Error" toc={true}>

- `contact_email` must be an array of email addresses. Returned by `PUT /api/space/billing_profile`.
</ParamField>
...
<ParamField path="countries_must_be_array" type="Error" toc={true}>

- `countries` must be an array of ISO 3166-1 alpha-2 country codes. Returned by `PUT /api/space/geographic_permissions`.
</ParamField>
...
<ParamField path="header_value_too_long" type="Error" toc={true}>

- The header value exceeds the maximum allowed byte length.
</ParamField>
...
<ParamField path="invalid_channels" type="Error" toc={true}>

- `channels` contains a value other than `audio`, `messaging`, or `video`.
</ParamField>

<ParamField path="invalid_codecs" type="Error" toc={true}>

- `codecs` contains a value outside the supported codec set for the address. Alias addresses accept `OPUS`, `G722`, `PCMU`, `PCMA`, `G729`, `VP8`, and `H264`.
</ParamField>
...
<ParamField path="invalid_header_name" type="Error" toc={true}>

- Header name must be a non-empty RFC 7230 token (no CR/LF or whitespace).
</ParamField>

<ParamField path="invalid_header_value" type="Error" toc={true}>

- Header value must be a non-empty string with no CR/LF characters.
</ParamField>
...
<ParamField path="invalid_job_title" type="Error" toc={true}>

- `job_title` must be `entrepreneur`, `product_manager`, `developer`, or `other`. Returned by `PATCH /api/space/members/{id}`.
</ParamField>
...
<ParamField path="invalid_month" type="Error" toc={true}>

- `month` must be formatted `YYYY-MM`. Returned by `GET /api/space/billing_statement` and `GET /api/space/usage`.
</ParamField>
...
<ParamField path="invalid_parent_project_id" type="Error" toc={true}>

- `parent_project_id` does not name a root project the caller can reach. Returned by `POST /api/projects` when authenticating with a Personal access token.
</ParamField>

<ParamField path="invalid_project_id" type="Error" toc={true}>

- `project_id` does not name a project the caller can reach. Returned by `POST /api/project/tokens` when authenticating with a Personal access token.
</ParamField>
...
<ParamField path="invalid_role" type="Error" toc={true}>

- `role` must be `admin` or `employee`; `owner` can't be assigned. Returned by `POST /api/space/members` and `PATCH /api/space/members/{id}`.
</ParamField>
...
<ParamField path="last_project_cannot_be_deleted" type="Error" toc={true}>

- The last remaining project in a Space can't be deleted. Returned by `DELETE /api/projects/{id}`.
</ParamField>
...
<ParamField path="month_is_in_the_future" type="Error" toc={true}>

- `month` is after the current month. Returned by `GET /api/space/billing_statement` and `GET /api/space/usage`.
</ParamField>

<ParamField path="month_precedes_space_creation" type="Error" toc={true}>

- `month` is before the month the Space was created. Returned by `GET /api/space/billing_statement` and `GET /api/space/usage`.
</ParamField>

<ParamField path="monthly_invoices_locked" type="Error" toc={true}>

- Monthly invoices can't be turned off on a postpay Space. Returned by `PUT /api/space/billing_profile` when `monthly_invoices` is `false`.
</ParamField>
...
<ParamField path="no_supported_countries" type="Error" toc={true}>

- None of the submitted countries are supported for this Space, so nothing would remain selected. Returned by `PUT /api/space/geographic_permissions`.
</ParamField>
...
- Only subprojects can be deleted when authenticating with a project API token. Returned by `DELETE /api/projects/{id}` when the target is a root project; a Personal access token can delete a root project.
</ParamField>
...
<ParamField path="payment_method_invalid" type="Error" toc={true}>

- `payment_method_id` does not name a payment method on this Space. Returned by `POST /api/space/balance/top_ups`.
</ParamField>

<ParamField path="payment_method_is_active_topup_source" type="Error" toc={true}>

- The card is the active auto top-up source and can't be deleted. Change or disable auto top-up first. Returned by `DELETE /api/space/payment_methods/{id}`.
</ParamField>

<ParamField path="pdf_requires_a_closed_month" type="Error" toc={true}>

- A PDF statement is only available for a closed month; the current month is available as JSON and CSV. Returned by `GET /api/space/billing_statement.pdf`.
</ParamField>
...
<ParamField path="send_topup_cannot_be_disabled" type="Error" toc={true}>

- This Space is required to keep auto top-up enabled. Returned by `PUT /api/space/low_balance_setting` when `send_topup` is `false`.
</ParamField>
...
<ParamField path="subproject_access_is_inherited_from_root" type="Error" toc={true}>

- Project access is granted on root projects only, and a subproject inherits it from its root. The message names the root project to enable instead. Returned by `PUT` and `DELETE /api/space/members/{member_id}/projects/{project_id}`.
</ParamField>

<ParamField path="subprojects_must_be_removed" type="Error" toc={true}>

- All subprojects must be removed from a project before it can be deleted. Returned by `DELETE /api/projects/{id}`.
</ParamField>

<ParamField path="taken" type="Error" toc={true}>

- The value is already in use by another record in the same scope. For an alias address, `name` matches another address in the same context.
</ParamField>
...
<ParamField path="threshold_above_maximum" type="Error" toc={true}>

- `threshold_in_microdollars` is above the maximum of 1,000,000,000,000 microdollars. Returned by `PUT /api/space/low_balance_setting`.
</ParamField>

<ParamField path="threshold_below_minimum" type="Error" toc={true}>

- `threshold_in_microdollars` is below the minimum of 5,000,000 microdollars (5.00 USD). Returned by `PUT /api/space/low_balance_setting`.
</ParamField>

<ParamField path="threshold_not_whole_cents" type="Error" toc={true}>

- `threshold_in_microdollars` must be a whole number of cents, so a multiple of 10,000. Returned by `PUT /api/space/low_balance_setting`.
</ParamField>
...
<ParamField path="topup_amount_below_minimum" type="Error" toc={true}>

- `topup_amount_in_microdollars` is below the minimum of 10,000,000 microdollars (10.00 USD). Returned by `PUT /api/space/low_balance_setting`.
</ParamField>

<ParamField path="topup_amount_missing" type="Error" toc={true}>

- `topup_amount_in_microdollars` is required when `send_topup` is `true`. Returned by `PUT /api/space/low_balance_setting`.
</ParamField>

<ParamField path="topup_amount_not_whole_cents" type="Error" toc={true}>

- `topup_amount_in_microdollars` must be a whole number of cents, so a multiple of 10,000. Returned by `PUT /api/space/low_balance_setting`.
</ParamField>

<ParamField path="topup_payment_method_invalid" type="Error" toc={true}>

- `topup_payment_method_id` does not name a payment method on this Space. Returned by `PUT /api/space/low_balance_setting`.
</ParamField>

<ParamField path="topup_payment_method_missing" type="Error" toc={true}>

- `topup_payment_method_id` is required when `send_topup` is `true`. Returned by `PUT /api/space/low_balance_setting`.
</ParamField>
...
<ParamField path="unknown_role" type="Error" toc={true}>

- The `role` filter is not a known role. Returned by `GET /api/space/members`.
</ParamField>
...
<ParamField path="webhook_method_invalid" type="Error" toc={true}>

- `webhook_method` must be `GET` or `POST`. Returned by `PUT /api/space/low_balance_setting`.
</ParamField>

<ParamField path="webhook_url_missing" type="Error" toc={true}>

- `webhook_url` is required when `send_webhook` is `true`. Returned by `PUT /api/space/low_balance_setting`.
</ParamField>

<ParamField path="-32001" type="Error" toc={true}>

- No conversation with that id exists in your project.
</ParamField>

<ParamField path="-32002" type="Error" toc={true}>

- Your `config_url` could not be fetched. Check that it's reachable from the public internet — not `localhost`, not a private address.
</ParamField>

<ParamField path="-32003" type="Error" toc={true}>

- The model provider failed.
</ParamField>

<ParamField path="-32004" type="Error" toc={true}>

- Storage failure.
</ParamField>

<ParamField path="-32005" type="Error" toc={true}>

- Too many `summarize` calls on this conversation, which allows one per minute.
</ParamField>

<ParamField path="-32006" type="Error" toc={true}>

- The chat rate limit was exceeded.
</ParamField>

<ParamField path="-32007" type="Error" toc={true}>

- Another chat operation is already in progress for this conversation.
</ParamField>

<ParamField path="-32600" type="Error" toc={true}>

- The JSON-RPC envelope was malformed.
</ParamField>

<ParamField path="-32601" type="Error" toc={true}>

- No method by that name.
</ParamField>

<ParamField path="-32602" type="Error" toc={true}>

- A required param is missing, or one carries an invalid value.
</ParamField>

<ParamField path="-32603" type="Error" toc={true}>
