...
The AI chat endpoint is the exception: it speaks JSON-RPC, returns numeric codes, and reports failures under HTTP 200. Its codes are listed under [AI chat](#ai-chat) in the reference below.
...
The [AI chat endpoint](/docs/apis/rest/ai-chat/chat-methods) speaks JSON-RPC 2.0, so its errors are
numeric rather than the string codes above, and they arrive in an `error` object under **HTTP 200** —
check for that key rather than reading the status code. `-32600` and below are the standard JSON-RPC
codes; the rest are specific to AI chat.

```json
{ "jsonrpc": "2.0", "error": { "code": -32001, "message": "Conversation not found" }, "id": "req-1" }
```

Messages are deliberately coarse and don't describe internals. When diagnosing, the detail you need
is usually in your own agent's logs or in the SWML you served.
...
- Too many messages on this conversation.
</ParamField>

<ParamField path="-32007" type="Error" toc={true}>

- A turn is already running on this conversation. Wait for it to return rather than retrying, which fails the same way.
</ParamField>

<ParamField path="-32008" type="Error" toc={true}>

- The SWML your `config_url` served failed validation.
</ParamField>

<ParamField path="-32009" type="Error" toc={true}>

- Credentials are missing, rejected, or lack the `chat` scope.
</ParamField>
...
<ParamField path="-32700" type="Error" toc={true}>

- The request body was not valid JSON.
</ParamField>

A `502` with `{"error": "AI chat service is unavailable"}` is the one failure that carries no code at
all: SignalWire could not reach the chat service.
