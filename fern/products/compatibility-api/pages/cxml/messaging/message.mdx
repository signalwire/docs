...
The `<Message>` verb sends an SMS or MMS message to a phone number. To send a message in combination with [Voice XML](/docs/compatibility-api/voice/index) verbs, use the [`<Sms>` Voice verb](/docs/compatibility-api/voice/sms).
