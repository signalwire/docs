...
use [Compatibility APIs](compatibility-api/rest): if you are
migrating from a different provider, these APIs will likely be very similar. In
case you need more advanced features, check out our [Realtime APIs](realtime-sdk/).
...
Numbers section, you can [buy a new phone number](platform/phone-numbers) if
...
You can also
[port an existing phone number](platform/porting-into-signalwire)
...
your preferred programming language), or you can use [one of our SDKs](compatibility-api/sdks). We'll show examples for both methods.
...
To send a message, you can use the [Create a Message](compatibility-api/rest/messages/create-message) endpoint from our
...
deliver your messages. Please see [**Campaign Registry - Everything You Need To Know**](platform/messaging/campaign-registry) for more information.
...
particular the [Compatibility REST API](compatibility-api/rest), to
send a message. For more advanced, real-time applications, you'll want to check
out our [Realtime SDK](server-sdk/node).
