...
<CardGroup cols={3}>
  <Card title="Voice" icon="phone" href="/compatibility-api/cxml/reference/voice">
    Control calls with `<Say>`, `<Play>`, `<Dial>`, `<Gather>`, `<Record>`, and more.
  </Card>
  <Card title="Messaging" icon="message" href="/compatibility-api/cxml/reference/messaging">
    Handle SMS/MMS with `<Message>` and `<Redirect>`.
  </Card>
  <Card title="Fax" icon="fax" href="/compatibility-api/cxml/reference/fax">
...
Each channel type has its own set of available verbs. See the [Voice](/compatibility-api/cxml/reference/voice#verbs), [Messaging](/compatibility-api/cxml/reference/messaging#verbs), and [Fax](/compatibility-api/cxml/reference/fax#verbs) overview pages for the complete list of verbs for each channel.
