...
[`<Dial>`](compatibility-api/dial) verb's `<Conference>` noun allows the connection to a named conference room.
For example:
...
<ParamField path="coach" type="string">
  Coach accepts a call [SID](platform/what-is-a-sid) of a call that is currently connected to an in-progress conference. Specifying a call SID that does not exist or is no longer connected to the conference will result in the call failing to the action URL and throwing a 13240 error.
</ParamField>
...
You can expect several parameters to be present in the request associated to the `statusCallback` URL. First, you have the [Standard Request Parameters](compatibility-api/cxml/reference/voice#request-parameters). Then, you also have the following specific parameters:
