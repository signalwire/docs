...
After a Dialogflow conversation is completed, SignalWire can make a request to the `<Connect>` verb's `action` attribute. In addition to the [Standard Request Parameters](/compatibility-api/cxml/reference/voice#request-parameters), the following are parameters passed back to your application when SignalWire makes the request.
...
<ParamField path="VirtualAgentErrorCode" type="string">
  An [error code](/compatibility-api/error-codes), in case `VirtualAgentStatus` is `"failed"`.
</ParamField>
