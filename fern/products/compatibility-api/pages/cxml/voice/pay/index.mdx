...
- [`<Parameter>`](/compatibility-api/cxml/reference/voice/pay/parameter): Pass custom parameters to your payment processor.
- [`<Prompt>`](/compatibility-api/cxml/reference/voice/pay/prompt): Customize the prompts used during payment collection.
...
When a transaction is completed, SignalWire will `POST` to your `action` URL. The body of the request will contain the [standard request parameters](/compatibility-api/cxml/reference/voice#request-parameters)
...
<Note title="You can customize prompt messages in any language using the [`<Prompt>`](/compatibility-api/cxml/reference/voice/pay/prompt) noun, regardless of the selected `language` attribute."/>
