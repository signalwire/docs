...
A RoomSessionDevice represents a device (such as a microphone or a camera) that is at some point in its lifetime part of a [RoomSession](/browser-sdk/reference/video/room-session). You can obtain a RoomSessionDevice from the [RoomSession](/browser-sdk/reference/video/room-session) methods [RoomSession.addCamera](/browser-sdk/reference/video/room-session/methods/add-camera), [RoomSession.addMicrophone](/browser-sdk/reference/video/room-session/methods/add-microphone), and [RoomSession.addDevice](/browser-sdk/reference/video/room-session/methods/add-device).
...
<ParamField path="cameraId" type="string | null" required={true}>
  The id of the video device, or null if not available.
</ParamField>

<ParamField path="cameraLabel" type="string | null" required={true}>
  The label of the video device, or null if not available.
</ParamField>

<ParamField path="localAudioTrack" type="MediaStreamTrack | null" required={true}>
  Provides access to the local audio [MediaStreamTrack](https://developer.mozilla.org/en-US/docs/Web/API/MediaStreamTrack).
</ParamField>

<ParamField path="localStream" type="MediaStream | undefined" required={true}>
  Provides access to the local [MediaStream](https://developer.mozilla.org/en-US/docs/Web/API/MediaStream).
</ParamField>

<ParamField path="localVideoTrack" type="MediaStreamTrack | null" required={true}>
...
<ParamField path="microphoneId" type="string | null" required={true}>
  The id of the audio input device, or null if not available.
</ParamField>

<ParamField path="microphoneLabel" type="string | null" required={true}>
  The label of the audio input device, or null if not available.
</ParamField>

<ParamField path="remoteStream" type="MediaStream | undefined" required={true}>
