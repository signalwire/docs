...
<Indent>
  <ParamField path="params.channels" type="string | string[]" required={true} toc={true}>
    Channels for which to set the state.
  </ParamField>
  <ParamField path="params.memberId" type="string" required={true} toc={true}>
    Id of the member to affect. If not provided, defaults to the current member.
  </ParamField>
  <ParamField path="params.state" type="Record<any, any toc={true}>" required={true}>
