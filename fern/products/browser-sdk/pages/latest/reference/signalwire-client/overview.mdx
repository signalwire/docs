...
```bash npm2yarn
npm install @signalwire/js@3.30.0
```
...
The access token is received after the [subscriber](/docs/platform/subscribers) completes the OAuth2 flow and successfully logs in. Learn more at the [Authentication][Authentication] section further below.
...
<AccordionGroup>
  <Accordion title="Create a Guest Token" defaultOpen>
    ...
    <EndpointRequestSnippet endpoint="POST /api/fabric/guests/tokens" />
  </Accordion>
  <Accordion title="Create a Subscriber Invite Token">
    ...
    <EndpointRequestSnippet endpoint="POST /api/fabric/subscriber/invites" />
  </Accordion>
