...
Then you can interact with the SDK using the global variable `SignalWire`. We'll mainly be interested in the `SignalWire.Chat.Client` class for this guide, but if you'd like to explore this API, feel free to browse the [SDK documentation](browser-sdk/js/guides).
...
The backend technology and stack is not relevant. The important part is that the SignalWire Chat REST API is called to [Generate a new Chat Token](apis/chat/tokens/create-chat-token) with the appropriate parameters.
...
- [Technical Reference](guides/chat)
