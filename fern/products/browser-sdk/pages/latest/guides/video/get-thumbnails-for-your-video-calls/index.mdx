---
id: 74096c3d-77d5-44fa-b724-66f024b73796
...
![A preview of a Video Room. Text at the top reads 'Join a room'. Four previews are shown, each labeled with the room name.](/assets/images/project/video-thumbnail/preview_video_thumbnail.webp)
...
If you’re using the API to programmatically [create rooms](/apis/video/rooms/create-room), you need to set the `enable_room_previews` attribute to `true` when creating the new room.
...
![This screenshot shows the configuration options for Video Conferences.](/assets/images/video/conference-settings.webp)
</Frame>
...
If you have a proxy backend (as described in the [Simple Video Demo](/browser-sdk/guides/build-a-video-calling-application-with-the-browser-sdk)), you can query the Rest API for the room sessions. You can either list all room sessions with the [`GET /api/video/room_sessions`](/apis/video/room-sessions/list-room-sessions) endpoint. Or if you have the id of your current room session, you can [`GET /api/video/room_sessions/{id}`](/apis/video/room-sessions/get-room-session).
...
[`room.started`](/server-sdk/node/reference/video/client#onroomstarted)
event to get new room sessions as they are created.

#### RELAY v4

For Realtime API (RELAY v4), you can add an event listener for
[`onRoomStarted`](/server-sdk/node/reference/video/client#onroomstarted)
event to get new room sessions as they are created.

For the Video Client SDK running in the browser, the `previewUrl` is available in the same [RoomSession](/browser-sdk/video/room-session) object you create to start the video call.

You will find the preview image in the `previewUrl` attribute of the RoomSession object.

### Programmable Video Conferences

If you're using SignalWire's Programmable Video Conferences to create and administrate rooms through the Dashboard, you can access the Video Preview by tapping into the [`setupRoomSession`](/platform/video) parameter when setting up the video room in your web page.
