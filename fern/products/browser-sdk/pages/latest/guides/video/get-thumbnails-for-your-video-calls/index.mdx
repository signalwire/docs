...
![A preview of a Video Room. Text at the top reads 'Join a room'. Four previews are shown, each labeled with the room name.](/docs/assets/images/project/video-thumbnail/preview_video_thumbnail.webp)
...
If you’re using the API to programmatically [create rooms](/docs/apis/video/rooms/create-room), you need to set the `enable_room_previews` attribute to `true` when creating the new room.
...
![This screenshot shows the configuration options for Video Conferences.](/docs/assets/images/video/conference-settings.webp)
</Frame>
...
[`room.started`](/docs/server-sdk/node/reference/video/client#onroomstarted)
event to get new room sessions as they are created.

#### RELAY v4

For Realtime API (RELAY v4), you can add an event listener for
[`onRoomStarted`](/docs/server-sdk/node/reference/video/client#onroomstarted)
event to get new room sessions as they are created.

For the Video Client SDK running in the browser, the `previewUrl` is available in the same [RoomSession](/docs/browser-sdk/video/room-session) object you create to start the video call.
