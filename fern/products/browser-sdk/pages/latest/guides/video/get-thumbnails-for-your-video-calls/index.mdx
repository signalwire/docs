...
If you’re using the API to programmatically [create rooms](/docs/apis/rest/video/rooms/create-room), you need to set the `enable_room_previews` attribute to `true` when creating the new room.
...
For the Video Client SDK running in the browser, the `previewUrl` is available in the same [RoomSession](/docs/browser-sdk/v3/js/reference/video/room-session) object you create to start the video call.
