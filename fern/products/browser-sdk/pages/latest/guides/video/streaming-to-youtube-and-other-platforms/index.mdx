---
id: aa113205-7951-4729-9d85-6727c9cd1dad
...
![The YouTube stream parameters menu. The Stream Settings tab is selected, with fields for Stream Key type, Stream Key, Stream URL, and Backup server URL. Appropriate SignalWire parameters have been entered in each field.](/assets/images/external/youtube-rtmp-setting.webp)
...
Later, we will put click handlers on the start and stop buttons to call [`startStream`](/browser-sdk/reference/video/room-session/methods/start-stream) and [`stopStream`](/browser-sdk/reference/video/room-session-stream/methods/stop) respectively. The [`startStream`](/browser-sdk/reference/video/room-session/methods/start-stream) function is available on the Room Session object, so first we need to use the [`setupRoomSession`](/platform/video) callback function on the PVC to get that object. So, the `VideoConference` constructor at the end of the embed script should look like this:
...
While this demo used a PVC, you can use the same methods on a video room without the prebuilt UI. For a complete guide on building video rooms without a prebuilt UI, see [Simple Video Demo](/browser-sdk/guides/build-a-video-calling-application-with-the-browser-sdk). From there, you can add start and stop stream buttons and hook them up in the same way as above.
...
- [API Streams Reference](/apis/video/streams/list-room-streams)
- [SDK Streaming Demo](https://github.com/signalwire/guides/tree/main/Video/RTMP-Streaming)
- [SDK Streaming Reference](/browser-sdk/reference/video/room-session-stream)
- [PVC Reference](/platform/video)
