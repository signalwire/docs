...
1. [The SignalWire Video SDK](docs/browser-sdk/reference/video) will run in the client's browser.
...
2. We will use the [SignalWire REST APIs for Video](docs/apis/video/rooms/create-room)
...
3. We perform an HTTP POST request using Axios to the [room_tokens](docs/apis/video/room-tokens/create-room-token) endpoint.
...
[RoomSession](docs/apis/video/rooms/create-room) object
...
The `useWebRTC()` hook is the React wrapper for the [`WebRTC`](docs/browser-sdk/reference/webrtc) namespace in the SDK.
...
We can query for the list of active room sessions via the REST API at the [room_sessions](docs/apis/video/room-sessions/list-room-sessions) endpoint. Sending a GET request to this endpoint with a query parameter `status` set to `in-progress` will get us the list of active room sessions.
...
with a [`<RoomPreview/>`](https://signalwire-community.github.io/docs/react/components/roompreview) component ([described in detail here](docs/browser-sdk/guides/displaying-call-thumbnails)).
