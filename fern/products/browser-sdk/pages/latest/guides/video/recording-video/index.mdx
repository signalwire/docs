...
![Embedded video conference widget with Start Recording' selected](/assets/images/video/pvc-start-recording.webp)

</Frame>

If you are using [AppKit](https://www.npmjs.com/package/@signalwire/app-kit) to create or [extend embeddable rooms](platform/video),
use the `setupRoomSession` callback to get a reference to the [`RoomSession`](browser-sdk/video/room-session) object.
...
To start recording in an ongoing room session from the browser SDK, use the [`RoomSession.startRecording()`](browser-sdk/reference/video/room-session/methods/start-recording) method. You must have the `room.recording` permission to be able to start and stop recording.

This method returns a Promise which resolves to a [RoomSessionRecording](browser-sdk/reference/video/room-session-recording) object. You can use this returned object to control the recording, including pausing and stopping it.
...
[`record_on_start`](apis/video/rooms/create-room) option while creating the room. Further, you have to make
sure that the `room.recording` [permission](apis/permissions) is set in the room token.
...
precisely from your server, use the [Realtime SDK](server-sdk/node/reference/video/room-session). The Realtime SDK
exposes a RoomSession object similar to the one in the [Browser SDK](browser-sdk/video/room-session), so you
...
Use the [`RoomSessionRecording.stop()`](browser-sdk/video/room-session-recording) method to stop
the ongoing recording. This method is included on the object returned when you called the
[`RoomSession.startRecording()`](browser-sdk/reference/video/room-session/methods/start-recording) method.
...
[`https://<space_name>.signalwire.com/api/video/room_recordings`](apis/video/room-sessions/list-room-session-recordings).
