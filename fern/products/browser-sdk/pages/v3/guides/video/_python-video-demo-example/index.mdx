...
<figure>
  <img class="img-800" src={require("./bf8d189-moderator_view_video_demo.webP").default} />
</figure>

Below is a screenshot of the **guest view**:

<figure>
  <img class="img-800" src={require("./a25912d-guest_view_video_demo.webP").default} />
</figure>
...
<figure>
  <img class="img-800" src={require("./623ac8c-listed_participants.webP").default} />
</figure>
...
In the moderator view, we have several functions for controlling other users shown below. These will take a member ID (gathered from `room.params` on room state changes) as the argument and contain the methods shown in the [documentation](/browser-sdk/video/room-session#methods) for muting/unmuting/kicking out a user. These will not work if the token does not have the correct permission defined. These will be attached as on click events to buttons that are next to each user on the nav bar **only if** you are in the moderator view.
