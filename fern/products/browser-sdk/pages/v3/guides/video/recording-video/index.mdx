...
use the `setupRoomSession` callback to get a reference to the [`RoomSession`](/browser-sdk/reference/video/roomsession) object.
...
To start recording in an ongoing room session from the browser SDK, use the [`RoomSession.startRecording()`](/browser-sdk/reference/video/roomsession#startrecording) method. You must have the `room.recording` permission to be able to start and stop recording.

This method returns a Promise which resolves to a [RoomSessionRecording](/browser-sdk/reference/video/roomsession-recording) object. You can use this returned object to control the recording, including pausing and stopping it.
...
exposes a RoomSession object similar to the one in the [Browser SDK](/browser-sdk/reference/video/roomsession), so you
...
Use the [`RoomSessionRecording.stop()`](/browser-sdk/reference/video/roomsession-recording) method to stop
the ongoing recording. This method is included on the object returned when you called the
[`RoomSession.startRecording()`](/browser-sdk/reference/video/roomsession#startrecording) method.
