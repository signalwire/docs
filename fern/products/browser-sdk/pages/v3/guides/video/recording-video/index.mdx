...
To start recording in an ongoing room session from the browser SDK, use the [`RoomSession.startRecording()`](/browser-sdk/video/room-session#startrecording) method. You must have the `room.recording` permission to be able to start and stop recording.

This method returns a Promise which resolves to a [RoomSessionRecording](/browser-sdk/video/room-session-recording) object. You can use this returned object to control the recording, including pausing and stopping it.
...
[`RoomSession.startRecording()`](/browser-sdk/video/room-session#startrecording) method.
