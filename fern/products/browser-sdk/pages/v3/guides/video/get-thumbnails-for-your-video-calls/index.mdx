...
![A preview of a Video Room. Text at the top reads 'Join a room'. Four previews are shown, each labeled with the room name.](/images/project/video-thumbnail/preview_video_thumbnail.webp)
...
![This screenshot shows the configuration options for Video Conferences.](/images/video/conference-settings.webp)
</Frame>
...
For the Video Client SDK running in the browser, the `previewUrl` is available in the same [RoomSession](/browser-sdk/v-3/reference/video/room-session) object you create to start the video call.
