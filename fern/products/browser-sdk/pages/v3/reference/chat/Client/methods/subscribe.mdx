...
Note that the `subscribe` function is idempotent, and calling it again with a different set of channels _will not_ unsubscribe you from the old ones. To unsubscribe, use [unsubscribe](/docs/browser-sdk/v3/js/reference/chat/client/unsubscribe).

## Parameters

<ParamField path="channels" type="string | string[]" required={true} toc={true}>
