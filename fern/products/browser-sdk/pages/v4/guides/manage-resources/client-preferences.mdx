...
The full surface is documented in [`ClientPreferences`] reference. These are the ones most
apps touch, with their code defaults:

| Preference | Default | Controls |
|---|---|---|
| `receiveVideo` | `false` | accept inbound video on a call |
...
Persisted: timeouts, ICE / recovery tuning, codec preferences, stats and
device-management flags, and `userVariables`. Persistence is an explicit
allowlist — not everything that happens to be serializable.

Not in this blob: the `MediaDeviceInfo` objects behind `preferredAudioInput`
/ etc., and media constraints (`defaultVideoConstraints`,
`inputAudioConstraints`, and the other `*Constraints` fields) — even though
constraints are JSON-serializable.

**Device selections persist separately, and on by default.** Independent of
`savePreferences`, the device controller writes your mic / camera / speaker
pick to `localStorage` (keyed by `deviceId`, keeping `label` / `groupId` to
re-match when IDs rotate across sessions) and restores it next time. This is
governed by the `persistDeviceSelection` preference (default `true`); set it
to `false` to opt out.
...
A video stream is an occasional **keyframe** — a complete, self-contained
frame — followed by **delta frames** that encode only the change from the
