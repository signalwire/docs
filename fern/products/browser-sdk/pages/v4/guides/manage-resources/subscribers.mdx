...
A [**Subscriber**](/docs/platform/subscribers) is a kind
of user: the SignalWire Resource type that represents a permanent
account in your application. Each Subscriber has credentials, a
profile, a private [Address](/docs/platform/addresses) for direct
dialing, and may own public phone numbers. When the Browser SDK
...
Not every user is a stored Subscriber.
Guest and invite tokens produce sessions that don't map to a
Subscriber Resource, yet the SDK exposes all of them through the same
[`User`] object on `client.user`, which carries whatever identity and
scopes the token was minted with.
