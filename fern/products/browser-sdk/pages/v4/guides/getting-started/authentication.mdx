...
The SDK acts on behalf of a user, whether that's a full-access [**Subscriber**](/docs/platform/subscribers) or a guest with limited access. It authenticates with a **Subscriber Access Token (SAT)**, a short-lived credential that identifies that user and carries the capabilities granted to them. Your backend creates the SAT using your Project API Token, then hands it to the browser, where the SDK uses it to open a WebSocket session with SignalWire.
...
| Public usage | **[Embed token](/docs/apis/rest/subscribers/tokens/create-guest-embed-token)** | ✗ | ✓ | Tied to a single resource | Anyone visiting a public page |
...
2. Exchange the C2C token for an **embed token** by calling [`POST /api/embeds/tokens`](/docs/apis/rest/subscribers/tokens/create-guest-embed-token). The embed token is what the SDK is built to consume for public widgets.
...
refresh — subscribe to [`client.warnings$`](/docs/browser-sdk/v4/reference/signalwire/warnings$):
...
| Persist across reloads | off | `persistSession: true` | (constructor only) |
...
With `persistSession: true` a Client Bound SAT survives a page reload: the SDK persists the token alongside its DPoP key pair in IndexedDB and re-binds on the new page rather than degrading the session to an unbound refresh token. The stored token is useless without the key, so the binding itself is the theft protection. If the persisted credential has expired by the time the page reloads, the SDK re-mints it through your credential provider before the session and its attach records are restored.
...
Closing the session entirely is a separate step. Both [`disconnect()`](/docs/browser-sdk/v4/reference/signalwire/disconnect) and [`destroy()`](/docs/browser-sdk/v4/reference/signalwire/destroy) end the session and clear its persisted resume state and attach records — `disconnect()` tears down the WebSocket (the client can reconnect), while `destroy()` also releases all timers, subscriptions, and resources. Credentials and device preferences survive both; call [`resetToDefaults()`](/docs/browser-sdk/v4/reference/signalwire/reset-to-defaults) for a full wipe on explicit logout.
...
<CardGroup cols={2}>
  ...
  <Card title="Outbound Calls" href="/docs/platform/voice/make-and-receive-calls#make-an-outbound-call" icon="fa-regular fa-phone-arrow-up-right">
