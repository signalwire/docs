...
For production, swap the pasted SAT for a `fetch("/api/signalwire-token")` call against your own backend and ship a [custom provider](#server-fetched-sats) with `refresh()` for sessions that outlive a single SAT.
