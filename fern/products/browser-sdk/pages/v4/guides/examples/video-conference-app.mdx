...
A Zoom-style multi-party video conference built on the Browser SDK: a backend that registers users and mints tokens, a dashboard with device selection, multi-party video, in-call controls, screen share, server-side layouts, a participant roster, and chat.

The app has two parts — a small **backend** that creates accounts, checks passwords, and exchanges your API token for short-lived user tokens, and a **browser client** that does everything else. Every piece of the client is driven by [RxJS observables][rxjs] on the SDK: nothing polls, everything subscribes and reacts to state the SDK pushes.

<Info>
**Before you start.** You'll need a SignalWire Space and a project (Project ID + API token) with at least one of the **Voice / Messaging / Fax / Video** scopes — the backend uses it to create [Subscribers][users] and mint their tokens. Mic and camera access requires an HTTPS origin (`localhost` is the development exception).
</Info>
...
flowchart LR
    ...
    subgraph Backend["Backend (Node / Express)"]
        AUTH["/api/register<br/>/api/login<br/>(local users, password check)"]
        SAT["/api/sat<br/>/api/sat/refresh<br/>(stores refresh_token)"]
    end
    Fabric["SignalWire Fabric"]

    CP -->|"POST + session cookie"| AUTH
    AUTH -->|"Basic auth — create Subscriber"| Fabric
...
The browser **never sees your API token**, and refresh tokens **never reach the browser** — they live server-side, keyed by a session cookie, and rotate on every refresh. Registering creates a local user record and a matching SignalWire [Subscriber][users]; signing in checks the password against that local record. Either way, the client ends up with a short-lived [Subscriber Access Token][auth] (SAT), and the SDK schedules its own refresh through the same backend.
...
Copy `.env.example` to `.env` and fill in your Space and project credentials:
...
`npm run dev` starts the backend on port `3001` and the Vite dev server on `5173`. Vite proxies `/api/*` to the backend, so the browser only ever talks to one origin. Open `http://localhost:5173`, click **Need an account? Create one**, fill in a name/email/password to register, pick your devices, and **Join** the default room (`/public/test-room`). Open a second browser (or an incognito window) and register a second account to call between them.

</Steps>

## How it works

### The backend registers users and mints tokens

`server/index.ts` exposes endpoints for account creation and token minting, all backed by in-memory `Map`s keyed by a session cookie.

`POST /api/register` creates a local user record — the password is hashed with `scrypt`, never stored in plain text — and a matching SignalWire [Subscriber][users], using your Project API token as HTTP Basic auth:
...
app.post('/api/register', async (req, res) => {
  ...
  sessionUsers.set(req.sid, email);
  res.json({ name, email });
});
```

`POST /api/login` checks the submitted password against the stored hash and, on success, associates the session cookie with that email. Neither endpoint hands the browser anything but the user's own name and email — the Project API token never leaves the server.

`POST /api/sat` looks up the email tied to the current session, then exchanges your API token for a fresh SAT scoped to that Subscriber's `reference`, stashes the returned `refresh_token` in memory, and returns **only** the `token` and `expires_at` to the browser:

```ts title="server/index.ts"
app.post('/api/sat', async (req, res) => {
  const email = sessionUsers.get(req.sid);
...
<Note>
The demo keeps users and refresh tokens in in-memory `Map`s, so accounts don't survive a server restart. A real app stores both in a database, with refresh tokens in an encrypted, per-user column. See [Authentication][auth].
</Note>

Three REST API endpoints back those routes:

<CardGroup cols={3}>
  <Card title="Create Subscriber" icon="fa-regular fa-user-plus" href="/docs/apis/rest/subscribers/create-subscriber">
    Called from `/api/register` to create the Subscriber.
  </Card>
  <Card title="Create Subscriber Token" icon="fa-regular fa-key" href="/docs/apis/rest/subscribers/tokens/create-subscriber-token">
    Called from `/api/sat` to mint a SAT for the signed-in user.
  </Card>
  <Card title="Refresh Subscriber Token" icon="fa-regular fa-arrows-rotate" href="/docs/apis/rest/subscribers/tokens/refresh-subscriber-token">
    Called from `/api/sat/refresh` to rotate the SAT before it expires.
  </Card>
</CardGroup>
...
Submitting either form posts JSON to `/api/register` or `/api/login`; on success `src/main.ts` builds the client with that provider and calls [`register()`][register] — without it, inbound calls never arrive even when someone dials this user:
