...
### When a mid-call switch fails

A live swap acquires the new device *before* releasing the old one, so a
failure — permission revoked, the device held by another application, a stale
`deviceId` — leaves the call sending the device it was already using. The
failure is reported on
[`call.errors$`](/docs/browser-sdk/v4/reference/webrtc-call/errors$) rather
than thrown, so subscribe there to reset the picker to the device still in use:

```ts Browser
import { MediaAccessError } from "@signalwire/js";

activeCall.errors$.subscribe(({ error }) => {
  if (error instanceof MediaAccessError) {
    micSelect.value = client.selectedAudioInputDevice?.deviceId ?? "";
    showToast(`Couldn't switch ${error.media}. Still using the previous device.`);
  }
});
```

Hardware that admits only one opener at a time — a phone's front and back
cameras, typically — is the exception: the SDK releases the current capture,
retries, and re-acquires the original device if the retry also fails. A stale
`deviceId` falls back through progressively looser constraints rather than
failing outright, so the switch may land on a different device than the one
picked; read `selectedVideoInputDevice` after the switch if that matters to
your UI.
...
Enter a destination in the **Destination** field (a `/public/<resource>`, `/private/<user>`, PSTN number, or SIP URI the token can reach — see [Outbound calling](/docs/platform/voice/outbound-calling#choose-a-destination) for the destination shapes) and click **Dial**. The local and remote tiles populate once the call connects.
