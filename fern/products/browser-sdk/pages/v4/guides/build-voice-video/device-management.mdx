...
]).subscribe(([mics, selected]) => {
  micSelect.innerHTML = "";
  for (const mic of mics) {
    const option = new Option(mic.label || `Microphone ${mic.deviceId.slice(0, 6)}`, mic.deviceId);
    option.selected = mic.deviceId === selected?.deviceId;
    micSelect.append(option);
  }
});
```

The same pattern fits `videoInputDevices$` and `audioOutputDevices$` — only the property names change.
...
<html lang="en">
  ...
  <body>
    ...
    <script type="module">
      import { SignalWire, StaticCredentialProvider } from "https://esm.sh/@signalwire/js@latest";
      import { combineLatest } from "https://esm.sh/rxjs@7";
      ...
      function populate(selectEl, devices, selected) {
        selectEl.innerHTML = "";
        ...
        if (!devices.length) selectEl.innerHTML = "<option>(none)</option>";
      }
      ...
      $("connect").onclick = () => {
        ...
        combineLatest([client.audioInputDevices$, client.selectedAudioInputDevice$])
          .subscribe(([devices, selected]) => populate($("mic"), devices, selected));
        combineLatest([client.videoInputDevices$, client.selectedVideoInputDevice$])
          .subscribe(([devices, selected]) => populate($("cam"), devices, selected));
        combineLatest([client.audioOutputDevices$, client.selectedAudioOutputDevice$])
          .subscribe(([devices, selected]) => populate($("speaker"), devices, selected));
