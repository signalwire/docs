...
If the mic or camera can't be captured, the call is answered in
receive-only mode by default and the reason is reported on `errors$` — see
[when local media can't be captured](/docs/platform/voice/make-and-receive-calls#when-local-media-cant-be-captured).
