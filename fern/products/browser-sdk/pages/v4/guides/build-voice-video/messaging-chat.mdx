...
```ts
interface TextMessage<A = Address> {
  id: string;
  text: string;
  created: string;             // ISO timestamp
  fromAddress$: Observable<A>; // who sent it
}
```

`fromAddress$` is an observable because the sender is itself a
resolved `Address` instance — useful for rendering an avatar or
name without an extra fetch.
...
The same conversation log carries call history. Reach it via
`address.history$`:
...
```ts
interface AddressHistory<A = Address> {
  id: string;
  kind: string;          // 'inbound' | 'outbound' | 'missed' | ...
  status: string;        // server-defined status string
  started: string;       // ISO timestamp
  ended: string;         // ISO timestamp
  fromAddress$: Observable<A>;
}
```

Both `textMessages$` and `history$` paginate the same way — they're
just two views of the same underlying conversation, filtered to chat
vs. call entries.

## Per-conversation observables

The `Address` instance is the conversation handle. Two convenient
shortcuts:

| Member             | Purpose                                              |
| ------------------ | ---------------------------------------------------- |
| `address.textMessages$` | Observable<TextMessageCollection> — chat thread |
| `address.history$`      | Observable<AddressHistoryCollection> — call log |
| `address.sendText(text)`| Append a chat message to the thread           |
