...
<CardGroup cols={3}>
  ...
  <Card title="Place an outbound call" icon="fa-regular fa-phone-arrow-up-right" href="/docs/platform/voice/make-and-receive-calls#make-an-outbound-call">
...
A call carries at most one screen share. Starting a second one throws
[`ScreenShareAlreadyActiveError`](/docs/browser-sdk/v4/reference/errors/screen-share-already-active-error)
and leaves the live share untouched — stop the first one before replacing it.
Read `screenShareStatus` before calling and treat `'starting'` and `'stopping'`
as busy.

A screen-share failure is never fatal: whatever goes wrong with the share,
the call itself keeps running. The picker is human time and sits outside the
SDK's connect budget, so a user who takes a minute to choose a window is not
timed out; only the 15 seconds of negotiation that follow are bounded, with
[`AuxiliaryLegTimeoutError`](/docs/browser-sdk/v4/reference/errors/auxiliary-leg-timeout-error)
if they run out.
...
function handleShareError(err) {
  if (err instanceof AuxiliaryLegCancelledError) {
    // Your own stopScreenShare() beat the leg to the finish — nothing to report.
    return;
  }
  if (err instanceof ScreenShareAlreadyActiveError) {
    showToast("Already sharing. Stop the current share first.");
    return;
  }
...
Pass `{ audio: true }` to also request the shared surface's audio. The default
is video only — the share never carries the local microphone, which stays on
the main connection:
...
Whether audio is actually captured depends on the browser, the OS, and the
surface the user picks. A share the user grants without audio yields a
video-only stream, and the share still starts normally — so treat surface
audio as best-effort, not a guarantee.

In practice:
- Chrome / Edge: offered for tabs and windows; system audio depends on the OS
  and version.
- Firefox: video only.
- Safari: video only (and screen sharing requires explicit user-initiated
...
`screenShareStatus$` to `'none'` automatically — no action required
...
- [`SelfParticipant.screenShareStatus$`] / [`screenShareStatus`] — reactive status (`'none' | 'starting' | 'started' | 'stopping'`)
