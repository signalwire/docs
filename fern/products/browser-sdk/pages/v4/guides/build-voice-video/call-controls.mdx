...
| Member readiness  | Any control issued before the member's call context has arrived — [`CallNotReadyError`] for your own leg, [`ParticipantNotReadyError`] for someone else's | Drive controls off `self$` / `participants$` rather than raw IDs      |
...
### Toggles that report an outcome instead of throwing

The microphone-processing setters on `Call` — [`setEchoCancellation`],
[`setNoiseSuppression`], [`setAutoGainControl`] — and the constraint
setters on `SelfParticipant` return a `boolean` rather than throwing when
the constraint can't be applied. `false` means the change did not reach
the media the call is sending: the leg carries media the SDK did not
capture (a stream you supplied, a display capture), or it sends no audio
at all. A UI that reflects the toggle must read the return value instead
of assuming success:

```js
agcSwitch.onchange = async (e) => {
  const applied = await call.setAutoGainControl(e.target.checked);
  if (!applied) {
    e.target.checked = !e.target.checked;
    showToast("This microphone can't be reconfigured mid-call.");
  }
};
```

Any failure behind a `false` is also reported on [`call.errors$`].
...
[`CallNotReadyError`]: /docs/browser-sdk/v4/reference/errors/call-not-ready-error
[`ParticipantNotReadyError`]: /docs/browser-sdk/v4/reference/errors/participant-not-ready-error
[`setEchoCancellation`]: /docs/browser-sdk/v4/reference/webrtc-call/set-echo-cancellation
[`setNoiseSuppression`]: /docs/browser-sdk/v4/reference/webrtc-call/set-noise-suppression
[`setAutoGainControl`]: /docs/browser-sdk/v4/reference/webrtc-call/set-auto-gain-control
