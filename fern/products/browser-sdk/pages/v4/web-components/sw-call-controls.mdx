...
<ParamField path="screenShareAudio" type="boolean" default="false" toc={true}>
  Also request the shared surface's audio when the user starts a screen share. Best-effort — whether audio is captured depends on the browser, the OS, and the surface the user picks.
</ParamField>
