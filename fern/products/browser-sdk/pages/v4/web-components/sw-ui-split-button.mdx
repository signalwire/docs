...
<ParamField path="disabled" type="boolean" default="false" toc={true}>
  Renders the button inert: the main (or solo) button gets the native `disabled` attribute, hover styling is suppressed, and clicks fire no event. The chevron zone is unaffected.
</ParamField>
