...
<Accordion title="Inherited from CallParticipant">
  ...
  <ParamField path="callId" type="string | undefined" required toc={true}>
  Call ID for this participant's leg — the participant's own call context, not necessarily the local call's ID.
</ParamField>
  ...
  <ParamField path="target" type="MemberTarget" required toc={true}>
  The member's own RPC target triple. Throws [`ParticipantNotReadyError`](/docs/browser-sdk/v4/reference/errors/participant-not-ready-error) until the member's call context has been received.
</ParamField>
