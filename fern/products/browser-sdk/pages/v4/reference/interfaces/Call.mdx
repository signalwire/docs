...
<ParamField path="positions" type="Record<string, VideoPosition>" required toc={true}>
  Map of member IDs to layout positions. Pass `{}` to change only the layout and let the server place members. See [`VideoPosition`](/docs/browser-sdk/v4/reference/type-aliases/video-position).
</ParamField>
