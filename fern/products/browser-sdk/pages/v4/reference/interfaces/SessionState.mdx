...
<Accordion title="Inherited from ClientSession">
  <ParamField path="callControl" type="'routed' | 'in-dialog'" required toc={true}>
    Control transport for every call in this session, set once via [`SignalWireOptions.callControl`](/docs/browser-sdk/v4/reference/interfaces/signalwire-options). It is session-wide, so a reattached call reads it here rather than restoring it from persisted state.
  </ParamField>
