---
id: 22c575e7-c92b-422d-9b02-61d008aaf05a
title: "Click-to-Call"
...
[client-sdk]: /docs/browser-sdk/js/reference/signalwire
...
[SignalWire Client SDK][client-sdk]. 
...
<div id="call" style="height: 500px;"></div> <!-- This will display the call interface when active -->
```
...
<Warning>
The call container (the element targeted by `callParentSelector`, `#call` above) must have an
explicit CSS `height`. The widget does not set a `height` or `min-height` of its own, so without
one the container collapses to `0px`: the button still works and the call connects, but the call
interface renders as a blank, invisible area. Set a fixed height such as `height: 500px;` (or any
value that fits your layout) on this container.
</Warning>
...
<body>
  ...
  <div id="click2call"></div>                  <!-- The call button -->
  <div id="call" style="height: 500px;"></div> <!-- The call interface (needs an explicit height) -->
...
If the button works and the call connects (you can hear audio) but the call interface shows only a
blank or white area, the call container is collapsed to `0px`. Set an explicit CSS `height` on the
`callParentSelector` element (`#call`), as shown above.
