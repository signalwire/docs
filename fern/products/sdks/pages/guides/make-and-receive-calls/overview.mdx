...
|--------|--------|------------|-----|
| Answer | `await call.answer()` | `await call.answer()` | `call.Answer()` |
| Play TTS | `await call.play([{"type": "tts", "text": "Hello"}])` | `await call.play([{ type: 'tts', text: 'Hello' }])` | `call.Play([]relay.Media{{Type: "tts", Text: "Hello"}})` |
| Record | `await call.record(audio={...})` | `await call.record({ audio: {...} })` | `call.Record(relay.AudioOpts{...})` |
| Hang up | `await call.hangup()` | `await call.hangup()` | `call.Hangup()` |
| Connect | `await call.connect(devices)` | `await call.connect(devices)` | `call.Connect(devices)` |
...
| `pause()` | PlayAction, RecordAction | Pause playback/recording |
| `resume()` | PlayAction, RecordAction | Resume playback/recording |
| `volume(db)` | PlayAction, CollectAction | Adjust volume |
...
| Topic | Reference |
|-------|-----------|
| Agent-based approach | [Your First Agent](/docs/sdks/guides/first-agent) |
