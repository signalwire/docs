...
| Go | `agent.NewAgent("my-agent", agent.WithNativeFunctions("web_search"))` |
| Ruby | `AgentBase.new(name: 'my-agent', native_functions: ['web_search'])` |
| Java | `new AgentBase("my-agent", Map.of("nativeFunctions", List.of("web_search")))` |
| Perl | `AgentBase->new(name => 'my-agent', native_functions => ['web_search'])` |
| C++ | `AgentBase("my-agent", {{"native_functions", {"web_search"}}})` |
| PHP | `$agent->setNativeFunctions(['web_search'])` |
...
You've now learned all about SWAIG functions. Next, explore [Skills](/docs/sdks/guides/understanding-skills) to add pre-built capabilities to your agents.
