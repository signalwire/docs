...
| Ruby | `require 'signalwire_agents'` then `SignalWireAgents::Prefabs::SurveyAgent` |
...
<Note>
Survey handlers return `SwaigFunctionResult(string)` objects (not plain dicts). If extending SurveyAgent with custom handlers, always return a `SwaigFunctionResult`.
</Note>
