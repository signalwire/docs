...
description: Record calls using record_call() and stop_record_call() methods on FunctionResult with support for stereo/mono, multiple formats, and webhook notifications.
slug: /guides/call-recording
max-toc-depth: 3
---
...
| Ruby | `result.record_call(control_id: 'main', stereo: true, format: 'wav')` | `result.stop_record_call(control_id: 'main')` |
| Java | `result.recordCall("main", true, "wav")` | `result.stopRecordCall("main")` |
| Perl | `$result->record_call(control_id => 'main', stereo => 1, format => 'wav')` | `$result->stop_record_call(control_id => 'main')` |
| C++ | `result.record_call("main", true, "wav")` | `result.stop_record_call("main")` |
| PHP | `$agent->recordCall(format: 'wav', stereo: true)` | `$result->stopRecordCall(controlId: 'main')` |
*/}
...
| `initial_timeout` | float | None | Seconds to wait for speech (voicemail use) |
| `end_silence_timeout` | float | None | Silence duration to auto-stop (voicemail use) |
...
def start_stereo_recording(self, args, raw_data):
    return (
        FunctionResult("Recording in stereo mode")
        .record_call(
...
def start_mono_recording(self, args, raw_data):
    return (
        FunctionResult("Recording in mono mode")
        .record_call(
            control_id="mono_rec",
...
def record_agent_only(self, args, raw_data):
    return (
        FunctionResult("Recording agent audio")
...
def record_caller_only(self, args, raw_data):
    return (
        FunctionResult("Recording caller audio")
...
def record_both(self, args, raw_data):
    return (
        FunctionResult("Recording full conversation")
...
def start_auto_stop_recording(self, args, raw_data):
    return (
        FunctionResult("Recording with auto-stop")
        .record_call(
...
class ControlledRecordingAgent(AgentBase):
    ...
    def stop_recording(self, args, raw_data):
        return (
            FunctionResult("Recording has stopped")
...
def start_recording_with_beep(self, args, raw_data):
    return (
        FunctionResult("This call will be recorded")
        .record_call(
...
class ComplianceAgent(AgentBase):
    ...
    def start_compliant_recording(self, args, raw_data):
        ...
        return (
            FunctionResult("Recording has begun. Thank you for your consent.")
            .record_call(
    ...
    def resume_recording(self, args, raw_data):
        ...
        return (
            FunctionResult("Recording resumed.")
            .record_call(
                control_id=f"compliance_{call_id}",
                stereo=True,
                format="wav"
