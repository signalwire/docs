...
The DataMap fluent builder API:

| Method | Python |
|--------|--------|
| Create | `DataMap("name")` |
| Description | `.description("d")` |
| Parameter | `.parameter("p","string","d")` |
| Webhook | `.webhook("GET","url")` |
| Output | `.output(FunctionResult("..."))` |
| Fallback | `.fallback_output(...)` |

<Note>
The builder examples below use Python syntax.
</Note>
...
weather_dm = (
    ...
    .fallback_output(FunctionResult("Couldn't get weather for ${args.city}"))
)
self.register_swaig_function(weather_dm.to_swaig_function())
```
