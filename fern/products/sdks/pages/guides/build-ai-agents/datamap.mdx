---
title: "DataMap"
description: DataMap provides serverless API integration, letting you define functions that call REST APIs directly from SignalWire's infrastructure without running code on your server.
...
a := agent.NewAgent("weather-agent")
a.AddLanguage("English", "en-US", "rime.spore")

weatherDm := datamap.DataMap("get_weather").
    Description("Get current weather for a city").
...
agent = AgentBase.new(name: 'weather-agent')
agent.add_language('English', 'en-US', 'rime.spore')

weather_dm = DataMap.new('get_weather')
  .purpose('Get current weather for a city')
  .parameter('city', 'string', 'City name')
  .webhook('GET', 'https://api.weather.com/v1/current?key=API_KEY&q=${enc:args.city}')
...
<Tab title="Java">
```java
import com.signalwire.agents.*;

AgentBase agent = new AgentBase("weather-agent");
agent.addLanguage("English", "en-US", "rime.spore");

DataMap weatherDm = new DataMap("get_weather")
    .description("Get current weather for a city")
    .parameter("city", "string", "City name", true)
    .webhook("GET", "https://api.weather.com/v1/current?key=API_KEY&q=${enc:args.city}")
...
my $agent = SignalWire::Agents::AgentBase->new(name => 'weather-agent');
$agent->add_language('English', 'en-US', 'rime.spore');

my $weather_dm = DataMap->new('get_weather')
    ->description('Get current weather for a city')
...
auto agent = signalwire::AgentBase("weather-agent");
agent.add_language("English", "en-US", "rime.spore");
...
$weatherDm = (new DataMap('get_weather'))
    ->purpose('Get current weather for a city')
    ->parameter('city', 'string', 'City name', required: true)
    ->webhookUrl('GET', 'https://api.weather.com/v1/current?key=API_KEY&q=${enc:args.city}')
    ->output(new FunctionResult(
        'The weather in ${args.city} is ${response.current.condition.text}, '
        . '${response.current.temp_f} degrees Fahrenheit'
    ));
...
var agent = new AgentBase(new AgentOptions { Name = "weather-agent" });
agent.AddLanguage("English", "en-US", "rime.spore");
...
The DataMap fluent builder API is available across all SDK languages:

| Method | Python | TypeScript | Go | Ruby | Java | Perl | C++ | PHP | C# |
|--------|--------|------------|-----|------|------|------|-----|-----|-----|
| Create | `DataMap("name")` | `new DataMap("name")` | `datamap.DataMap("name")` | `DataMap.new("name")` | `new DataMap("name")` | `DataMap->new("name")` | `datamap::DataMap("name")` | `new DataMap('name')` | `new DataMap("name")` |
| Description | `.description("d")` | `.purpose("d")` | `.Description("d")` | `.purpose("d")` | `.description("d")` | `->description("d")` | `.description("d")` | `->purpose('d')` | `.Description("d")` |
| Parameter | `.parameter("p","string","d")` | `.parameter("p","string","d")` | `.Parameter("p","string","d",true)` | `.parameter("p","string","d")` | `.parameter("p","string","d",true)` | `->parameter("p","string","d")` | `.parameter("p","string","d",true)` | `->parameter('p','string','d',required:true)` | `.Parameter("p","string","d",required:true)` |
| Webhook | `.webhook("GET","url")` | `.webhook("GET","url")` | `.Webhook("GET","url")` | `.webhook("GET","url")` | `.webhook("GET","url")` | `->webhook("GET","url")` | `.webhook("GET","url")` | `->webhookUrl('GET','url')` | `.Webhook("GET","url")` |
| Output | `.output(SwaigFunctionResult("..."))` | `.output(new SwaigFunctionResult("..."))` | `.Output(swaig.NewFunctionResult("..."))` | `.output(FunctionResult.new("..."))` | `.output(new FunctionResult("..."))` | `->output(FunctionResult->new("..."))` | `.output(swaig::FunctionResult("..."))` | `->output(new FunctionResult('...'))` | `.Output(new FunctionResult("..."))` |
| Fallback | `.fallback_output(...)` | `.fallbackOutput(...)` | `.FallbackOutput(...)` | `.fallback_output(...)` | `.fallbackOutput(...)` | `->fallback_output(...)` | `.fallback_output(...)` | `->fallbackOutput(...)` | `.FallbackOutput(...)` |

<Note>
The builder examples below use Python syntax. The fluent-chaining pattern is the same in every language -- translate method names as shown in the table above.
</Note>
...
<Note>
The examples below use Python. The DataMap fluent builder pattern and variable substitution syntax (`${args.city}`, `${response.field}`) are identical across all SDK languages. See the builder methods table above for language-specific method names.
</Note>
...
class MultiServiceAgent(AgentBase):
    def __init__(self):
        ...
        self.register_swaig_function(
            ...
            .output(SwaigFunctionResult(
                "${args.city}: ${response.current.temp_f} degrees F, "
                "${response.current.condition.text}"
            ))
            .fallback_output(SwaigFunctionResult(
...
def get_weather(self, args, raw_data):
    ...
    return SwaigFunctionResult(
        f"Weather in {city}: {data['temp']} degrees F, {data['condition']}"
    )
```
...
weather_dm = (
    ...
    .output(SwaigFunctionResult(
        "Weather in ${args.city}: ${response.temp} degrees F, ${response.condition}"
    ))
