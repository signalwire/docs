...
```typescript
import { SkillBase, SwaigFunctionResult, AgentBase } from 'signalwire-agents';
...
class GreetingSkill extends SkillBase {
  ...
  greetHandler(args: Record<string, any>, rawData: any): SwaigFunctionResult {
    ...
    return new SwaigFunctionResult(greeting);
  }
}
```
</Tab>
...
func (s *GreetingSkill) greetHandler(args map[string]any, rawData map[string]any) *agent.SwaigFunctionResult {
    ...
    return agent.NewSwaigFunctionResult(greeting)
}
```
</Tab>
<Tab title="Ruby">
...
class GreetingSkill < SignalWireAgents::SkillBase
  ...
  def greet_handler(args, raw_data)
    ...
    greeting = @greeting_style == 'formal' ?
      "Good day, #{name}. How may I assist you?" :
      "Hey #{name}! Great to hear from you!"
    SwaigFunctionResult.new(greeting)
  end
end
```
</Tab>
...
public class GreetingSkill extends SkillBase {
    ...
    private SwaigFunctionResult greetHandler(Map<String, Object> args, Map<String, Object> rawData) {
...
sub greet_handler {
    ...
    my $greeting = $self->{greeting_style} eq 'formal'
        ? "Good day, $name. How may I assist you?"
        : "Hey $name! Great to hear from you!";
    return SignalWire::Agents::SwaigFunctionResult->new($greeting);
}
1;
```
</Tab>
...
private:
    ...
    signalwire::agents::SwaigFunctionResult greet_handler(const nlohmann::json& args, const nlohmann::json& raw) {
        std::string name = args.value("name", "friend");
        std::string greeting = (greeting_style_ == "formal")
            ? "Good day, " + name + ". How may I assist you?"
            : "Hey " + name + "! Great to hear from you!";
        return signalwire::agents::SwaigFunctionResult(greeting);
    }
};
```
</Tab>
...
class GreetingSkill extends SkillBase {
    ...
    private function greetHandler(array $args, array $rawData): SwaigFunctionResult {
...
class StatefulSkill(SkillBase):
    ...
    def my_handler(self, args, raw_data):
        ...
        result = SwaigFunctionResult(f"Call #{count + 1}")
