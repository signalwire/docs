...
| Section | Description |
|---------|-------------|
| [Architecture](/docs/sdks/guides/architecture) | AgentBase class and mixin composition |
| [SWML](/docs/sdks/guides/swml) | Understanding SWML document structure |
| [SWAIG](/docs/sdks/guides/swaig) | How AI calls your functions |
| [Lifecycle](/docs/sdks/guides/lifecycle) | Request/response flow |
| [Security](/docs/sdks/guides/security) | Authentication and token security |
...
agent.defineTool({
  ...
  parameters: { order_id: { type: 'string', description: 'Order ID' } },
  handler: (args) => {
    const result = new SwaigFunctionResult(`Order ${args.order_id}: Shipped, arrives tomorrow`);
    return result;
  },
});
...
Now that you understand how AgentBase is structured, let's look at the [SWML documents](/docs/sdks/guides/swml) it generates.
