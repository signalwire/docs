...
- Core concepts from Chapter 2 ([SWML](/docs/sdks/guides/swml), [SWAIG](/docs/sdks/guides/swaig), [Lifecycle](/docs/sdks/guides/lifecycle))
...
| Section | Description |
|---------|-------------|
| [AgentBase](/docs/sdks/guides/agent-base) | Core class and constructor options |
| [Static vs Dynamic](/docs/sdks/guides/static-vs-dynamic) | Choosing the right pattern |
| [Prompts & POM](/docs/sdks/guides/prompts-pom) | Prompt engineering for voice AI |
| [Voice & Language](/docs/sdks/guides/voice-language) | Voice selection and multi-language |
| [AI Parameters](/docs/sdks/guides/ai-parameters) | Behavior tuning |
| [Hints](/docs/sdks/guides/hints) | Speech recognition improvement |
| [Call Flow](/docs/sdks/guides/call-flow) | Customizing call answer behavior |
...
|--------|--------|------------|-----|
| Add language | `add_language(name, code, voice)` | `addLanguage({ name, code, voice })` | `AddLanguage(map[string]any{...})` |
| Add prompt | `prompt_add_section(title, body)` | `promptAddSection(title, { body })` | `PromptAddSection(title, body, bullets)` |
| Set params | `set_params(dict)` | `setParams(obj)` | `SetParam(key, val)` |
| Add hints | `add_hints(list)` | `addHints(array)` | `AddHints([]string{...})` |
| Define tool | `define_tool(name=..., handler=fn)` | `defineTool({ name, handler })` | `DefineTool(ToolDefinition{...})` |
| Post-prompt | `set_post_prompt(text)` | `setPostPrompt(text)` | `SetPostPrompt(text)` |
