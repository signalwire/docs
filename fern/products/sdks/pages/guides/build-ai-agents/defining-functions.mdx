...
agent.defineTool({
  ...
  handler: (args) => {
    ...
    return new SwaigFunctionResult(`Order ${args.order_number}: ${status}`);
  }
});
...
| Section | Description |
|---------|-------------|
| [Defining Functions](/docs/sdks/guides/defining-functions) | Creating SWAIG functions with define_tool() |
| [Parameters](/docs/sdks/guides/parameters) | Defining and validating function parameters |
| [Results & Actions](/docs/sdks/guides/result-actions) | Returning results and triggering actions |
| [DataMap](/docs/sdks/guides/data-map) | Serverless API integration |
| [Native Functions](/docs/sdks/guides/native-functions) | Built-in SignalWire functions |
...
| Language | Signature |
|----------|-----------|
| Python | `def handler(self, args, raw_data):` returns `SwaigFunctionResult` |
| TypeScript | `(args: Record<string, any>, raw?: Record<string, any>) =>` returns `SwaigFunctionResult` |
...
class TypedAgent(AgentBase):
    ...
    def set_priority(self, ticket_id: str, level: Literal["low", "medium", "high"]):
        ...
        """
        return SwaigFunctionResult(f"Ticket {ticket_id} set to {level} priority")
```
...
def caller_info(self, raw_data: dict):
    ...
    return SwaigFunctionResult(f"Your number is {caller}")
```
