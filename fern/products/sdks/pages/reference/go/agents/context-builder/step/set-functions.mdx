---
title: "SetFunctions"
slug: /reference/go/agents/context-builder/step/set-functions
description: Set which SWAIG functions are available during this step.
max-toc-depth: 3
---

[ref-step]: /docs/sdks/reference/go/agents/context-builder/step

Set which SWAIG functions are available during this step. Restricting functions
per step prevents the AI from calling irrelevant tools.

## **Parameters**

<ParamField path="functions" type="interface{}" required={true} toc={true}>
  Either `"none"` to disable all functions, or a list of function names to allow.
</ParamField>

## **Returns**

[`Step`][ref-step] -- Self for method chaining.

## **Example**

```go {9,12}
package main

import (
    "github.com/signalwire/signalwire-go/pkg/agent"
    "github.com/signalwire/signalwire-go/pkg/contexts"
)

func main() {
    agent := agent.NewAgentBase(agent.WithName("my-agent"), agent.WithRoute("/agent"))

    contexts := agent.define_contexts()
    ctx := contexts.add_context("default")
    greet := ctx.add_step("greet")
    greet.SetText("Welcome the caller.")
    greet.SetFunctions("none")
    verify := ctx.add_step("verify")
    verify.SetText("Verify the caller's identity.")
    verify.SetFunctions([]string{"lookup_account", "verify_identity"})

    agent.Serve()
}
```
