...
slug: /reference/go/agents/context-builder/gather-classes
description: Structured information gathering within context steps using the server-side gather_info system.
max-toc-depth: 3
---

[step-setgatherinfo]: /docs/sdks/reference/go/agents/context-builder/step/set-gather-info
[step-addgatherquestion]: /docs/sdks/reference/go/agents/context-builder/step/add-gather-question
...
<ParamField path="output_key" type="string" toc={true}>
  Key in `global_data` under which to nest all answers. When `nil`, answers
...
<ParamField path="completion_action" type="string" toc={true}>
  ...
  - `nil` -- return to normal step mode
...
func main() {
    // IntakeAgent embeds AgentBase
        func __init__() *swaig.FunctionResult {
            // Base config: name="intake-agent", route="/intake"
