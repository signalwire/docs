...
slug: /reference/go/agents/livewire/agent/pipeline-nodes
...
agent.stt_node(audio=nil, model_settings=nil)
```

No-op. SignalWire handles speech recognition in its control plane.

<ParamField path="audio" type="interface{}" default="None" toc={true}>
  Audio input. Accepted for API compatibility.
</ParamField>

<ParamField path="model_settings" type="interface{}" default="None" toc={true}>
...
```go
agent := Agent(instructions="You are a helpful assistant.", llm="gpt-4o")
agent.llm_node(chat_ctx=nil, tools=nil, model_settings=nil)
```

No-op. SignalWire handles LLM inference in its control plane.

<ParamField path="chat_ctx" type="interface{}" default="None" toc={true}>
  Chat context. Accepted for API compatibility.
</ParamField>

<ParamField path="tools" type="interface{}" default="None" toc={true}>
...
```go
agent := Agent(instructions="You are a helpful assistant.", llm="gpt-4o")
agent.tts_node(text=nil, model_settings=nil)
```

No-op. SignalWire handles text-to-speech in its control plane.

<ParamField path="text" type="interface{}" default="None" toc={true}>
