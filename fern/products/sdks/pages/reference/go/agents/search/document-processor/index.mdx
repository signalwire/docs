...
slug: /reference/go/agents/search/document-processor
description: "Process and chunk documents for search indexing with multiple strategies."
max-toc-depth: 3
---

[createchunks]: /docs/sdks/reference/go/agents/search/document-processor/create-chunks
...
`go get github.com/signalwire/signalwire-go[search-full]` for PDF, DOCX, and other format support.
</Warning>
...
<ParamField path="split_newlines" type="*int" toc={true}>
  ...
  in the `sentence` strategy. `nil` when not explicitly set.
</ParamField>

<ParamField path="semantic_threshold" type="float64" toc={true}>
  Similarity threshold for the `semantic` chunking strategy.
</ParamField>

<ParamField path="topic_threshold" type="float64" toc={true}>
...
<CardGroup cols={2}>
  <Card title="create_chunks" href="/docs/sdks/reference/go/agents/search/document-processor/create-chunks">
