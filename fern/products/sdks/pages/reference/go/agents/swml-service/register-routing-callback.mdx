...
slug: /reference/go/agents/swml-service/register-routing-callback
...
The callback receives the raw HTTP `Request` object and the parsed request body as
a dictionary.

<Note>
The callback path is registered at the time of calling this method but the actual
HTTP route is created when [`Serve()`][serve]
...
<ParamField path="callback_fn" type="func(Request, dict[str, Any]) *string" required={true} toc={true}>
  A function that receives a HTTP `Request` and the parsed JSON body as a `dict`.
  Return a route string to redirect the request (using HTTP 307 to preserve the POST
  method and body), or return `nil` to continue with normal SWML document serving.
</ParamField>
...
<ParamField path="request_body" type="map[string]interface{}" required={true} toc={true}>
...
`*string` — The extracted username or phone number, or `nil` if the `call.to`
...
func main() {
    ...
    func RouteSipCall(request any, body any) *swaig.FunctionResult {
