...
slug: /reference/go/agents/helpers
...
func main() {
    ...
    for skill_name, skill_info in schema.items():
        fmt.Println(fmt.Sprintf("%v: %v", skill_name, skill_info.get('description', '')))
        for param, meta in skill_info[]string{"parameters"}.items():
            fmt.Println(fmt.Sprintf("  %v: %v (required=%v)", param, meta['type'], meta.get('required', false)))
}
```
...
func main() {
    class MyWeatherSkill:
        ...
        func Setup() *swaig.FunctionResult {
            return true

        func RegisterTools() *swaig.FunctionResult {
            DefineTool(name="get_weather", handler=_get_weather, description="Get weather")
...
func main() {
    ...
    agent := AgentBase()
    agent.AddSkill("my_custom_skill")
}
```
