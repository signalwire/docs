...
[skill]: /docs/sdks/reference/go/agents/skills#native_vector_search
...
<ParamField path="amenities" type="map[str, dict[str]str]" required={true} toc={true}>
...
<ParamField path="hours_of_operation" type="map[string]string" toc={true}>
  Dictionary of operating hours by department or service. Defaults to `{"default": "9 AM - 5 PM"}`.
</ParamField>

<ParamField path="special_instructions" type="[]string" toc={true}>
...
agent := ConciergeAgent(
    ...
    services := []string{"room service", "spa treatments", "restaurant reservations", "tours"},
    amenities := {
    ...
    special_instructions := []string{"Mention the daily happy hour at the pool bar (4-6 PM)."},
...
<ParamField path="faqs" type="[]map[string]string" required={true} toc={true}>
...
<ParamField path="questions" type="[]dict" toc={true}>
  List of question dictionaries. If `nil`, the agent operates in dynamic mode where
...
```go
func GetQuestions(queryParams map[string]any, bodyParams map[string]any, headers map[string]string) *swaig.FunctionResult {
    question_set := query_params[]string{"type"}
...
<ParamField path="callback" type="func(dict, dict, dict) []dict" required={true} toc={true}>
...
func main() {
    ...
    agent.DefineTool(agent.ToolDefinition{
        ...
        Description: "Process collected answers",
        Handler: func(args map[string]any, rawData map[string]any) *swaig.FunctionResult {
        // Inside a tool handler
        global_data := rawData[]string{"global_data"}
        answers := global_data[]string{"answers"}
        // [{"key_name": "name", "answer": "John Doe"}, ...]
        return swaig.NewFunctionResult(fmt.Sprintf("Processed %v answers.", len(answers)))
...
<ParamField path="departments" type="[]map[string]string" required={true} toc={true}>
...
agent := SurveyAgent(
    ...
    questions := [
        ...
        },
        {
            ...
            "options": []string{"Poor", "Fair", "Good", "Excellent"}
        },
        {
            "id": "purchase_again",
...
func main() {
    // MyFAQBot embeds FAQBotAgent
        ...
        func Escalate(args map[string]any, rawData map[string]any) *swaig.FunctionResult {
            return swaig.NewFunctionResult("Transferring to agent...").connect("+15551234567")
