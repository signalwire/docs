---
title: "Constants"
slug: /reference/go/relay/constants
description: "RELAY constants for call states, events, and message states."
max-toc-depth: 3
---

[callstateevent-endreason]: /docs/sdks/reference/go/relay/events
[connectevent-connectstate]: /docs/sdks/reference/go/relay/events
[call-on]: /docs/sdks/reference/go/relay/call
[relayevent-eventtype]: /docs/sdks/reference/go/relay/events
[message-wait]: /docs/sdks/reference/go/relay/message
[playevent-state]: /docs/sdks/reference/go/relay/events
[recordevent-state]: /docs/sdks/reference/go/relay/events
...
func main() {
    ...
    func HandleCallState(event any) *swaig.FunctionResult {
        if event.call_state == CALL_STATE_ANSWERED:
            fmt.Println("Call answered!")
        elif event.call_state == CALL_STATE_ENDED:
            fmt.Println(fmt.Sprintf("Call ended: %v", event.end_reason))

    // Check terminal message states
    func HandleMessageState(message any) *swaig.FunctionResult {
        if message.state in MESSAGE_TERMINAL_STATES:
            fmt.Println("Message delivery complete")
}
```
