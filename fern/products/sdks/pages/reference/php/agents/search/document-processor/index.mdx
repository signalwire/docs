...
slug: /reference/php/agents/search/document-processor
description: "Process and chunk documents for search indexing with multiple strategies."
max-toc-depth: 3
---

[createchunks]: /docs/sdks/reference/php/agents/search/document-processor/create-chunks
...
```php
<?php
use SignalWire\Search\DocumentProcessor;
...
`composer require signalwire/sdk[search-full]` for PDF, DOCX, and other format support.
</Warning>
...
<ParamField path="split_newlines" type="?int" toc={true}>
  ...
  in the `sentence` strategy. `null` when not explicitly set.
</ParamField>
...
<CardGroup cols={2}>
  <Card title="create_chunks" href="/docs/sdks/reference/php/agents/search/document-processor/create-chunks">
