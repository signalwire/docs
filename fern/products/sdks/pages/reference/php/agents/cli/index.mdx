...
|-------|-----------------|--------------|
| `search` | `composer require "signalwire[search]"` | `sw-search` build and query |
| `search-full` | `composer require "signalwire[search-full]"` | PDF and DOCX processing |
| `search-nlp` | `composer require "signalwire[search-nlp]"` | Advanced NLP features (spaCy) |
| `search-queryonly` | `composer require "signalwire[search-queryonly]"` | Query-only mode (smaller install) |
| `pgvector` | `composer require "signalwire[pgvector]"` | PostgreSQL vector storage |
