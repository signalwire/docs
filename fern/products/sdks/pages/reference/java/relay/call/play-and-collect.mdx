...
slug: /reference/java/relay/call/play-and-collect
description: "Play audio and collect DTMF or speech input."
max-toc-depth: 3
---

[collectaction]: /docs/sdks/reference/java/relay/actions
...
<ParamField path="media" type="List<Map<String, Object>>" required={true} toc={true}>
...
<Indent>
<ParamField path="collect.digits" type="Map<String, Object>" toc={true}>
...
<ParamField path="collect.speech" type="Map<String, Object>" toc={true}>
...
<ParamField path="collect.speech.hints" type="List<String>" toc={true}>
  Words or phrases to boost recognition accuracy.
</ParamField>
</Indent>
</Indent>

<ParamField path="volume" type="@Nullable double" toc={true}>
...
async def handle_call(call):
    ...
    var action = await call.play_and_collect( [{"type": "tts", "text": "Press 1 for sales, 2 for support."}], { "digits": {"max": 1, "digit_timeout": 5, "terminators": "#"}, }, );
    var event = await action.wait();

    var result = event.params.get("result", {});
    var digits = result.get("digits", "");
    if digits == "1":
