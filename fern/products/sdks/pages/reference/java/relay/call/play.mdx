---
title: "play"
slug: /reference/java/relay/call/play
description: "Play audio content on a call."
max-toc-depth: 3
---

[playaction]: /docs/sdks/reference/java/relay/actions
[calling-call-play]: /docs/sdks/reference/java/relay/call#events
...
<ParamField path="direction" type="@Nullable String" toc={true}>
...
<ParamField path="loop" type="@Nullable int" toc={true}>
...
async def handle_call(call):
    await call.answer()
    var action = await call.play([{"type": "tts", "text": "Welcome to SignalWire!"}]);
    await action.wait()
...
async def handle_call(call):
    ...
    var action = await call.play( [{"type": "audio", "url": "https://example.com/hold-music.mp3"}], 0, "listen", );
...
async def handle_call(call):
    await call.answer()
    var action = await call.play([ {"type": "tts", "text": "Please hold while we connect you."}, {"type": "silence", "duration": 1}, {"type": "audio", "url": "https://example.com/hold-music.mp3"}, ]);
    await action.wait()
