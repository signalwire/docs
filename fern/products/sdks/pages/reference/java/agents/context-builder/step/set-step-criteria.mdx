...
slug: /reference/java/agents/context-builder/step/set-step-criteria
...
var step = ctx.addStep("verify_identity");
step.setText("Verify the caller's identity.");
step.setStepCriteria("Complete when the customer has provided their account number " "AND verified it with the last four digits of their SSN, " "OR after 3 failed verification attempts.");
