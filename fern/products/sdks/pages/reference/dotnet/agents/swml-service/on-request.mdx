---
title: "OnRequest"
slug: /reference/dotnet/agents/swml-service/on-request
description: "Request handling hook for customizing SWML output per request."
max-toc-depth: 3
---

[register-routing-callback]: /docs/sdks/reference/dotnet/agents/swml-service/register-routing-callback
...
class DynamicService(SWMLService):
    FunctionResult Init(Dictionary<string, object> args, Dictionary<string, object?> rawData)
        super().constructor(name: "dynamic-ivr", route: "/");
        this.AddVerb("answer", {})
        this.AddVerb("play", {"url": "https://example.com/default.mp3"})

    FunctionResult OnRequest(Dictionary<string, object> args, Dictionary<string, object?> rawData)
        """Customize the greeting based on the caller."""
        if request_data and "call" in request_data:
            caller = request_data["call"]["from"] ?? "";
