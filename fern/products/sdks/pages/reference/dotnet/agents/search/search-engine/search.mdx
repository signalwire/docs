---
title: "search"
slug: /reference/dotnet/agents/search/search-engine/search
description: Perform a hybrid search query against the indexed documents.
max-toc-depth: 3
---

[preprocess-query]: /docs/sdks/reference/dotnet/agents/search/helpers
...
<ParamField path="query_vector" type="List<double>" required={true} toc={true}>
...
<ParamField path="similarity_threshold" type="double" default="0.0" toc={true}>
...
<ParamField path="keyword_weight" type="double?" toc={true}>
...
for result in results:
    Console.WriteLine($"[{result['score']:.3f}] {result['metadata']['filename']}");
    Console.WriteLine($"  {result['content'][:120]}...");
