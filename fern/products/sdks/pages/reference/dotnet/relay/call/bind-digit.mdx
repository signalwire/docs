---
title: "BindDigit"
slug: /reference/dotnet/relay/call/bind-digit
description: Bind a DTMF digit sequence to trigger a RELAY method.
max-toc-depth: 3
---

[clear-digit-bindings]: /docs/sdks/reference/dotnet/relay/call/clear-digit-bindings
...
async Task HandleCall(call)
{
    ...
    var result = await call.bind_digit(;
        digits: "*0",
        bind_method: "calling.transfer",
        bind_params: {"dest": "operator"},;
        realm: "shortcuts",
    )
    Console.WriteLine($"Digit binding created: {result}");
