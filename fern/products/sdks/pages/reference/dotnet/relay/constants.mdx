...
[callstateevent-endreason]: /docs/sdks/reference/dotnet/relay/events
[connectevent-connectstate]: /docs/sdks/reference/dotnet/relay/events
[call-on]: /docs/sdks/reference/dotnet/relay/call
[relayevent-eventtype]: /docs/sdks/reference/dotnet/relay/events
[message-wait]: /docs/sdks/reference/dotnet/relay/message
[playevent-state]: /docs/sdks/reference/dotnet/relay/events
[recordevent-state]: /docs/sdks/reference/dotnet/relay/events
[call-detect]: /docs/sdks/reference/dotnet/relay/call/detect
...
<ParamField path="PROTOCOL_VERSION" type="dict" toc={true}>
...
<ParamField path="CALL_STATES" type="(string, ...)" toc={true}>
...
[`CallStateEvent.end_reason`][callstateevent-endreason] when the call
...
[`ConnectEvent.connect_state`][connectevent-connectstate].
...
matching against [`RelayEvent.event_type`][relayevent-eventtype].
...
<ParamField path="MESSAGE_TERMINAL_STATES" type="(string, ...)" toc={true}>
  Tuple of terminal states that resolve a [`Message.wait()`][message-wait] call:
...
FunctionResult HandleCallState(Dictionary<string, object> args, Dictionary<string, object?> rawData)
    if event.call_state == CALL_STATE_ANSWERED:
        Console.WriteLine("Call answered!");
    elif event.call_state == CALL_STATE_ENDED:
        Console.WriteLine($"Call ended: {event.end_reason}");

// Check terminal message states
FunctionResult HandleMessageState(Dictionary<string, object> args, Dictionary<string, object?> rawData)
    if message.state in MESSAGE_TERMINAL_STATES:
        Console.WriteLine("Message delivery complete");
