...
[stop]: /docs/sdks/reference/dotnet/relay/actions/detect-action/stop
...
<CardGroup cols={2}>
  <Card title="stop" href="/docs/sdks/reference/dotnet/relay/actions/detect-action/stop">
...
async Task HandleCall(call)
{
    ...
    var event = await action.wait(timeout=10);
    detect_result = event.params.get("detect", {});
    Console.WriteLine($"Detected: {detect_result}");
