...
def on_call(call)
    def handle_state(event)
        puts("Call state: #{event.call_state}")
...
<ParamField path="payload" type="Hash" required={true} toc={true}>
...
<ParamField path="payload.params" type="Hash" required={true} toc={true}>
...
puts(event.event_type)  # "calling.call.state"
```
...
<ParamField path="device" type="Hash" toc={true}>
...
def handle_call(call)
    def handle_state(event)
        puts("State: #{event.call_state}, reason: #{event.end_reason}")
...
dispatched at the client level via the `@client.on_call` method rather than
through `call.on()`.
...
<ParamField path="record" type="Hash" toc={true}>
...
def handle_call(call)
    def handle_record(event)
        puts("Recording #{event.state}: #{event.url}")
...
<ParamField path="result" type="Hash" toc={true}>
...
<ParamField path="final" type="Boolean, nil" toc={true}>
...
@client.on_call
def handle_call(call)
    def handle_collect(event)
        puts("Collected: #{event.result}")
...
<ParamField path="peer" type="Hash" toc={true}>
...
def handle_call(call)
    def handle_connect(event)
        puts("Connection: #{event.connect_state}")
...
<ParamField path="detect" type="Hash" toc={true}>
...
@client.on_call
def handle_call(call)
    def handle_detect(event)
        puts("Detected: #{event.detect}")
...
<ParamField path="fax" type="Hash" toc={true}>
...
<ParamField path="tap" type="Hash" toc={true}>
...
def handle_call(call)
    def handle_tap(event)
        puts("Tap #{event.state}: #{event.tap}")
...
def handle_call(call)
    def handle_stream(event)
        puts("Stream #{event.state}: #{event.url}")
...
@client.on_call
def handle_call(call)
    def handle_digits(event)
        puts("Send digits: #{event.state}")
...
<ParamField path="call" type="Hash" toc={true}>
...
@client.on_call
def handle_call(call)
    def handle_refer(event)
        puts("Refer to #{event.sip_refer_to}: #{event.state}")
...
@client.on_call
def handle_call(call)
    def handle_hold(event)
        puts("Hold: #{event.state}")
...
@client.on_call
def handle_call(call)
    def handle_denoise(event)
        puts("Denoise active: #{event.denoised}")
...
@client.on_call
def handle_call(call)
    def handle_pay(event)
        puts("Payment: #{event.state}")
...
def handle_call(call)
    def handle_queue(event)
        puts("Queue #{event.queue_name}: position #{event.position}/#{event.size}")
...
@client.on_call
def handle_call(call)
    def handle_conference(event)
        puts("Conference #{event.name}: #{event.status}")
...
def handle_call(call)
    def handle_transcribe(event)
        puts("Transcription #{event.state}: #{event.url}")
...
def handle_call(call)
    def handle_error(event)
        puts("Error #{event.code}: #{event.message}")
...
def handle_message(message)
    def handle_state(event)
        puts("Message #{event.message_id}: #{event.message_state}")
