...
slug: /reference/ruby/agents/swml-service/register-routing-callback
...
<ParamField path="callback_fn" type="Proc" required={true} toc={true}>
  A function that receives a Rack `Request` and the parsed JSON body as a `Hash`.
...
<ParamField path="request_body" type="Hash" required={true} toc={true}>
...
`String` or `nil` — The extracted username or phone number, or `nil` if the `call.to`
