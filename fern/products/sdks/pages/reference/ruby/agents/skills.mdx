...
[infogathereragent]: /docs/sdks/reference/ruby/agents/prefabs#infogathereragent
...
<ParamField path="files" type="Array<dict>" required={true} toc={true}>
...
**Requirements:** Search extras installed (`gem install "signalwire[search]"`)
...
<ParamField path="services" type="Array<dict>" toc={true}>
  Specific services and tools to enable. Each hash has `name` (str) and `tools` (str `"*"`
...
<ParamField path="questions" type="Array<dict>" required={true} toc={true}>
  List of question hashes with `key_name`, `question_text`, and optional `confirm`.
</ParamField>
...
<ParamField path="categories" type="Array\<String\>" toc={true}>
...
You can create custom skills and register them from external directories or via gems.
...
Install skills as gems by defining entry points:
