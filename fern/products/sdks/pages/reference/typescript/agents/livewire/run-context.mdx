...
@function_tool
function savePreference(ctx: RunContext, color: string): string {
...
<ParamField path="session" type="AgentSession | undefined" toc={true}>
  ...
  that owns this context. May be `undefined` if the tool is invoked outside a session.
</ParamField>
...
<ParamField path="speechHandle" type="unknown" toc={true}>
  Handle to the current speech output. Accepted for API compatibility.
</ParamField>

<ParamField path="functionCall" type="unknown" toc={true}>
...
@function_tool
function addNote(ctx: RunContext, note: string): string {
