...
**list_skills_with_params**() -> `Record<str, Record<string, unknown>>`
...
`Record<str, Record<string, unknown>>` -- Dictionary keyed by skill name. Each value
...
for skill_name, skill_info in schema.items():
    console.log(`${skill_name}: ${skill_info.get('description', '')}`)
    for param, meta in skill_info.get("parameters", {}).items():
        console.log(`  ${param}: ${meta['type']} (required=${meta.get('required', false)})`)
```
...
**register_skill**(`skill_class`) -> `undefined`
...
<ParamField path="skillClass" type="class" required={true} toc={true}>
...
**add_skill_directory**(`path`) -> `undefined`
