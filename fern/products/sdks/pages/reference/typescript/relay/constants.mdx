...
<ParamField path="PROTOCOLVersion" type="dict" toc={true}>
  RELAY protocol version. Value: `{"major": 2, "minor": 0, "revision": 0}`.
</ParamField>

<ParamField path="AGENTString" type="string" toc={true}>
  User agent string sent during connection. Value: `"signalwire-agents-python/1.0"`.
</ParamField>

<ParamField path="DEFAULTRelayHost" type="string" toc={true}>
...
<ParamField path="CALLStates" type={"[string, ...]"} toc={true}>
...
<ParamField path="MESSAGETerminalStates" type={"[string, ...]"} toc={true}>
