---
title: "Constants"
slug: /reference/cpp/relay/constants
description: "RELAY constants for call states, events, and message states."
max-toc-depth: 3
---

[callstateevent-endreason]: /docs/sdks/reference/cpp/relay/events
[connectevent-connectstate]: /docs/sdks/reference/cpp/relay/events
[call-on]: /docs/sdks/reference/cpp/relay/call
[relayevent-eventtype]: /docs/sdks/reference/cpp/relay/events
[message-wait]: /docs/sdks/reference/cpp/relay/message
[playevent-state]: /docs/sdks/reference/cpp/relay/events
[recordevent-state]: /docs/sdks/reference/cpp/relay/events
...
<ParamField path="PROTOCOL_VERSION" type="nlohmann::json" toc={true}>
  RELAY protocol version. Value: `{"major": 2, "minor": 0, "revision": 0}`.
</ParamField>

<ParamField path="AGENT_STRING" type="std::string" toc={true}>
  User agent string sent during connection. Value: `"signalwire-agents-python/1.0"`.
</ParamField>

<ParamField path="DEFAULT_RELAY_HOST" type="std::string" toc={true}>
...
<ParamField path="CALL_STATES" type="std::tuple<std::string, ...>" toc={true}>
...
<ParamField path="MESSAGE_TERMINAL_STATES" type="std::tuple<std::string, ...>" toc={true}>
...
void handle_call_state(const auto& event) {
    if (event.call_state == CALL_STATE_ANSWERED) {
        std::cout << "Call answered!" << std::endl;
    } else if (event.call_state == CALL_STATE_ENDED) {
        std::cout << "Call ended: " + std::string(event.end_reason) << std::endl;

// Check terminal message states
void handle_message_state(const auto& message) {
