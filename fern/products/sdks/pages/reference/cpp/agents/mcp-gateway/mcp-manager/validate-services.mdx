...
slug: /reference/cpp/agents/mcp-gateway/mcp-manager/validate-services
description: Validate that all configured services can be started successfully.
max-toc-depth: 3
---

[ref-mcpgateway]: /docs/sdks/reference/cpp/agents/mcp-gateway/mcp-gateway
...
for service, valid in results.items():
    ...
    std::cout << "  " + std::string(service) + ": " + std::string(status) << std::endl;
