...
slug: /reference/cpp/agents/mcp-gateway/session-manager/create-session
...
<ParamField path="timeout" type="std::optional<int>" toc={true}>
  Session timeout in seconds. Defaults to the manager's `default_timeout` (300).
</ParamField>

<ParamField path="metadata" type="std::optional<std::map<std::string, nlohmann::json>>" toc={true}>
