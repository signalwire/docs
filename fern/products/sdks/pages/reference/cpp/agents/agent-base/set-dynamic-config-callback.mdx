...
slug: /reference/cpp/agents/agent-base/set-dynamic-config-callback
...
<ParamField path="callback" type="std::function<void(nlohmann::json, nlohmann::json, nlohmann::json, AgentBase)>" required={true} toc={true}>
...
void configure_per_request(const auto& query_params, const auto& body_params, const auto& headers, const auto& agent) {
    tier = query_params.value("tier", "standard");
    if (tier == "premium") {
}
