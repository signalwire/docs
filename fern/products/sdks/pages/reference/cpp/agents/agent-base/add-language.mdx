---
title: "add_language"
slug: /reference/cpp/agents/agent-base/add-language
...
<ParamField path="code" type="std::string" required={true} toc={true}>
  Language code (e.g., `"en-US"`, `"fr-FR"`, `"es-MX"`).
</ParamField>

<ParamField path="voice" type="std::string" required={true} toc={true}>
...
<ParamField path="speech_fillers" type="std::optional<std::vector<std::string>>" toc={true}>
  Filler phrases used during natural speech pauses (e.g., `["Um...", "Let me see..."]`).
</ParamField>

<ParamField path="function_fillers" type="std::optional<std::vector<std::string>>" toc={true}>
  Filler phrases spoken while executing SWAIG functions
  (e.g., `["One moment please...", "Looking that up..."]`).
</ParamField>

<ParamField path="engine" type="std::optional<std::string>" toc={true}>
  Explicit TTS engine name (e.g., `"elevenlabs"`, `"rime"`). Overrides the
  combined string format if provided.
</ParamField>

<ParamField path="model" type="std::optional<std::string>" toc={true}>
