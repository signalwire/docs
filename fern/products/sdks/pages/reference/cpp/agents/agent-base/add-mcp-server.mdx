...
slug: /reference/cpp/agents/agent-base/add-mcp-server
description: Add an external MCP server for tool discovery and invocation.
max-toc-depth: 3
---

[enable-mcp-server]: /docs/sdks/reference/cpp/agents/agent-base/enable-mcp-server
...
<ParamField path="headers" type="std::optional<std::map<std::string, std::string>>" toc={true}>
...
<ParamField path="resource_vars" type="std::optional<std::map<std::string, std::string>>" toc={true}>
