...
slug: /reference/cpp/agents/context-builder/step/set-gather-info
description: Enable structured info gathering for this step.
max-toc-depth: 3
---

[add-gather-question]: /docs/sdks/reference/cpp/agents/context-builder/step/add-gather-question
...
<ParamField path="output_key" type="std::string" toc={true}>
  Key in `global_data` to store collected answers under. When `std::nullopt`, answers are
  stored at the top level of `global_data`.
</ParamField>

<ParamField path="completion_action" type="std::string" toc={true}>
  ...
  - `std::nullopt` -- return to normal step mode after gathering
</ParamField>

<ParamField path="prompt" type="std::string" toc={true}>
