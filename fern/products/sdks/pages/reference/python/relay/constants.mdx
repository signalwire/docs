...
[`CallStateEvent.end_reason`](/docs/sdks/reference/python/relay/events) when the call
...
[`ConnectEvent.connect_state`](/docs/sdks/reference/python/relay/events).
...
handlers with [`Call.on()`](/docs/sdks/reference/python/relay/call) or when
matching against [`RelayEvent.event_type`](/docs/sdks/reference/python/relay/events).
...
<ParamField path="MESSAGE_TERMINAL_STATES" type="tuple[str, ...]" toc={true}>
  Tuple of terminal states that resolve a [`Message.wait()`](/docs/sdks/reference/python/relay/message) call:
...
`detect` parameter passed to [`call.detect()`](/docs/sdks/reference/python/relay/call/detect).
