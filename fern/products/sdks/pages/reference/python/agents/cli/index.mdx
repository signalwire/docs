...
| Command | Purpose |
|---------|---------|
| [`swaig-test`](/docs/sdks/reference/python/agents/cli/swaig-test) | Test SWAIG functions and generate SWML output locally |
| [`sw-search`](/docs/sdks/reference/python/agents/cli/sw-search) | Build, search, and validate vector search indexes |
| [`sw-agent-init`](/docs/sdks/reference/python/agents/cli/sw-agent-init) | Scaffold new agent projects with templates |
| [`sw-agent-dokku`](/docs/sdks/reference/python/agents/cli/sw-agent-dokku) | Deploy agents to Dokku hosting |
| [`mcp-gateway`](/docs/sdks/reference/python/agents/cli/mcp-gateway) | Bridge MCP servers to SWAIG functions |
...
|-------|-----------------|--------------|
| `search` | `pip install "signalwire-agents[search]"` | `sw-search` build and query |
| `search-full` | `pip install "signalwire-agents[search-full]"` | PDF and DOCX processing |
| `search-nlp` | `pip install "signalwire-agents[search-nlp]"` | Advanced NLP features (spaCy) |
| `search-queryonly` | `pip install "signalwire-agents[search-queryonly]"` | Query-only mode (smaller install) |
| `pgvector` | `pip install "signalwire-agents[pgvector]"` | PostgreSQL vector storage |

<CardGroup cols={2}>
  <Card
    title="swaig-test"
    href="/docs/sdks/reference/python/agents/cli/swaig-test"
  >
    Test SWAIG functions, generate SWML, and simulate serverless environments.
  </Card>
  <Card
    title="sw-search"
    href="/docs/sdks/reference/python/agents/cli/sw-search"
  >
    Build, search, and validate vector search indexes for agent knowledge bases.
  </Card>
  <Card
    title="sw-agent-init"
    href="/docs/sdks/reference/python/agents/cli/sw-agent-init"
  >
    Scaffold new agent projects with configurable templates and platform targets.
  </Card>
  <Card
    title="sw-agent-dokku"
    href="/docs/sdks/reference/python/agents/cli/sw-agent-dokku"
  >
    Deploy and manage agents on Dokku with CI/CD support.
  </Card>
  <Card
    title="mcp-gateway"
    href="/docs/sdks/reference/python/agents/cli/mcp-gateway"
  >
