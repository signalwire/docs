...
<ParamField path="project" type="str" toc={true}>
  SignalWire project ID. Set via constructor or `SIGNALWIRE_PROJECT_ID` env var.
</ParamField>

<ParamField path="token" type="str" toc={true}>
  API token for authentication. Set via constructor or `SIGNALWIRE_API_TOKEN` env var.
</ParamField>

<ParamField path="host" type="str" toc={true}>
  SignalWire space hostname (e.g., `your-space.signalwire.com`). Set via constructor
  or `SIGNALWIRE_SPACE` env var.
</ParamField>
