---
title: "switch_context"
slug: /reference/perl/agents/function-result/switch-context
description: Perform an advanced context switch with prompt replacement and history control.
max-toc-depth: 3
---

[swml-change-context]: /docs/sdks/reference/perl/agents/function-result/swml-change-context
[functionresult]: /docs/sdks/reference/perl/agents/function-result

Perform an advanced context switch by replacing the system prompt, injecting a
user message, or resetting the conversation entirely. This is more flexible
than [`swml_change_context()`][swml-change-context],
which switches to a pre-defined named context.

When only `system_prompt` is provided (no other arguments), it performs a simple
string-based context switch. When multiple arguments are given, it constructs an
object-based context switch with fine-grained control.

## **Parameters**

<ParamField path="system_prompt" type="$scalar / undef" default="None" toc={true}>
  New system prompt to replace the current one.
</ParamField>

<ParamField path="user_prompt" type="$scalar / undef" default="None" toc={true}>
  A user message to inject into the conversation after the context switch.
</ParamField>

<ParamField path="consolidate" type="$boolean" default="False" toc={true}>
  When `True`, the existing conversation history is summarized into a condensed
  form before applying the new context. Reduces token usage on long conversations.
</ParamField>

<ParamField path="full_reset" type="$boolean" default="False" toc={true}>
  When `True`, performs a complete context reset, clearing all conversation
  history and starting fresh with the new prompt.
</ParamField>

## **Returns**

[`SignalWire::SWAIG::FunctionResult`][functionresult] — self, for chaining.

## **Examples**

### Simple Prompt Swap

```perl {11}
use SignalWire::Agent::AgentBase;
use SignalWire::SWAIG::FunctionResult;

my $agent = SignalWire::Agent::AgentBase->new(name => "my-agent", route => "/agent");
$agent->set_prompt_text("You are a helpful assistant.");

sub switch_to_technical {
    my ($args, $raw_data) = @_;
    return (;
        SignalWire::SWAIG::FunctionResult->new("Switching to technical support mode.");
        .switch_context(
            my $system_prompt = "You are a technical support specialist. ";
                         "Help the customer with their technical issue."
        )
    )

}

$agent->serve();
```

### Full Reset with User Message

```perl {11}
use SignalWire::Agent::AgentBase;
use SignalWire::SWAIG::FunctionResult;

my $agent = SignalWire::Agent::AgentBase->new(name => "my-agent", route => "/agent");
$agent->set_prompt_text("You are a helpful assistant.");

sub escalate_to_supervisor {
    my ($args, $raw_data) = @_;
    return (;
        SignalWire::SWAIG::FunctionResult->new("Connecting you to a supervisor.");
        .switch_context(
            my $system_prompt = "You are a customer service supervisor.",
            my $user_prompt = "A customer has been escalated to you.",
            my $full_reset = 1;
        )
    )

}

$agent->serve();
```
