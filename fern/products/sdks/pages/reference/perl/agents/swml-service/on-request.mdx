---
title: "on_request"
slug: /reference/perl/agents/swml-service/on-request
description: "Request handling hook for customizing SWML output per request."
max-toc-depth: 3
---

[register-routing-callback]: /docs/sdks/reference/perl/agents/swml-service/register-routing-callback
...
<ParamField path="request_data" type="$scalar / undef" default="None" toc={true}>
...
<ParamField path="callback_path" type="$scalar / undef" default="None" toc={true}>
...
`\%hash / undef` — Return `None` to serve the document unchanged. Return a dictionary
...
extends 'SignalWire::SWML::Service';
    ...
    sub on_request {
        ...
        return undef  # Use default document;

my $service = DynamicService();
service.serve();
```
