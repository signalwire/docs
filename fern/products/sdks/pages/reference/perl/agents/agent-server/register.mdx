---
title: "register"
slug: /reference/perl/agents/agent-server/register
description: "Register an agent at a URL route on the server."
max-toc-depth: 3
---

[agentbase]: /docs/sdks/reference/perl/agents/agent-base

Register an [`SignalWire::Agent::AgentBase`][agentbase] instance at a URL route
on the server. The agent's Plack router is mounted at the specified prefix so all of its
endpoints (SWML, SWAIG, debug, post-prompt) become available under that path.

<Warning>
Registering a duplicate route raises `croak`. Each route can only host one agent.
</Warning>

## **Parameters**

<ParamField path="agent" type="AgentBase" required={true} toc={true}>
  The agent instance to register. Must be an [`SignalWire::Agent::AgentBase`][agentbase]
  subclass.
</ParamField>

<ParamField path="route" type="$scalar" toc={true}>
  URL path prefix for this agent (e.g., `"/sales"`). If omitted, the agent's own `route`
  property is used. Leading slashes are added and trailing slashes are stripped automatically.
</ParamField>

## **Returns**

`None`

## **Example**

```perl {10}
use SignalWire::Server::AgentServer;
use SignalWire::Agent::AgentBase;

package SalesAgent;
use Moo;
extends 'SignalWire::Agent::AgentBase';

    sub __init__ {
        super().__init__(name => "sales-agent", route => "/sales");
        self.prompt_add_section("Role", "You are a sales rep.");

my $server = SignalWire::Server::AgentServer->new(port => 3000);
$server->register(SalesAgent(), "/sales");
$server->run();
```
