...
slug: /reference/perl/agents/agent-server/sip-routing
...
<ParamField path="route" type="$scalar" default="/sip" toc={true}>
...
<ParamField path="username" type="$scalar" required={true} toc={true}>
...
my $sales = SignalWire::Agent::AgentBase->new(name => "sales", route => "/sales");
...
$server->setup_sip_routing("/sip", auto_map => 1);
