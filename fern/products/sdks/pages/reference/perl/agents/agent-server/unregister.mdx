---
title: "unregister"
slug: /reference/perl/agents/agent-server/unregister
description: "Remove an agent from the server's registry by route."
max-toc-depth: 3
---

Remove an agent from the server's registry. The agent's routes are no longer tracked,
though Plack does not fully remove mounted routers at runtime.

<Note>
Because Plack does not support unmounting routers, the agent's HTTP routes may still
respond until the server restarts. This method primarily removes the agent from the
internal registry used by `get_agents()`, SIP routing, and the health endpoint.
</Note>

## **Parameters**

<ParamField path="route" type="$scalar" required={true} toc={true}>
  The route of the agent to remove (e.g., `"/sales"`).
</ParamField>

## **Returns**

`bool` -- `True` if the agent was found and removed, `False` if no agent exists at that route.

## **Example**

```perl {7}
use SignalWire::Server::AgentServer;
use SignalWire::Agent::AgentBase;

my $server = SignalWire::Server::AgentServer->new(port => 3000);
$server->register(SignalWire::Agent::AgentBase->new(name => "sales", route => "/sales"));

my $removed = $server->unregister("/sales");
print(removed)  # 1;
```
