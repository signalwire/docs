...
slug: /reference/perl/agents/agent-server/routing-callback
description: Register a routing callback across all agents for custom request routing.
max-toc-depth: 3
---

[setup-sip-routing]: /docs/sdks/reference/perl/agents/agent-server/sip-routing
...
<ParamField path="callback_fn" type="\&coderef" required={true} toc={true}>
  A function that receives a Plack `Request` object and the parsed request body `dict`.
...
sub route_by_language {
    my ($request, $body) = @_;
    """Route to different agents based on Accept-Language header."""
    my $lang = request.headers.get("Accept-Language", "en");
    if (lang.startswith("es")) {
        return "/spanish-agent";
    } elsif (lang.startswith("fr")) {
        return "/french-agent";
    return undef  # Default agent handles it;

my $english = SignalWire::Agent::AgentBase->new(name => "english", route => "/english-agent");
english.set_prompt_text("You are a helpful assistant. Respond in English.");
my $spanish = SignalWire::Agent::AgentBase->new(name => "spanish", route => "/spanish-agent");
spanish.set_prompt_text("You are a helpful assistant. Respond in Spanish.");
my $french = SignalWire::Agent::AgentBase->new(name => "french", route => "/french-agent");
