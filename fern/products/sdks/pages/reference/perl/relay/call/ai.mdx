---
title: "ai"
slug: /reference/perl/relay/call/ai
description: Start an AI agent session on a call.
max-toc-depth: 3
---

[aiaction]: /docs/sdks/reference/perl/relay/actions
[agentbase]: /docs/sdks/reference/perl/agents/agent-base
[amazon-bedrock]: /docs/sdks/reference/perl/relay/call/amazon-bedrock
...
use [`SignalWire::Agent::AgentBase`][agentbase]. The `ai()` method
...
<ParamField path="agent" type="$scalar / undef" toc={true}>
...
<Indent>
<ParamField path="prompt.text" type="$scalar" toc={true}>
...
<Indent>
<ParamField path="post_prompt.text" type="$scalar" toc={true}>
...
<ParamField path="post_prompt_auth_user" type="$scalar / undef" toc={true}>
  Username for basic auth on the post-prompt webhook.
</ParamField>

<ParamField path="post_prompt_auth_password" type="$scalar / undef" toc={true}>
...
<ParamField path="pronounce" type="\@arrayref / undef" toc={true}>
  Pronunciation rules for words or phrases the TTS engine should handle
  specially.
</ParamField>

<ParamField path="hints" type="\@arrayref / undef" toc={true}>
  Speech recognition hints to improve accuracy for domain-specific terms.
</ParamField>

<ParamField path="languages" type="\@arrayref / undef" toc={true}>
...
$client->on_call(sub {
    ...
    my $action = $call->ai(
        my $prompt = {"text": "You are a helpful customer support agent for Acme Corp."},
        my $hints = ["Acme", "support", "billing"],
