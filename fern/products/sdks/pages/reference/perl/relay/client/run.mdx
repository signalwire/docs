---
title: "run"
slug: /reference/perl/relay/client/run
...
[ref-relayclient]: /docs/sdks/reference/perl/relay/client

Blocking entry point that connects to RELAY and runs the event loop until
interrupted. This is the recommended way to start a [`SignalWire::Relay::Client`][ref-relayclient] for long-running
...
$client->on_call(sub {
    my ($call) = @_;
    $call->answer();
    my $action = $call->play([{"type": "tts", "text": "Welcome! Goodbye."}]);
    $action->wait();
    $call->hangup();
