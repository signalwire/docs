...
<ParamField name="event_channel" type="string" required>
  ID for the event channel on which these room session's events are reported.
</ParamField>

<ParamField name="timestamp" type="number" required>
  Time of the event.
</ParamField>

<ParamField name="space_id" type="string" required>
...
<Tip>
For real-time call state changes within your SDK application without webhooks, use the [`onStateChanged`](/realtime-sdk/voice/call#onstatechanged) event on the Call object instead.
</Tip>
