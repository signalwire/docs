---
id: e39988a0-9bb4-472b-804f-a43959fa94ed
title: Stop Robocalls with SignalWire's Communication API
slug: guides/stop-robocalls
x-custom:
  ported_from_readme: true
  repo: https://github.com/signalwire/spam-call-filter-example
description: Learn how to stop robocalls with the SignalWire Realtime SDK v2.
max-toc-depth: 3
...
```js
const recordResult = await call.record({
  direction: "both",
  initial_timeout: 10,
...
console.log("Recording the call to " + recordResult.url);
```
...
```js
if (result.digits.trim() == expectedAnswer || result.text.includes(expectedAnswer.toString())) {
  await call.playTTS({
...
<Warning title="Don't use this in production!">
...
await call.prompt({
  type: 'speech',
  speech_end_silence_timeout: 1.0,
  media: [
    {
      type: 'audio',
