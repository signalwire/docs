...
<ParamField path="status_code" type="int | None" toc={true}>
...
<ParamField path="headers" type="dict[str, str] | None" toc={true}>
  Response headers from the API. `None` for a transport failure.
</ParamField>

<ParamField path="request_id" type="str | None" toc={true}>
