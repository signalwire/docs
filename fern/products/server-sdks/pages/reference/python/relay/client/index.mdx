...
<ParamField path="host" type="str" default="relay.signalwire.com" toc={true}>
  ...
  do not set your space here. The constructor argument wins, then the `SIGNALWIRE_SPACE`
  environment variable, then the default. `RestClient` reads `SIGNALWIRE_SPACE` as the
  REST host, so when it is set for REST, pass `host="relay.signalwire.com"` to keep
  the default.
</ParamField>
...
<ParamField path="max_active_calls" type="int | None" default="1000" toc={true}>
