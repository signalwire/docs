...
In practice the call has usually passed `ringing` by the time you hold a reference to it.
[`client.dial()`][dial] returns only after the far end answers, and an inbound call reaches
your `on_call` handler already ringing. In both cases the method returns at once and
`event.params["call_state"]` reports where the call is, so it works as a guard rather than a
wait.
