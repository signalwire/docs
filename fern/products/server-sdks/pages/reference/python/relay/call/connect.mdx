...
<ParamField path="ringback" type="list[dict] | None" default="None" toc={true}>
  Media items to play to the caller while the destination is ringing. Same
  format as [`play()`][play] media items.
</ParamField>

<ParamField path="tag" type="str | None" default="None" toc={true}>
  Correlation tag for the connected call.
</ParamField>

<ParamField path="max_duration" type="int | None" default="None" toc={true}>
  Maximum duration of the connected call in seconds.
</ParamField>

<ParamField path="max_price_per_minute" type="float | None" default="None" toc={true}>
...
async def handle_call(call):
    await call.answer()
    await call.play([{"type": "tts", "params": {"text": "Connecting you now..."}}])
