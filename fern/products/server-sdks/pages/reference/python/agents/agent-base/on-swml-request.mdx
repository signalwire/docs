...
<ParamField path="request_data" type="dict[str, Any] | None" default="None" toc={true}>
  Parsed POST body from the incoming request, if available.
</ParamField>

<ParamField path="callback_path" type="str | None" default="None" toc={true}>
  The path segment that triggered this request.
</ParamField>

<ParamField path="request" type="Request | None" default="None" toc={true}>
...
`dict | None` -- A dictionary of modifications to apply to the SWML document,
