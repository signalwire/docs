...
<ParamField path="event" type="dict[str, Any] | None" default="None" toc={true}>
  Serverless event object. Pass the Lambda event, Cloud Functions request, or
  Azure Functions HttpRequest here.
</ParamField>

<ParamField path="context" type="Any | None" default="None" toc={true}>
  Serverless context object (Lambda context, etc.).
</ParamField>

<ParamField path="force_mode" type="str | None" default="None" toc={true}>
...
<ParamField path="host" type="str | None" default="None" toc={true}>
  Host override for server mode.
</ParamField>

<ParamField path="port" type="int | None" default="None" toc={true}>
  Port override for server mode.
</ParamField>

## **Returns**

`str | dict[str, Any] | None` -- In serverless modes, returns the platform-specific
response object: a `str` in CGI mode, a `dict` otherwise. In server mode, blocks
until shutdown and returns `None`.
