...
`HostAppRouter` -- A FastAPI `APIRouter` subclass with all agent endpoints (SWML
delivery, SWAIG function handling, post-prompt, debug events, etc.). It adds no
behavior; the subclass exists to give the return type a stable name.
