...
<ParamField path="pom" type="PromptObjectModel | list[dict[str, Any]]" required={true} toc={true}>
  POM structure. A list of section dictionaries is converted to a
  `PromptObjectModel` before it is stored; an existing `PromptObjectModel` is
  accepted as-is. Each section dictionary has keys:
