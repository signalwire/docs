...
```python
async say(nonce: str, text: str) -> bool
```
...
`bool` -- `True` when the text was delivered. `False` when `send_message` isn't configured, the
nonce is unknown or has no call ID, the text is empty, the message cap is reached, or the callback
raised.
