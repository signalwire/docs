...
`pay` verb that walks the caller through entering card details by DTMF, then
submits to your payment connector endpoint.
...
<ParamField path="input_method" type="str" default="dtmf" toc={true}>
  How the caller provides card details. The SWML `pay` verb accepts only
  `"dtmf"`: the caller enters digits on the keypad.
</ParamField>
...
<ParamField path="parameters" type="list[dict[str, str]] | None" default="None" toc={true}>
...
<ParamField path="prompts" type="list[dict[str, Any]] | None" default="None" toc={true}>
...
<ParamField path="ai_response" type="str | None" default="'The payment status is ${pay_result}...'" toc={true}>
