...
Only the CLI reads `SIGNALWIRE_SPACE_NAME`. `RestClient` and `RelayClient` read
`SIGNALWIRE_SPACE`, the full hostname `your-space.signalwire.com`, so add it to
the generated `.env` if the agent uses either client.
