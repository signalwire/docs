...
`SIGNALWIRE_SPACE_NAME` configures the CLI and the deployed app's dashboard links.
If the agent also uses `RestClient` or `RelayClient`, set `SIGNALWIRE_SPACE` to the
full hostname, `my-space.signalwire.com`, in the same `config set` call.
