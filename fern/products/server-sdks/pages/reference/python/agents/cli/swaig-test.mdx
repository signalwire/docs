...
<ParamField path="--parse-only, --dry-run" type="flag" toc={true}>
  Validate the invocation's arguments and exit without loading the agent or
  touching the network. Prints `parse OK` on success and exits non-zero on
  invalid arguments. Useful for checking documented commands in CI.
</ParamField>
