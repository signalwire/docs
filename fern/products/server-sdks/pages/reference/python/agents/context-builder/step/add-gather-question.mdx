...
<ParamField path="isolated" type="bool | None" default="None" toc={true}>
  Override the gather's `isolated` default for this one question. `True` hides
  the sibling questions and answers while this question is asked; `False` keeps
  them visible even in an isolated gather. `None` inherits the gather's setting.
</ParamField>
