...
<ParamField path="callback_fn" type="Callable[[dict[str, Any], dict[str, Any]], str | None]" required={true} toc={true}>
  A function that receives the parsed request body and the request headers, both as a
  `dict` — `callback_fn(body, headers)`. Return a route string (e.g., `"/sales"`) to
  redirect, or `None` to let the current agent handle the request.
</ParamField>
