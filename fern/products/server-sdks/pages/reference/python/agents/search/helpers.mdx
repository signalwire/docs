...
<ParamField path="pos_to_expand" type="list[str] | None" default="None" toc={true}>
...
<ParamField path="model_name" type="str | None" default="None" toc={true}>
...
<ParamField path="index_nlp_backend" type="str" default="nltk" toc={true}>
  ...
  - `"nltk"` -- fast, the default.
  - `"spacy"` -- slower; also expands content with WordNet synonyms, and the expansion is written into the index. The effect depends on your corpus, so measure it before choosing. Requires spaCy models.
</ParamField>
