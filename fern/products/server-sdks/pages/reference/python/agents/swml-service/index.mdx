...
<Note>
SWMLService also hosts SWAIG tools directly. It owns a tool registry (via `ToolMixin`)
and serves a `/swaig` endpoint, so SWAIG functions can be registered and dispatched
without subclassing [`AgentBase`][agentbase]. The tool-registration and dispatch methods
(for example `define_tool` and `on_function_call`) are documented under
[`AgentBase`][agentbase], which inherits them.
</Note>
