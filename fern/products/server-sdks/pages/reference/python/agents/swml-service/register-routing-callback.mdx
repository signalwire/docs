...
[ai-sidecar-callbacks]: /docs/swml/reference/calling/ai-sidecar#webhook-callbacks
...
The callback receives the parsed request body and the request headers, both as a
dictionary — `callback_fn(body, headers)`.

<Tip>
This is also how you receive an [`ai_sidecar`](/docs/swml/reference/calling/ai-sidecar) verb's webhook
...
<ParamField path="callback_fn" type="Callable[[dict[str, Any], dict[str, Any]], str | None]" required={true} toc={true}>
  A function that receives the parsed JSON body and the request headers, both as a
  `dict` — `callback_fn(body, headers)`. Return a route string to redirect the request
  (using HTTP 307 to preserve the POST method and body), or return `None` to continue
  with normal SWML document serving.
</ParamField>
...
`str | None` — The extracted username or phone number, or `None` if the `call.to`
