...
Return one summary dictionary per available skill. The lighter counterpart of
[`list_skills_with_params()`](#list_skills_with_params), drawn from the same
skill registry.

#### Returns

`list[dict[str, Any]]` -- One entry per skill with its name, description,
version, required packages and environment variables, and whether it supports
multiple instances.

#### Example

```python
from signalwire import list_skills
