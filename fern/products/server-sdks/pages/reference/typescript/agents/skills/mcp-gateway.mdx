...
<ParamField path="verify_ssl" type="boolean" default="true" toc={true}>
  Whether to verify TLS certificates on outbound requests. Setting this to
  `false` alone is ignored; verification stays on unless `allow_insecure_tls`
  is also `true`.
</ParamField>

<ParamField path="allow_insecure_tls" type="boolean" default="false" toc={true}>
  Explicit opt-in that, together with `verify_ssl: false`, disables TLS
  certificate verification.
</ParamField>

<Warning title="Disabling TLS verification accepts any certificate">
With `verify_ssl: false` and `allow_insecure_tls: true`, outbound requests to
the gateway accept any certificate and are exposed to interception. Use it only
against a trusted self-signed gateway on a private network.
</Warning>
