...
[ref-defineskilltool]: /docs/server-sdks/reference/typescript/agents/helpers#defineskilltool
...
Wrap an entry in [`defineSkillTool()`][ref-defineskilltool] to have the
handler's `args` typed from the `parameters` schema instead of
