...
<ParamField path="toolDef.secure" type="boolean" default="true" toc={true}>
  Whether to require token validation on tool calls. Tools are secure by
  default: the rendered webhook URL carries a per-tool token. Pass `false`
  only to expose the tool on the shared, unauthenticated webhook URL.
</ParamField>
</Indent>
