...
<ParamField path="skillName" type="SkillNameOrString" required={true} toc={true}>
  Registered skill name (read from the target class's `SKILL_NAME`). The
  `SkillName` union lists the 19 built-in names for editor autocompletion; any
  other string resolves against skills you registered yourself.
</ParamField>
