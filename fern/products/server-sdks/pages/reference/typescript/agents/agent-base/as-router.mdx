...
`HostAppRouter` -- The agent's Hono app, with all agent endpoints (SWML
delivery, SWAIG function handling, post-prompt, debug events, health checks,
etc.). `HostAppRouter` is a type alias for `Hono`, named for the host-app
mounting contract it fulfills.
