...
`new RestClient(...)`, provided for parity with the Python SDK's
`RestClient(*args, **kwargs)` call form.

```typescript {1}
restClient(args?: string[], kwargs?: Record<string, unknown>): RestClient
```

<ParamField path="args" type="string[]" toc={true}>
  Positional credentials, a compatibility shim for ports that pass
  project/token/host positionally. Usually empty in TypeScript.
</ParamField>

<ParamField path="kwargs" type="Record<string, unknown>" toc={true}>
  Keyword-style credentials (`project`, `token`, `host`). When omitted, the same
