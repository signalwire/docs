...
The `Calling` resource provides REST-based call control through the
...
Every method takes the call ID first, then any required command fields as
positional arguments, then an optional `options` object for the remaining
fields. The `options` object also accepts `extras`, a bag of additional fields
merged into the command as-is. A final optional
[`requestOptions`][request-options] argument sets the timeout, retries, and
abort signal for that one request.
...
const call = await client.calling.dial("+15551234567", "+15559876543", {
  url: "https://example.com/call-handler",
});

const controlId = "greeting-1";
await client.calling.play(call.id, [{ type: "tts", params: { text: "Hello!" } }], { control_id: controlId });
await client.calling.playStop(call.id, controlId);
await client.calling.end(call.id, { reason: "hangup" });
```
