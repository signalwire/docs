...
Play audio or text-to-speech on an active call. The response confirms that
the command was accepted; playback state arrives asynchronously at
`status_url`.

<Markdown src="/snippets/server-sdks/control-id-note.mdx" />

Keep it to manage the playback later with [`playPause()`][play-pause],
[`playResume()`][play-resume], [`playStop()`][play-stop], and
[`playVolume()`][play-volume].
