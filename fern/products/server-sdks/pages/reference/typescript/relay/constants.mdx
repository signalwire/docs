...
Each state family also has a string-literal union type, the list of its terminal
values, and a predicate, so you can narrow states without comparing against
individual constants.

| Type | Terminal values | Predicate |
|------|-----------------|-----------|
| `CallState` | `CALL_STATE_TERMINAL` = `["ended"]` | `isCallStateTerminal(state)` |
| `DialState` (`dialing`, `answered`, `failed`) | `DIAL_STATE_TERMINAL` = `["answered", "failed"]` | `isDialStateTerminal(state)` |
| `MessageState` | `MESSAGE_STATE_TERMINAL` = `["delivered", "undelivered", "failed"]` | `isMessageStateTerminal(state)` |

Two more closed sets are exported for method options: `TtsGender` (`"male"` or
`"female"`) and `FaxTone` (`"CED"` or `"CNG"`).

```typescript {1,4}
import { isMessageStateTerminal, type MessageState } from '@signalwire/sdk';

function handleMessageState(message: { state: MessageState }) {
  if (isMessageStateTerminal(message.state)) {
