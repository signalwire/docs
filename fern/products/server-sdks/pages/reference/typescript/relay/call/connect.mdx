...
<ParamField path="devices" type={"Record<string, unknown>[][]"} required={true} toc={true}>
  ...
  - `"type"` -- Device type (`"phone"`, `"sip"`, or `"fabric"` for a resource
    address)
  - `"params"` -- Type-specific parameters (`to_number` and `from_number` for
    phone; `to` plus optional `from` and `timeout` for fabric)
</ParamField>
