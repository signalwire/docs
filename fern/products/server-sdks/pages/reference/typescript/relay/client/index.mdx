...
<ParamField path="host" type="string" default="relay.signalwire.com" toc={true}>
  Relay WebSocket endpoint. The default is the endpoint for SignalWire projects; you
  do not set your space here. The constructor option wins, then `SIGNALWIRE_RELAY_HOST`,
  then `SIGNALWIRE_SPACE`, then the default. `RestClient` reads `SIGNALWIRE_SPACE` as
  the REST host, so when it is set for REST, set `SIGNALWIRE_RELAY_HOST` or pass `host`
  to keep the default.
</ParamField>

<ParamField path="contexts" type="string[]" default="[]" toc={true}>
...
<ParamField path="scheme" type="'ws' | 'wss'" default="'wss'" toc={true}>
...
<CardGroup cols={3}>
  ...
  <Card title="onEvent" href="/docs/server-sdks/reference/typescript/relay/client/on-event">
    Observe every inbound Relay event (low-level).
  </Card>
  <Card title="notify" href="/docs/server-sdks/reference/typescript/relay/client/notify">
