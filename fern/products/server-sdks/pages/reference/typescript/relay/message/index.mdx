...
<ParamField path="isTerminal" type="boolean" toc={true}>
  `true` when `state` is `delivered`, `undelivered`, or `failed`. Unlike
  `isDone`, this tests the state value alone, so it is meaningful on an inbound
  message before any event has been dispatched.
</ParamField>
