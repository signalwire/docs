...
func init() {
    ...
    a.DefineTool("say_hello", "Say hello to a user", agent.ToolHandler(
        func(args map[string]interface{}) *agent.FunctionResult {
            name, _ := args["name"].(string)
            if name == "" {
                name = "World"
            }
            return agent.NewFunctionResult("Hello " + name + "!")
        },
    ))
}
...
import com.signalwire.agents.agent.FunctionResult;
...
```mermaid
flowchart TD
    cloud["SignalWire"] --> gateway["API Gateway<br/>HTTPS endpoint"]
    gateway --> lambda["Lambda function<br/>agent.run(event, context)"]
    lambda -->|path /| swml["Return SWML document"]
    lambda -->|path /swaig| function["Execute SWAIG function"]
    swml & function --> response["Return JSON response to SignalWire"]
```
