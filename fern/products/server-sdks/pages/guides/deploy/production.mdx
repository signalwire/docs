...
```mermaid
flowchart TD
    internet["Internet"] --> loadBalancer["Optional load balancer<br/>for high availability"]
    loadBalancer --> nginx["nginx reverse proxy<br/>SSL termination and rate limiting"]
    nginx --> workers["Uvicorn and agent<br/>multiple workers"]
    workers --> apis["External APIs, databases, and services"]
```
