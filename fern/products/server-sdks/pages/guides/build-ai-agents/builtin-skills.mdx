...
- `get_joke` - Get a random joke. Takes a required `type` argument, `jokes` or `dadjokes`
...
| Parameter | Type | Description | Default |
|-----------|------|-------------|---------|
| `tool_name` | string | Custom function name | "get_joke" |
...
Preload named files via `files`; the caller selects one by `key`:
...
| `token` | string | API token (falls back to `SIGNALWIRE_API_TOKEN`) | Required |
