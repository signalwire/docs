...
| PHP | `(new FunctionResult('Transferring...'))->connect('+15551234567')` | `$result->swmlTransfer(dest: 'https://...', final: true)` |
...
class TransferAgent(AgentBase):
    ...
    def transfer_to_sales(self, args, raw_data):
        return (
            FunctionResult("Transferring you to sales now.")
...
class SmartTransferAgent(AgentBase):
    ...
    def transfer_permanent(self, args, raw_data):
        ...
        return (
            FunctionResult(f"Transferring you now. Goodbye!")
    ...
    def transfer_temporary(self, args, raw_data):
        number = args.get("number")
        return (
            FunctionResult("Connecting you briefly. I'll be here when you're done.")
...
def transfer_to_sip(self, args, raw_data):
    return (
        FunctionResult("Connecting to internal support")
...
def transfer_with_custom_callerid(self, args, raw_data):
    return (
        FunctionResult("Connecting you now")
        .connect(
            "+15551234567",
            final=True,
...
```mermaid
flowchart LR
    caller1["Caller"] --> agent1["Agent"] --> message1["Transferring..."] --> destination1["Destination"]
    message1 -->|final=true| exit["Agent exits"]
    caller2["Caller"] --> agent2["Agent"] --> message2["Connecting..."] --> destination2["Destination"]
    destination2 -->|hangs up; final=false| return["Return to agent"]
    return --> continue["Agent continues conversation"]
```
...
class ReceptionistAgent(AgentBase):
    ...
    def transfer_to_department(self, args, raw_data):
        ...
        return (
            FunctionResult(f"Transferring you to {dept_name} now. Have a great day!")
            .connect(number, final=True)
        )
...
func main() {
    ...
    a.DefineTool("transfer_to_department", "Transfer caller to a department",
        ...
        func(args map[string]any, rawData map[string]any) *agents.FunctionResult {
            ...
            if !ok {
                return agents.NewFunctionResult("Unknown department.")
            }
            return agents.NewFunctionResult("Transferring you now.").
                Connect(number, true)
        })
...
agent.define_tool(
  ...
  unless number
    next FunctionResult.new("Unknown department '#{dept}'.")
  end
  FunctionResult.new("Transferring you to #{dept} now.")
    .connect(number, final: true)
end
    ...
    public static void main(String[] args) {
        ...
        agent.defineTool("transfer_to_department", "Transfer caller to a department",
            ...
            (fnArgs, rawData) -> {
                ...
                if (number == null) return new FunctionResult("Unknown department.");
...
$agent->define_tool(
    ...
    handler     => sub {
        ...
        my $number = $departments{$dept} or
            return SignalWire::Agents::FunctionResult->new("Unknown department.");
    ...
    agent.define_tool("transfer_to_department", "Transfer caller to a department",
        ...
        [](auto args, auto raw_data) {
            ...
            if (it == departments.end())
                return FunctionResult("Unknown department.");
            return FunctionResult("Transferring you now.")
...
def transfer_with_sms(self, args, raw_data):
    ...
    return (
        FunctionResult("I'm transferring you and sending a confirmation text.")
        .send_sms(
...
def cold_transfer(self, args, raw_data):
    return (
        FunctionResult("Transferring you to support now.")
        .connect("+15551234567", final=True)
    )
```
...
def transfer_to_department(self, args, raw_data):
    ...
    return (
        FunctionResult(f"Transferring to {dept}.")
...
def route_to_queue(self, args, raw_data):
    ...
    return (
        FunctionResult(f"Routing you to our {issue_type} team.")
