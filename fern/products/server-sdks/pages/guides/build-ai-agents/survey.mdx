...
```mermaid
flowchart TD
    intro["Introduction"] --> ask["Ask question"]
    ask --> response["Get response"]
    response --> valid{"Valid?"}
    valid -->|yes| log["Log response"]
    valid -->|no| retry["Retry up to max_retries"]
    retry -->|valid| log
    retry -->|still invalid| fallback["Skip or ask again"]
    log & fallback --> more{"More questions?"}
    more -->|yes| ask
    more -->|no| conclusion["Conclusion"]
    conclusion --> summary["Generate summary"]
```
