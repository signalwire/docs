---
title: "Architecture"
description: Understand the fundamental architecture, protocols, and patterns that power the SignalWire Server SDK.
...
```mermaid
flowchart LR
    agent["AgentBase"] --> auth["AuthMixin<br/>basic auth, credentials, validation"]
    auth --> web["WebMixin<br/>FastAPI, routes, server"]
    web --> service["SWMLService<br/>schema, rendering, verbs"]
    agent --> prompt["PromptMixin<br/>POM, sections, templates"]
    prompt --> tool["ToolMixin<br/>SWAIG functions, decorators, DataMap"]
    tool --> skill["SkillMixin<br/>management, registry, loading"]
    agent --> config["AIConfigMixin<br/>languages, hints, parameters"]
    config --> serverless["ServerlessMixin<br/>Lambda, CGI, Azure"]
    serverless --> state["StateMixin<br/>session, call state, persistence"]
```
