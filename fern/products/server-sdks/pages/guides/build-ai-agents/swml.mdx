...
```mermaid
flowchart TD
    request["POST / arrives"] --> render["_render_swml()"]
    render --> prompt["Get prompt: text or POM"]
    render --> postPrompt["Get post-prompt"]
    render --> functions["Collect SWAIG functions"]
    render --> tokens["Generate security tokens"]
    render --> urls["Build webhook URLs"]
    render --> params["Collect hints, languages, and parameters"]
    prompt & postPrompt & functions & tokens & urls & params --> ai["Assemble AI verb"]
    ai --> document["Build document: answer and AI verbs"]
    document --> response["Return SWML JSON"]
```
