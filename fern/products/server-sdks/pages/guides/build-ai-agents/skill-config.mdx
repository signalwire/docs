...
```mermaid
flowchart TD
    add["add_skill() called with parameters"] --> instantiate["Instantiate skill<br/>store self.params and extract swaig_fields"]
    instantiate --> setup["Run setup()<br/>validate packages, environment, and custom rules"]
    setup -->|success| register["Call register_tools()"]
    setup -->|failure| error["Raise ValueError"]
```
