...
The SignalWire Server SDK implements multiple security layers:
...
| Go | `agent.ToolDefinition{ ..., Secure: true }` |
| Ruby | `agent.define_tool(..., secure: true)` |
| Java | `toolDef.setSecure(true)` |
| Perl | `$agent->define_tool(..., secure => 1)` |
| C++ | `ToolDefinition{ ..., .secure = true }` |
| PHP | `$agent->defineTool(..., secure: true)` |
...
You now understand the core concepts of the SignalWire Server SDK. Let's move on to [building agents][building-agents].
