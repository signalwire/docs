...
State in the Server SDK is **session-scoped** -- it exists only for the duration of a single call. When the call ends, all state is cleared. This is by design: each call is independent, and there's no built-in mechanism for persisting state between calls.
...
```mermaid
flowchart TD
    start["Call starts"] --> initial["set_global_data()<br/>initialize state from agent configuration"]
    initial --> conversation["Conversation<br/>prompts substitute global_data keys"]
    conversation --> function["Function call<br/>handler receives raw_data and call information"]
    function --> result["SwaigFunctionResult<br/>update_global_data() updates session state;<br/>set_metadata() updates function-scoped data"]
    result --> ending["Call ends"]
    ending --> post["Post-prompt extracts structured conversation data"]
```
...
| PHP | `(new FunctionResult('Done'))->updateGlobalData(['key' => 'val'])` |
...
func main() {
    ...
    a.DefineTool("add_item", "Add an item to the order",
        ...
        func(args map[string]any, rawData map[string]any) *agents.FunctionResult {
            item := args["item"].(string)
            price := args["price"].(float64)
            return agents.NewFunctionResult(fmt.Sprintf("Added %s ($%.2f) to your order", item, price)).
...
agent.define_tool(
  ...
  FunctionResult.new("Added #{args['item']} ($#{args['price']}) to your order")
    ...
    public static void main(String[] args) {
        ...
        agent.defineTool("add_item", "Add an item to the order",
            ...
            (fnArgs, rawData) -> {
                ...
                return new FunctionResult("Added " + item + " to your order")
...
$agent->define_tool(
    ...
    handler     => sub {
        my ($args, $raw_data) = @_;
        return SignalWire::Agents::FunctionResult
...
int main() {
    ...
    agent.define_tool("add_item", "Add an item to the order",
        ...
        [](auto args, auto raw_data) {
            ...
            return FunctionResult("Added " + item + " to your order")
...
$agent->defineTool('add_item', 'Add an item to the order', [
    ...
    return (new FunctionResult("Added {$args['item']} to your order"))
