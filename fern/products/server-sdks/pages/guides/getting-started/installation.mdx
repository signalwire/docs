...
| TypeScript | Node.js 22+ | npm | Linux, macOS, Windows |
{/*
...
<Note>
`@signalwire/sdk` is published as an ES module only (no CommonJS build). Your project must use ESM — add `"type": "module"` to your `package.json`, use `.mjs` file extensions, or use a bundler/transpiler that supports ESM. Import it with `import` syntax.
</Note>
</Tab>
</Tabs>
{/*
...
<Tab title="Ruby">
```bash
ruby -e "require 'signalwire'; puts 'SDK installed successfully!'"
```
</Tab>
<Tab title="Java">
```bash
...
```python
#!/usr/bin/env python3
"""Verify SignalWire Server SDK installation."""

def main():
    print("Checking SignalWire Server SDK installation...\n")
...
```text
Checking SignalWire Server SDK installation...
