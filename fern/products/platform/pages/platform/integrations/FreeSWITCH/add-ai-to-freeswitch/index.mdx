---
id: 0550c031-8350-4bdd-8505-8800b3e5bcd4
author: Len Graham
...
slug: freeswitch/add-ai-to-freeswitch
x-custom:
  tags:
    - product:freeswitch
    - favorite
...
![The Domain Applications page of the SignalWire dashboard with "Handle Incoming Calls" section selecting AI Agent](/assets/images/dashboard/sip/sip-domain-apps-ai-agent.webp)
...
In the Dashboard, open **SIP** > **Domain Apps**, edit the domain application, set **Handle Using** to **an AI Agent**, and select the agent under **When a Call Comes In**.
...
<extension name="testing_ai" continue="false" uuid="6d0ecaef-9049-4920-8d1a-fc97ff4731f0">
	<condition field="destination_number" expression="^55555$">
		...
		<action application="bridge" data="sofia/external/FreeSWITCH_AI@example-space-freeswitch-ai.dapp.signalwire.com;transport=tcp"/>
	</condition>
</extension>
```

`FreeSWITCH_AI@example-space-freeswitch-ai.dapp.signalwire.com` is a placeholder — replace it with your own domain
application's SIP address before you use the dialplan. The host is the domain you chose when you created the domain
app, in the form `<your-domain>.dapp.signalwire.com`, and it is shown on the `Domain Apps` page in your Space.
...
Bridges the call to a SignalWire SIP URI using the TCP transport. This effectively forwards the call to the specified
SIP address. `example-space-freeswitch-ai.dapp.signalwire.com` is a placeholder for your own domain app's host.
