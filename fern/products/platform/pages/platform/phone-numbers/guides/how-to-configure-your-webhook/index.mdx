...
SignalWire APIs allow you to easily send/receive HTTP requests from/to the SignalWire servers. For example, you can easily make an HTTP request to SignalWire in order to [Create a Call](/rest/signalwire-rest/endpoints/calling/call-commands) or [Send a Message](/rest/signalwire-rest/endpoints/messaging/send-message). However, if you are using SignalWire to handle inbound calls or inbound messages, you will need to use a webhook!
...
When one of your SignalWire phone numbers receives a call or is used to place an outgoing call, you can have asynchronous HTTP requests sent to your server that tell you about the status changes and the call details. You can do this by setting the `StatusCallback` parameter when [placing an outbound call](/rest/signalwire-rest/endpoints/calling/call-commands) through the API.
...
You can do this by setting the `StatusCallback` parameter when [sending an outbound message](/rest/signalwire-rest/endpoints/messaging/send-message) through the API.
...
For detailed instructions on how to configure your webhook to a SignalWire number, see the [Making and Receiving Phone Calls](/voice/getting-started/making-and-receiving-phone-calls) guide.
...
Select **SWML Script** for **When a Call Comes In** to use a SWML script. If you are using the [RELAY Realtime API](/sdks/realtime-sdk), select **RELAY** for **When a Call Comes In**. The other options are all third-party integrations you can choose to use for different use cases!
...
```js
import { RestClient } from "@signalwire/realtime-api";
