...
SignalWire supports several Resource types. Some of the most common are:
...
Every Project starts with a `public` and a `private` context, you can add named contexts from the Dashboard, and the name defaults to the Resource's name in lowercase. Addresses are mutable, and a single Resource can have multiple addresses, including phone numbers and SIP URIs.
