---
id: 5da1e93c-7fa0-4996-aee2-f90e0386e80c
...
Next, we need to turn this local webhook into a publicly exposed address so that it can receive HTTP requests and be interpreted by SignalWire. 
...
If you prefer using SWML scripts that are hosted on SignalWire servers and manually setting your phone number settings to associate your webhooks, then no worries. You can read more about the process [HERE](/platform/phone-numbers/guides/how-to-configure-your-webhook) 
...
You must have the SignalWire Python SDK installed. You can install it using pip.  
...
![a text conversation between the user and the application](/assets/images/project/python-pyngrok-create-publicly-exposed-webhook/join_the_sith.webp)
...
Lastly, you will need to tell SignalWire where to find your application. This can be done through the [Phone Number Settings API](/rest/signalwire-rest/endpoints/space/update-phone-number) and setting the appropriate parameter. In our example, we set the Messaging Webhook for our number so we updated the webhook URL parameter with our public URL.
