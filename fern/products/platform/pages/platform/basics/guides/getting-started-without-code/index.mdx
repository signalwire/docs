---
id: f76e1005-1887-46e6-ba07-d33aaa3bf998
...
[Purchasing a Phone Number via REST API](/rest/signalwire-rest/endpoints/space/purchase-phone-number), 
...
![The phone number purchase page](/assets/images/dashboard/phone-numbers/choose.webp)
...
<Frame caption="The Call Flow Builder page" ogImage>

![Image of SignalWire Call Flow Builder, show-casing a series of nodes being connected visually to make a call-flow.](/assets/images/call-flow/sample-flow.webp)
...
You can write SWML scripts manually to handle calls, messages, and faxes in your SWML Script list page.
You will find this page in the Resources section of your SignalWire Space.
...
![The phone number settings page](/assets/images/dashboard/phone-numbers/assign-resource-voice.webp)
...
In your SignalWire Dashboard in the Phone Numbers settings page, select 'a Call Flow' under the 'Handle Calls using' section.

![legacy settings for phone number](/assets/images/dashboard/legacy/external-cxml-webhook.webp)
...
You can view all the traffic directed through your SWML scripts in the Logs page of the SignalWire Space. 

<Frame caption="The Logs page of the SignalWire Space">

![The Logs page of the SignalWire Space](/assets/images/dashboard/logs.webp)
...
<Frame caption="Embedded Video Room">

![A Image showing the embeddable video for web-pages](/assets/images/video/embed-pvc.webp)
...
You can find the AI Agent Space in your Dashboard, on the left sidebar under `AI Agent`, or under `Resources`.
...
<Frame caption="SignalWire Dashboard that shows the sidebar option 'AI Agent' selected, and is displaying the AI Agent page">

![SignalWire Dashboard that shows the sidebar option 'AI Agent' selected, and is displaying the AI Agent page](/assets/images/dashboard/resources/ai-agent/ai-agent-resource-page.webp)
...
This will open a new page where you can select `Edit Settings`. There you can select a new AI Agent resource for the incoming call.
...
Ensure that the phone number has been assigned "an AI Agent" under "Inbound Call Settings". 

<Markdown src="/snippets/common/dashboard/_ui-accordion.mdx" />

<Accordion title="For Legacy UI">

For Legacy UI, in the phone number settings, select "an AI Agent" or "a Call Flow" under "Handle Calls Using" in the Phone Number settings page.

![SignalWire Dashboard that shows the sidebar option 'Phone Numbers' selected](/assets/images/dashboard/legacy/external-cxml-webhook.webp)
...
The SIP endpoints can be found in the Sidebar either under "My Resources". If you are still on the Legacy UI, the SIP entry should be directly in the left sidebar.

SIP endpoints are the hub for inbound and outbound calls.
Many resources, from SWML scripts to phone numbers, can point to the same SIP endpoint.
To create an endpoint, start by clicking the "Create a SIP Endpoint" button, configure your settings, and click "Save."

<Frame caption="The SIP settings page">

![The SIP endpoint settings with all default options selected.](/assets/images/dashboard/legacy/sip/edit-endpoint.webp)
...
To simply redirect incoming calls from a number to the endpoint, go to that number's settings and change "HANDLE CALLS USING" to 
"a SIP Credential" and specify which credential you would like to use.

You can also direct calls to your SIP endpoint using SWML scripts.
...
sections:
  main:
    - play:
        url: 'say:Hello. Thank you for your call. Our normal hours of operation are 10am to 6pm Monday through Friday. Our address is 1234 Main street in City. Please stay on the line to speak with an agent.'
    - connect:
        to: 'sip:SWHub@example.com'
```

See the [SWML connect method](/swml/methods/connect) reference for many more options to make your call handling as robust as you need it to be.

As for call management on your end, you can connect your SIP endpoint to any SIP-compatible softphone.
See [Using a softphone with SignalWire](/platform/basics/general/what-is-sip#softphone-configuration) for the required configuration settings.
...
[Auto Top Up & Card Rejection Errors](/platform/dashboard/billing#auto-top-up).
