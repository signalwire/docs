...
| Username | The username of your SIP endpoint |
| Password | The password set when creating your SIP endpoint |
| Transport | The signaling transport protocol: TLS, TCP, or UDP. Must match the transport configured on your SIP endpoint. |
| Encryption type | Controls media encryption (SRTP). Must match your endpoint setting: `required`, `optional`, or `disabled`. |

<Note>
The **Domain**, **Username**, and **Password** are all found on the SIP endpoint page
of your [SignalWire Dashboard](https://my.signalwire.com). Select your endpoint to view its credentials.
</Note>
