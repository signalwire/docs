...
slug: /platform/basics/general/how-to-set-caller-id-or-cnam
x-custom:
...
![A screenshot of the New Sip Endpoint page, showing username, password, caller ID, and Send As fields](/assets/images/dashboard/legacy/sip/new-endpoint.webp)
...
Verified caller ID lets you use phone numbers you already own (such as a wireless or landline number) as the outbound caller ID for SignalWire calls. These numbers are **not** ported to SignalWire — your original provider continues to service them. SignalWire does not charge for verified numbers themselves, but standard usage charges apply.

<Warning>
Verified numbers can **only** be used as outbound caller ID. They cannot receive inbound calls through SignalWire.
</Warning>
...
<Steps>
  <Step title="Open the Phone Numbers page">
    In the SignalWire Dashboard, click **Phone Numbers** in the sidebar.
  </Step>
  <Step title="Go to the Verified tab">
    Click the **Verified** tab, then click **Verify a Phone Number**.
  </Step>
  <Step title="Complete verification">
    Enter the phone number and click **Call Me**. Enter the verification code you receive, then click **Verify**.
  </Step>
</Steps>

Once verified, the number appears in the Verified tab and can be selected as the **Send As** caller ID on any SIP endpoint or used in API calls.
