...
description: Guidelines for building HIPAA-compliant communications and voice AI applications on SignalWire with SWML and the Server SDK.
max-toc-depth: 3
---
...
Track the caller's role in `global_data` after verification, and gate PHI functions accordingly. A SWAIG function can enable or disable other functions at runtime with `toggle_functions`, and you should independently re-check authorization before returning any PHI:
...
@agent.tool(name="grant_provider_access", description="Enable clinical tools for a verified provider", secure=True)
def grant_provider_access(args, raw_data=None):
    # Re-check authorization before enabling any PHI functions
    return (
        FunctionResult("You're verified as a provider. I can now access clinical records.")
        .update_global_data({"role": "provider"})
        .toggle_functions([
            {"function": "view_medical_record", "active": True},
            {"function": "add_clinical_note", "active": True},
        ])
...
agent.defineTool({
  name: 'grant_provider_access',
  description: 'Enable clinical tools for a verified provider',
  secure: true,
  // Re-check authorization before enabling any PHI functions
  handler: async () =>
    new FunctionResult("You're verified as a provider. I can now access clinical records.")
      .updateGlobalData({ role: 'provider' })
      .toggleFunctions([
        { function: 'view_medical_record', active: true },
        { function: 'add_clinical_note', active: true },
      ]),
...
def access_phi(args, raw_data=None):
    if (raw_data or {}).get("global_data", {}).get("emergency_mode"):
        # Refuse PHI access and disable the function during an incident
        return (
            FunctionResult(
...
agent.defineTool({
  ...
  secure: true,
  handler: async (args, rawData) => {
    if (rawData?.global_data?.emergency_mode) {
      // Refuse PHI access and disable the function during an incident
      return new FunctionResult(
        'This service is temporarily unavailable. Please call our main line for assistance.'
      ).toggleFunctions([{ function: 'access_phi', active: false }]);
    }
    // ... normal PHI access
