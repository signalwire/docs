...
HIPAA requires audit trails for PHI access. Write the audit entry in whichever handler backs your function — the tool handler (Server SDK) or your SWAIG `web_hook_url` handler (SWML) — and redact sensitive fields so that PHI never appears in plaintext logs. To mask PHI from the AI's own logs, events, and call log, see [`redact_prompt`](/docs/swml/reference/calling/ai/params#paramsredact_prompt) and the [content redaction guide](/docs/platform/ai/content-redaction).
...
One of the most effective ways to protect PHI is to keep it away from the LLM entirely. Use `global_data` or `meta_data` to store sensitive information while keeping it out of the model's context. To mask PHI from the text records — logs, events, webhooks, and the call log, though not the audio recording — see the [content redaction guide](/docs/platform/ai/content-redaction).
...
- [Content redaction guide](/docs/platform/ai/content-redaction)
