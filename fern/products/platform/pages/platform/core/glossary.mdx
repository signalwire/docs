...
With an API token, the [Projects API][projects-api] creates and deletes Subprojects beneath that token's Project. With a [personal access token](#personal-access-token) it manages root Projects as well. A Project must release its phone numbers and hold no Subprojects before it can be deleted, and the last Project in a Space cannot be deleted.
...
A user-level Dashboard token, issued from the user menu rather than from a Project's API Credentials page. It authenticates you as a person rather than a Project, so it works where the caller is a user: the Projects and API token endpoints of the REST API, the Space-level API, and FreeSWITCH and FreeSWITCH Advantage binary downloads. It is not interchangeable with an API token.
...
| [Personal access token](#personal-access-token) | Per user, not per Project. Authenticates user-level REST calls and FreeSWITCH downloads. |
...
A container object representing one communication entity. Every Resource has a **Type** and one or more [addresses][addresses] that make it reachable. The REST API defines fourteen types and returns each Resource's in its `type` field: Subscriber, AI Agent, SWML Script, SWML Webhook, cXML Script, cXML Application, cXML Webhook, Call Flow, Relay Application, FreeSWITCH Connector, SIP Credential, whose `type` value is `sip_endpoint`, SIP Gateway, Dialogflow Agent, and Video Room. The Dashboard also offers Remote AI Agent, which has no REST API type. All types share the same addressing and management model.
