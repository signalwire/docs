...
[conference-room-ref]: /docs/apis/rest/conference-rooms/create-conference-room
...
[space-api]: /docs/apis/rest/space/get-space
...
This glossary defines each SignalWire term, with the aliases and historical names it answers to and the similar terms it should not be confused with. An alias is listed only where it is still live in the product, such as a Dashboard label, an API path or field, an SDK parameter, or a package name, and each entry says where.
...
Subprojects can be created and deleted through the [Projects API][projects-api] with a Project's API token; creating or deleting a root Project through the API takes a [Personal access token](#personal-access-token). A Project must release its phone numbers and remove its Subprojects before it can be deleted, and the last Project in a Space cannot be deleted.
...
A Project-scoped credential used server-side, and the password half of HTTP Basic [authentication][authorization]. A Project can hold several tokens, each with its own scopes, and you create and revoke them from the Dashboard or through the REST API.

An API token does not belong in a browser; client-side credentials are created by calling an API endpoint with it.

### Personal access token [#personal-access-token]

A user-level Dashboard token, issued from the user menu rather than from a Project's API Credentials page. It authenticates you as a person rather than a Project: it is prefixed `pat_`, has no scopes, and carries your owner or admin role in the Space. It is the credential for the [Space Administration API][space-api], and on the Projects and Project Tokens APIs it unlocks what a Project's API token can't do, such as creating or deleting a root Project. It also authorizes you to download FreeSWITCH and FreeSWITCH Advantage binaries. It is not interchangeable with an API token; see [Authorization][authorization] for how to send one.
...
| [Personal access token](#personal-access-token) | Per user, not per Project. Administers the Space over the API and downloads FreeSWITCH binaries. |

## Resources and Addresses [#resources-and-addresses]

SignalWire's addressing model. Every communication entity — a user, a script, an AI agent, a SIP endpoint, a video room — is a [Resource][resources] reachable at an Address, so one dialing model reaches all of them and all of them are managed the same way. The Dashboard and the `/api/fabric/` REST paths still use the name **Call Fabric** for this model.

### Resource [#resource]

A container object representing one communication entity. Every Resource has a **Type** and one or more [Addresses][addresses] that make it reachable. The REST API defines fourteen types and reports each Resource's in the `type` field of its response: Subscriber, AI Agent, SWML Script, SWML Webhook, cXML Script, cXML Application, cXML Webhook, Call Flow, Relay Application, FreeSWITCH Connector, SIP Credential, whose `type` value is `sip_endpoint`, SIP Gateway, Dialogflow Agent, and Video Room. The Dashboard also offers Remote AI Agent, which has no REST API type. All types share the same addressing and management model.
...
Three variants exist for different levels of trust: the default SAT for a signed-in Subscriber, a Guest SAT that your backend creates for a visitor and limits to a list of Addresses, and an Invite SAT that a signed-in Subscriber creates for one recipient and one Address. A separate Embed token serves public widgets.

### Relay Application [#relay-application]

A Resource type representing your own server connected over [Relay](#relay). It carries a **topic**, and calls addressed to the Resource are sent to whichever connected Relay client is subscribed to that topic. The Dashboard labels the field **Topic**. In the Server SDKs, `RelayClient` subscribes to it through its `contexts` parameter.
...
A Resource type holding a room that several participants join at once, by audio or video, so an inbound call lands in a shared conversation instead of a one-to-one leg. A WhatsApp caller joins by audio only. Also written **Conference Room** in the [REST API reference][conference-room-ref] and the Dashboard Address picker; the Dashboard's Resource list and the platform docs say Video Room.
...
In SWML each leg has its own `call` object with its own [Call ID](#call-id), and connecting to a new leg re-initializes that object with the new leg's data.

### Call ID [#call-id]

The UUID of one [leg](#call-leg), fixed from the moment the leg starts until it hangs up. In Calling SWML it is `call.call_id`. In the Relay namespace of the Server SDKs it is `call.id`. SWML status callbacks and AI webhooks send it as `call_id`. Connecting to a new leg gives that leg its own Call ID, which the first leg's webhooks report as `peer.call_id`.

### Segment ID [#segment-id]

The UUID of a leg's log record, and the identifier to quote in a support ticket. A leg's first segment reuses its [Call ID](#call-id), so for an ordinary call the two are the same value. If SignalWire re-routes the leg to a new handler, such as a SIP REFER or a cXML transfer, a new segment begins on the same leg: the Segment ID changes and the Call ID does not. Messages and faxes have Segment IDs too.

The Dashboard labels it **Segment ID** in every log's detail view, and the [Segment IDs page][segment-ids] shows where. The REST APIs return it as `id`, and Calling SWML exposes it as `call.segment_id`. The Compatibility API writes it **SID**, as in `CallSid`, and its Dashboard log pages use the same abbreviation.

The identifiers at a glance:

| Identifier | Identifies | Where you see it |
|---|---|---|
| [Call ID](#call-id) | One leg, for its whole life | `call_id` in SWML, status callbacks, and AI webhooks; `call.id` in the Relay namespace of the Server SDKs |
| [Segment ID](#segment-id) | One leg's log record; a new one after a re-route | Dashboard logs; `id` in REST responses; `segment_id` in SWML and status callbacks |
| `CallSid` | The leg's Segment ID, under the Compatibility API's name | Compatibility REST responses and every cXML request and status callback |
| `ParentCallSid` | The Segment ID of the leg that created this one | Compatibility status callbacks; `parent_id` in the REST voice logs |
| `AccountSid` | Your [Project ID](#project-id) | Compatibility API |
| Message ID | One message's log record | `id` in REST responses; `message_id` in Relay events; `MessageSid` in Compatibility webhooks; **Message Segment ID** in the Dashboard |
| `FaxSid` | One fax's log record | Compatibility API; **Fax Segment ID** in the Dashboard. `MediaSid` is the stored document, a different ID |
| `ai_session_id` | One AI session on a leg | AI webhooks |

The `conversation_id` in AI webhooks is the value you set in `ai.params`, not a platform identifier.
...
The [document type][swml-messaging] for inbound SMS and MMS. A document can run up to 100 method steps and 10 `request` calls per inbound message, and execution stops at either ceiling.
...
**Realtime SDK** and **Relay Realtime SDK** are the names of the Node.js package `@signalwire/realtime-api`, the earlier Relay client for JavaScript. Relay in either name means this namespace.
...
**LaML** is the former name of cXML and of the Compatibility API. It is retired as a product name but still live as an identifier, so you will keep meeting it: in `/api/laml/` URL paths, in the `RestClient.LaML` SDK namespace, in the LAML Bins resource of the Compatibility API — not a [Resource](#resource) — and on Dashboard labels such as **cXML/LaML**. When you see LaML in a name or a URL, read cXML.
