...
**LaML** is the former name of cXML and of the Compatibility API. It is retired as a product name but still live as an identifier, so you will keep meeting it: in `/api/laml/` URL paths, in the `RestClient.LaML` SDK namespace, in the LAML Bins resource type, and on Dashboard labels such as **cXML/LaML**. When you see LaML in a name or a URL, read cXML.
