...
SignalWire's addressing model. Every communication entity — a user, a script, an AI agent, a SIP endpoint, a video room — is a [Resource][resources] reachable at an address, so one dialing model reaches all of them and all of them are managed the same way. Older pages call this model **Call Fabric**; that name is retired.
...
A Resource type representing your own server connected over [Relay](#relay). It carries a **topic**, and calls addressed to the Resource are sent to whichever connected Relay client is subscribed to that topic. Older SDK pages call the same field a context.
...
In SWML each leg has its own `call` object with its own `call_id`, and connecting to a new leg re-initializes that object with the new leg's data. The leg's [Segment ID](#segment-id) is `call.segment_id`, and it matches the leg's `call_id`.

### Segment ID [#segment-id]

A UUID identifying one [leg](#call-leg) of a call — or one message or fax — in the logs, written **SID**. A call with several legs produces several segments, and each leg keeps its SID for its whole lifecycle. In Calling SWML it is `call.segment_id`, and it matches the leg's `call_id`.
