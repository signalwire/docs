...
SignalWire APIs allow you to easily send/receive HTTP requests from/to the SignalWire servers. For example, you can easily make an HTTP request to SignalWire in order to [Create a Call](/apis/calling/calls/call-commands) or [Send a Message](/compatibility-api/rest/messages/create-message). However, if you are using SignalWire to handle inbound calls or inbound messages, you will need to use a webhook!
...
To learn how to use ngrok for local testing, check out our [guide](/platform/basics/how-to-test-with-ngrok)!
...
When one of your SignalWire phone numbers receives a call or is used to place an outgoing call, you can have asynchronous HTTP requests sent to your server that tell you about the status changes and the call details. You can do this by setting the `StatusCallback` parameter when [placing an outbound call](/apis/calling/calls/call-commands) through the API.
...
Read in-depth about voice status callbacks [here](/platform/phone-numbers/guides/signalwire-status-callbacks/voice-status-callbacks) to see all of the possible parameters & statuses as well as a working code example!
...
Read in-depth about [recording status callbacks](/platform/phone-numbers/guides/signalwire-status-callbacks/recording-status-callbacks) to see all of the possible parameters and statuses as well as a working code example!
...
You can do this by setting the `StatusCallback` parameter when [sending an outbound message](/compatibility-api/rest/messages/create-message) through the API.

Read in-depth about SMS status callbacks [here](/platform/phone-numbers/guides/signalwire-status-callbacks/sms-status-callbacks) to learn all of their possible parameters and statuses and see a working code example!
...
If you see a message with a status of **Delivered** that did not reach the end handset, feel free to open a support ticket with the [message SID](/platform/what-is-a-sid) for us to escalate the example to our carrier partners to find out why the MNOs (Verizon, AT&T, T-Mobile, etc) marked it as **Delivered**.
...
For detailed instructions on how to configure your webhook to a SignalWire number, see the [Making and Receiving Phone Calls](/swml/guides/make-and-receive-calls) guide.
...
Select **SWML Script** for **When a Call Comes In** to use a SWML script. If you are using the [RELAY Realtime API](/server-sdk/node), select **RELAY** for **When a Call Comes In**. The other options are all third-party integrations you can choose to use for different use cases!
