...
<Frame caption="The Project menu in the Dashboard">
  ![The Project menu open in the Dashboard, listing Project Configuration, Switch Project, and Create New Project.](/assets/images/dashboard/project-settings-menu.webp)
</Frame>
...
Subprojects can be created and deleted through the API with a Project's API token, so an application can
open and retire a Project per customer, per tenant, or per environment on its own.
Root Projects are created in the Dashboard, or through the same API with a
[Personal access token](/docs/apis/authorization#personal-access-tokens). That token carries your owner or
admin role in the Space rather than a Project's scopes, so it can create and delete root Projects too.
...
With a Project's API token, only Subprojects can be deleted; targeting the root Project returns
`422 only_subprojects_can_be_deleted`.
A Personal access token can delete a root Project as well, once its Subprojects are gone
(`422 subprojects_must_be_removed`), and the last Project in a Space can't be deleted
(`422 last_project_cannot_be_deleted`).
...
With a Personal access token the tree is wider: every root Project you are enabled for, plus their
Subprojects, and you can pick which members reach which root Projects with the
[Space Administration API](/docs/apis/rest/space/members/list-member-projects).
