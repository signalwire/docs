...
{/*
  Hidden page. `hidden: true` sits on this page's entry in platform.yml, which is where Fern reads
  it — frontmatter carries `noindex`, not `hidden`. The page stays reachable by direct URL, and out
  of the sidebar, search, and llms.txt.

  Much of what follows is headed for the AI overview page in a later PR rather than living here.
  Fold it in there, then decide whether this page keeps the remainder or goes away entirely.
*/}
...
gateway, so a handle cannot be used to read your prompt. Each message carries `role` and `content`;
`timestamp` is included when the source message has one. The response also carries `timeout` and
`last_activity` — enough to warn a visitor
that their next message will start a new conversation.

### What a leaked key costs you

A publishable key is public by definition, so design for it being taken. A key without a valid
handle cannot read a conversation. A caller holding a handle can use `log` to read that
conversation's filtered user and assistant transcript. A leaked key can also start conversations
and send messages. `max_new_conversations` limits new conversations per rolling `window_seconds`,
while `max_turns` limits the turns in each conversation.
...
For direct REST calls, check the HTTP status first. Platform failures such as malformed JSON,
rejected credentials, and an unavailable or slow service use non-200 responses and are not JSON-RPC
envelopes. On `200`, parse the JSON-RPC envelope and check for an `error` member. For chat requests,
the gateway creates its response as HTTP 200 before it processes the JSON-RPC body, so use that body
to determine the outcome.

For responses returned by the AI chat service, the public endpoint preserves the HTTP status. Its
own `400`, `401`, and `502` failures use the shapes documented in the API reference.

A direct REST turn that takes longer than 30 seconds returns the same 502 response as a service that
cannot be reached. The response does not distinguish those causes; retry with backoff.
...
`chat_log` can include your prompt and other non-dialogue entries. Filter to `user` and `assistant`
before displaying it. The gateway's `log` does this for you.
...
repeating: `call_log` has the same caveat as `chat_log`, in that it can include your system prompt
and other non-dialogue entries. Filter to `user` and `assistant` before showing it to anyone.
...
When it goes wrong, the failure is almost always one of these. Documented codes are listed under
...
- **`-32002` on the first call.** Your `config_url` could not be fetched. Open it from outside your
  ...
  machine.
- **HTTP `401` with `Unauthorized`.** Confirm that the project and token belong to the Space in the
  request URL and that the token includes the `chat` scope. These causes share the same response.
