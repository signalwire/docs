...
[chat-gateway]: /docs/server-sdks/reference/python/ai-chat/chat-gateway
[chat-client]: /docs/server-sdks/reference/python/ai-chat/ai-chat-client
...
On chat, a tool's reply can also split what the model *learns* from what it should *say*, by sending
an object where a call expects a string:

```json title="Your endpoint's JSON reply"
{
  "response": {
    "tool_result": "no driver until 16:00",
    "tool_prompt": "Offer four o'clock or later. Do not promise anything sooner."
  }
}
```

`tool_result` becomes the tool's result and `tool_prompt` is delivered as a separate instruction,
which is worth reaching for when an instruction buried inside a result would read as data. A voice
call takes the string form only, so a tool shared between both channels should return a plain
`response` string.

Setting [`swaig_post_conversation`][swaig-post-conversation] in your SWML adds the transcript to
every tool webhook, as the model currently sees it and in full. It is off by default because it puts
the whole conversation in a body leaving your control.
...
[AI post-prompt callback][post-prompt-webhook] documents the payload, which has two traps:

- `post_prompt_data` is `{"raw": "<string>"}`, not parsed JSON. Even when your post-prompt asks for
  JSON, what arrives is the model's text, commonly a fenced markdown block inside that string. Strip
  the fence and parse it yourself, and handle the model not complying.
- `call_log` has the same caveat as `chat_log`: it includes your system prompt and internal markers.
  Filter to `user` and `assistant` before showing it to anyone.
...
When it goes wrong, the failure is almost always one of these:
...
<Card title="Python ai_chat module" href="/docs/server-sdks/reference/python/ai-chat" icon="brands python">
  `AIChatClient` and `ChatGateway`, with every argument and default.
</Card>
