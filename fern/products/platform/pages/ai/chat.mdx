...
You need two things beyond an agent:
...
```python title="chat_with_agent.py"
import asyncio
...
async def main():
    ...
    async with AIChatClient(space="your-space") as client:
        ...
        if reply.user_event:
            handle(reply.user_event)  # the agent asked your app to do something
...
`config_url`, cannot read arbitrary conversations, and you can revoke it by redeploying with a new
one.
...
SWML sets one. Not instantly — expect seconds. Two traps in that payload:
