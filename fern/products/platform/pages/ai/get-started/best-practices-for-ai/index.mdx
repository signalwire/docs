...
[avoid-overprompting]: /docs/platform/ai/prompt-engineering#avoid-overprompting
...
flowchart LR
    subgraph P["The prompt · fixed at call start"]
        ...
        P3["The tools it can call<br/>names, descriptions, when to use"]
    end
    ...
    P3 -->|"the agent calls a tool · the request reaches your backend"| C3
    C3 -->|"your code runs and shapes the response · the agent answers from it"| P3

    N["The answer comes from your code, not the model ·<br/>the same answer on every call"]
```
...
and a function call reads the current value at the moment the caller asks.

Staleness isn't the only problem.
...
The model also has no way to tell your pasted policy apart from anything else it has read.
...
See [avoiding overprompting][avoid-overprompting] for techniques.
...
to a person.
Leaving them out is what produces a bland, directionless agent.

Business hours look like a stable fact and aren't: holidays move them, a weather closure moves them,
...
The platform's defaults are set to work across most agents, so building one rarely means touching
the parameters on the [`prompt` object][ai-prompt-reference].
...
Two controls shape how those pauses feel.
...
Your function handlers are part of that interval, so move slow work out of band and respond promptly.
