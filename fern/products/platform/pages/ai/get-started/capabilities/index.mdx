...
Two programmable interfaces create and configure agents — pick by how much control you need.

### SWML

Define the agent declaratively in a SWML script — readable JSON or YAML you can deploy serverlessly
from your Dashboard or serve from your own server.
See the [`ai` method reference](/docs/swml/reference/calling/ai) for every configuration option.

### Server SDKs

Build the agent in code with the Server SDKs' Agents namespace (`AgentBase`): the SDK generates the SWML,
hosts your SWAIG functions, and serves the whole agent as a production web service in the language of your choice.
See [build AI agents with the Server SDK](/docs/server-sdks/guides/build-ai-agents).

<Info title="Build agents without code">
Prefer a no- or low-code solution? Create and manage agents entirely from your Dashboard with the
**AI Agents UI**: write the prompt, choose a voice, add SWAIG functions, and assign a phone number,
all visually. Start with the [no-code agent guide](/docs/platform/ai/no-code-agents).
</Info>
