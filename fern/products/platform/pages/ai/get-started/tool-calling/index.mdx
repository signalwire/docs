...
and how busy the fleet is, so no prompt could hold it. The agent below confirms the addresses in code
before anything is priced, keeps them in `global_data` so the quote itself takes no arguments, covers
both waits with fillers, and declares both functions inline.
