...
Deploy a minimum viable product with no-code and low-code tools, then scale it with SWML or the Server SDKs.
...
Then open the number's settings and use **+ Assign Resource** to attach your new SWML script.
...
[AI in the media path](/docs/platform/ai/capabilities#ai-in-the-media-path) explains
what that architecture does for response time.

<llms-ignore>

<img src="/assets/images/img/ai-agent-flow-light.svg" alt="How a call flows: a caller converses with an AI agent on the SignalWire platform; SWAIG delivers function calls to your backend over HTTP and returns the JSON reply to the conversation." class="light-only" />
<img src="/assets/images/img/ai-agent-flow-dark.svg" alt="How a call flows: a caller converses with an AI agent on the SignalWire platform; SWAIG delivers function calls to your backend over HTTP and returns the JSON reply to the conversation." class="dark-only" />
...
<Markdown src="/snippets/common/diagrams/ai-agent-flow-inline.mdx" />
...
<Card title="Platform capabilities" href="/docs/platform/ai/capabilities" icon="regular list-check">
