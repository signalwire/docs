...
description: Build programmable, realtime voice AI agents with SWML, the SignalWire Dashboard, Call Flow Builder, or the Server SDK.
max-toc-depth: 3
---

SignalWire AI runs programmable voice agents on the same realtime platform that carries your calls.
Deploy an MVP with no-code and low-code tools, then scale it with SWML or the Server SDKs.
...
Diagram summary: the caller converses with the AI agent inside the SignalWire platform, which carries
the audio and holds the conversation; SWAIG delivers the agent's tool calls to your backend as HTTP
requests, and your backend's JSON replies return to the conversation.
