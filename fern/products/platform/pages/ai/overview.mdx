...
contexts, the state your handlers keep — answers a phone call, a SIP session, a browser or mobile
session, and a text chat. A tool call behaves the same way on all of them, so the code enforcing your
business rules is written once and does not care how the conversation arrived.
