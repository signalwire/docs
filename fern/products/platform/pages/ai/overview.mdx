...
An AI agent is a conversation you define once and make available over voice or text. The prompt,
contexts, state, and tools stay with the agent whichever channel a person uses. SignalWire runs the
model and the conversation; you supply the instructions and connect the agent to your systems.
...
<llms-ignore>
  ...
  <img src="/assets/images/img/ai-agent-flow-themed.svg" alt="Voice and text channels arrive at the same SignalWire AI agent, and the agent's reply travels back out the same channel. A channel carries only transport: voice over PSTN, SIP, or WebRTC, and text as one HTTP request per turn through the AI Chat API. The SignalWire platform handles end-pointing, speech-to-text, text-to-speech, and the call or chat session, and holds the agent's prompt, contexts, and state. The agent's SWAIG tools post each tool call to your backend over HTTP, the only network hop off the platform, where your webhook reads your own systems." />
...
```mermaid
flowchart LR
    subgraph Channels["Channels — carry the conversation, both ways"]
        Voice["Voice<br/>PSTN · SIP · WebRTC<br/>audio in, audio out"]
        Chat["Text<br/>AI Chat API<br/>one HTTP request per turn"]
    end

    subgraph SW["SignalWire platform<br/>end-pointing · speech-to-text · text-to-speech<br/>call and chat session"]
        Agent["One AI agent<br/>prompt · contexts · state"]
        SWAIG["SWAIG tools<br/>same functions · results · actions"]
    end
...
People can reach the same agent in two ways:

| Channel | How the conversation arrives | What the platform handles |
|---|---|---|
| Voice | A [phone number, SIP URI, or dialable resource name][addresses] over the Public Switched Telephone Network (PSTN), SIP (Session Initiation Protocol), or WebRTC | Audio, end-pointing, speech recognition, the model, and speech synthesis |
...
Server code can call the Chat API directly with [`AIChatClient`][chat-client]. A browser or mobile
app connects through [`ChatGateway`][chat-gateway], which runs in your application so your
SignalWire API token never reaches an untrusted client.

On a voice channel, every turn passes through four stages before the caller hears a reply: detecting
that the caller has finished speaking (end-pointing), transcribing the audio, generating the
response, and synthesizing the reply as speech. A text turn skips the audio stages, but reaches the
same prompt, contexts, state, and tools.
...
For both channels, the SWAIG tool call is the network hop to infrastructure you control. Enforcing a
fact in code means fetching it while the person waits, so your server's own response time is real.
...
| Layer | Who runs it |
|---|---|
| Model and conversation state | SignalWire, for voice and text conversations |
| End-pointing, speech-to-text, and text-to-speech | SignalWire, on voice channels |
| Phone numbers, SIP trunks, and WebRTC sessions | SignalWire |
| Browser and mobile chat connection | You, through `ChatGateway`; SignalWire handles each text turn through the AI Chat API |
...
An agent is not tied to one channel. The definition you write once — the prompt, the tools, the
contexts, and the state your handlers keep — handles voice calls and text conversations. SWAIG uses
the same function definitions and response structure for each. Its webhook identifies the channel
as `voice` or `chat`, but the code enforcing your business rules does not need a separate tool
implementation for each one.

What differs between channels is the audio-specific work. Choosing a voice, adding speech hints, and
detecting that a caller has finished speaking all shape a spoken turn and have nothing to act on in a
text conversation. Everything else below applies whichever channel a conversation arrives on.
...
<Card title="Speech hints" href="/docs/server-sdks/guides/hints" icon="regular comments">
  Recognition help for names, jargon, and product terms the model would otherwise mishear. Voice
  channels only.
</Card>

<Card title="Voices and languages" href="/docs/platform/voice/tts" icon="regular waveform-lines">
  More than a dozen text-to-speech providers, with a different voice per language. Voice channels
  only.
</Card>

<Card title="Conversation analytics" href="/docs/platform/ai/analytics" icon="regular gauge-high">
  Live diagnostic events for voice calls and a final report for voice or text conversations, with
  transcripts, tool activity, outcomes, and the measurements each channel produces.
</Card>
