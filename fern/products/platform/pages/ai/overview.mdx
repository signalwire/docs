...
<llms-ignore>
  ...
  <img src="/assets/images/img/ai-agent-flow-themed.svg" alt="How a call flows: a caller converses with an AI agent on the SignalWire platform; SWAIG delivers function calls to your backend over HTTP and returns the JSON reply to the conversation." />
...
half belongs in your code. The prompt shapes the conversation; your backend prices the trip, checks
the calendar, and books the seat. An agent whose rules live only in a prompt is an agent you have to
...
understanding the caller, collecting details, speaking naturally. Your backend stays the backend,
...
are configured in the [SWML AI prompt reference](/docs/swml/reference/calling/ai/prompt) and through
the Server SDK [contexts and workflows guide](/docs/server-sdks/guides/contexts-workflows).
...
SDKs generate SWML (SignalWire Markup Language), so the choice between them is one of authoring rather
than of capability.
...
You can also create an agent without writing code. In your SignalWire Space, an AI Agent is one of the
[resources](/docs/platform/resources) you can add from the Dashboard: set its prompt and parameters
there, then assign it a phone number.
...
For questions your documents answer rather than your API, [Datasphere](/docs/apis/rest/documents/list-documents)
