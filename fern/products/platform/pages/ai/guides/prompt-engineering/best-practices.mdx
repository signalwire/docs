...
subtitle: Essential techniques for AI Agent prompt engineering
...
A strong prompt controls *what* your AI Agent says. The
[`prompt`](/docs/swml/reference/calling/ai/prompt) object also exposes configuration parameters
that control *how* the model generates responses, so you can tune tone, variety, and
conversational pacing without rewriting the prompt.

- **Randomness (`temperature`, `top_p`):** Lower values keep responses focused and predictable;
  higher values add variety. Adjust one of these, not both.
- **End-of-utterance detection (`confidence`):** Sets how readily the agent decides the caller has
  finished speaking. Lower values shorten the pause after the caller speaks but can cause the agent
  to interrupt.
- **Topic and repetition (`presence_penalty`, `frequency_penalty`):** Positive values push the
  model toward new topics and away from repeating itself.
- **Response length (`max_tokens`):** Caps how long a single response can be.

<Tip>
Start from the defaults, change one parameter at a time, and test. For the full list of
parameters with their value ranges and defaults, see the
[`prompt` reference](/docs/swml/reference/calling/ai/prompt).
</Tip>
