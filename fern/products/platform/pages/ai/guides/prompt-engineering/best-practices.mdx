...
When crafting prompts, be precise and specific. Clearly define what you aim to achieve, set the boundaries and limitations of your project, specify the desired output formats, and outline the criteria for measuring success.
...
### Define success, not a list of don'ts
...
Then let the rules that must hold every time live somewhere sturdier than prose:
in your handlers, in the functions each step exposes, or in the conscience.
The [guardrails](/docs/platform/ai/prompt-engineering#guardrails) section covers that order in full.
