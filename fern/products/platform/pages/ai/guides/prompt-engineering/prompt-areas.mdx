...
`verify_account` decides whether the caller is verified, so nothing depends on the prompt
...
This is where the rules go that would otherwise pile up in the main prompt as a list of don'ts.
A rule stated once in a long main prompt competes with everything around it, while the same rule in
the conscience is reinforced throughout the call, including after each function call.
Keep the list short, and reserve it for the rules your code can't enforce on its own.
