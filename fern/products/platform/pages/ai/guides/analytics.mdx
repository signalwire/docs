...
class AnalyticsAgent(AgentBase):
    def __init__(self):
        ...
        self.set_params({"enable_accounting": True})
...
and documents the callback's core fields. The `conversation_type` field identifies voice or chat
reports.
...
[speech hints][speech-hints] for vocabulary that is repeatedly misrecognized.
...
When `enable_accounting` is true, the final report includes totals for:
...
| The agent mishears recurring terms | Low-confidence user turns | Add [speech hints][speech-hints] |
