...
SWAIG:
  functions:
    - function: check_inventory
      description: Check the warehouse system for an item's stock level
      parameters:
        type: object
        properties:
          sku:
        ...
        required:
          - sku
      web_hook_url: https://example.com/check-inventory
```
</CodeBlock>
...
The [order-status agent](#an-order-status-agent) below is a complete, runnable version of this in both
forms.
...
Declare remotely when a toolset outlives any single agent. The definitions sit next to the code that
runs them, you can add a parameter without editing any SWML, and one server can supply the same tools
to every agent that includes it.
