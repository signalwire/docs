...
The request is also published in the API reference as the
[AI SWAIG tool webhook][swaig-webhook], where you can read the schema for the payload SignalWire
sends and download it with the rest of the OpenAPI document to generate types or validate requests
in your tests.
The same fields appear in the [`SWAIG.functions` reference][swaig-functions] alongside the SWML
