...
description: Connect your AI agent to your backend with tool calls, so your code — not the prompt — owns business logic, live data, and decisions.
...
The most common voice AI failure in production isn't a bad model — it's architecture.
Teams stuff the menu, the prices, the policies, and the workflow into one giant prompt and hope the AI follows it.
That approach ("prompt and pray") survives a scripted demo. It doesn't survive real customers.

SignalWire agents are built around a different model.
The AI agent is the **front end** of the call: it speaks, listens, understands what the caller wants, and collects details.
Your backend stays what it already is — the source of truth — and the agent reaches it through **tool calling**,
also known as *function calling*.
On SignalWire, tool calls flow through **SWAIG**, the SignalWire AI Gateway.
...
the backend does, because the backend can be tested, versioned, and trusted.
A voice AI agent deserves the same architecture: the conversation is the interface, and your code is still the application.

A prompt is a suggestion. Code is a constraint.
An instruction like "never give discounts" holds *most* of the time — and "most of the time" is not a standard
...
Tool calling is what decides *which kind of work* the prompt should be doing.

## How a tool call works

A tool is a named capability you hand to the agent: a **name**, a **description**,
and a JSON Schema describing its **parameters**.
If you've used tool calling with OpenAI or Anthropic APIs, this is the same concept —
your existing intuition transfers directly.

The description does double duty: it's how the AI decides *when* to call the tool.
"Look up order status by order number" tells the agent both what the tool does and what it needs first.
Writing good tool descriptions is prompt engineering by another name.

A tool call round-trips like this:

1. The caller says something ("Where's my order? It's one-two-three-four-five.").
2. The AI decides a tool applies and extracts the arguments from the conversation.
3. The SignalWire platform sends an HTTP POST to the endpoint you configured for that function.
4. Your code runs — a database query, an API call, a calculation.
5. Your endpoint returns a `response` (text for the AI to work from) and, optionally, `action` objects.
6. The AI weaves the response into what it says next, and the platform executes any actions.
...
Notice that `response` is written *to the AI*, not to the caller —
it can carry data, and it can carry instructions about what to do next.
...
The prompt never claims to know order status — it only knows there's a tool for that.
...
And when a caller asks about an order that doesn't exist, the agent says so,
because your code said so. An AI that can answer "I don't know, let me get you to someone who does"
...
Alongside `response`, your code can return **actions** — instructions the platform executes on the live call:

- **Transfer the call** to a human, a queue, or another agent.
- **Send an SMS** — a confirmation, a link, a receipt.
...
the decision-making can stay in your code even when the *delivery* is conversational.
Your backend notices the customer qualifies for the combo upgrade, and its tool result tells the AI to offer it.
The AI supplies the charm; your code supplies the judgment.
...
Five habits separate agents that demo well from agents that hold up in production.

### Validate in code, recover in conversation

Every argument the AI fills in is extracted from spoken, imperfect audio — treat it as user input, not as truth.
...
("No match for that address — ask the caller to repeat it street first").
...
Write it to `global_data` — call-scoped state that lives with the session — and let downstream tools
take **no arguments at all**, reading the validated state instead:
...
The AI can't call `book_ride` before `get_quote` if `book_ride` doesn't exist yet from where it's standing.
See [contexts and workflows][contexts-workflows] in the Agents SDK,
or [toggling functions][toggle-functions] in SWML.

### Cover the wait

Real lookups take real time, and silence on a phone call reads as broken.
...
Set a [`post_prompt_url`][ai-reference] and the platform delivers a post-call report —
summary, full conversation log, and every tool call with its timing —
...
inspects these reports — transcript, telemetry, and latency in one place.
...
The AI never sees the catalog.
It gets `add_item`, `remove_item`, and `finalize_order`; your code resolves each request against
...
Don't let the model answer from its training data — that's how agents invent policy.
