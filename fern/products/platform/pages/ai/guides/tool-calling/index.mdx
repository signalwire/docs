...
The typical production failure in voice AI isn't a bad model. It's architecture:
...
That menu makes the prompt fragile.
...
def add_item(item: str, raw_data=None) -> FunctionResult:
    """Add an item to the order when the customer asks for one.

    Args:
        item: The menu item the customer asked for, in their own words
    """
...
def finalize_order(raw_data=None) -> FunctionResult:
    """Total the order and send it to the kitchen."""
...
In the SDK version, the Python function itself is the definition:
the docstring becomes the description the AI reads, and the type hints become the parameter schema.
...
The prompt never claims to know order status; it only knows there is a function for that.
...
Read real transcripts, find the turns where the AI hesitated or picked the wrong function,
and iterate on your function descriptions the way you'd iterate on any interface copy.
