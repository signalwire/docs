...
- **It is about records, not storage.** Redaction masks what gets recorded; it does not decide
  ...
  only matters during the call can go in the call's [`meta_data`][set-meta-data] — set through
  your APIs when the call starts or by a SWAIG function while it runs — which lives for the life
  of the call and is not stored afterward.
