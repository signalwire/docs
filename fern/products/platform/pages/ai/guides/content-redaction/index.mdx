...
- **It is about records, not storage.** Redaction masks what gets recorded; it does not decide
  where a value lives. Data that must outlive the call belongs on your server, while data that
  only matters during the call can go in [`meta_data`][set-meta-data] — declared in the SWML
  document or written by a SWAIG function's `set_meta_data` action, readable by the functions that
  share its `meta_data_token`, and gone when the session ends.
