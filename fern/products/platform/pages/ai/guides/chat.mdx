...
AI chat can be accessed in multiple ways, but not all functionality is available in all cases.

| Function | Direct | Gateway | Phone number |
|---|---|---|---|
| Run it from a server you control | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> |
| Run it from a browser, or any other untrusted client | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> |
| Reach the agent over SMS, with no client of your own | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> |
| Choose which agent a conversation reaches, per request | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> |
| Summarize or delete a conversation on demand | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> |
| Read the conversation log including its system entries | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> |

- **Direct** is JSON-RPC 2.0 against [`POST /api/ai/chat`][chat-endpoint], carrying your project API token. [`AIChatClient`][chat-client] covers the same ground in Python with typed methods and typed exceptions.
- **Gateway** is [`ChatGateway`][chat-gateway] mounted inside a web application you already host. It holds the API token, the project, and the `config_url`; the page holds a URL and a publishable key that the gateway manages itself.
- **From a phone number** sits on top of the direct path rather than beside it: someone texts your number and the agent texts back. Your server still holds the token.
