...
WhatsApp access is gated at the Space level. Contact your SignalWire representative to have the
feature enabled for your Space and to begin onboarding.

{/* TODO(team): replace with the canonical early-access request URL/form once confirmed. */}
...
```bash
curl https://{space}.signalwire.com/api/relay/rest/whatsapp/businesses \
  -u "{project_id}:{api_key}"
```

And list the WhatsApp numbers available to use as a `from` address:

```bash
curl https://{space}.signalwire.com/api/relay/rest/whatsapp/numbers \
  -u "{project_id}:{api_key}"
```

Each number record includes its association with a WhatsApp Business Account, voice-capability flags,
and the resource IDs used for routing calls or messages through SignalWire.
