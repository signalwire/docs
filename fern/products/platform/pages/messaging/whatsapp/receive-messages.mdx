...
[sending a message](/docs/platform/messaging/whatsapp/send-messages). SignalWire then POSTs a
message-status payload to your URL at each state change.
...
See the [Message status callback reference](/docs/apis/rest/webhooks/message-status-callback) for every field and status value.
