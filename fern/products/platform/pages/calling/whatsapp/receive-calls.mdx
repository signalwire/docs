...
greeting, then assign the number from the Dashboard or the REST API, read the caller's number in an
AI Agent, and find the calls in your logs.
...
The response carries the new Resource's `id`, which the SDK programs print. That is your
`<YOUR_RESOURCE_ID>`.
...
<EndpointResponseSnippet endpoint="GET /api/voice/logs" example="WhatsApp call" highlight={10} />
