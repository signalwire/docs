...
<Step title="Set up a calling address">

What the call can present as its caller depends on where you are calling.

| Dialing | Your calling address must be |
|---|---|
| A phone number | A [purchased number][phone-numbers] or [verified caller ID][caller-id], in E.164, with voice enabled |
| A SIP endpoint | An E.164 number, a SIP URI, or a short caller ID token of up to 11 letters, digits, or underscores |
| A Resource | Anything — the format is not checked, though one of your own numbers is the safe choice |

Whichever you use, how it is labeled decides whether people answer. The [spam labels][spam-labels]
and [STIR/SHAKEN][stir-shaken] guides cover call reputation and attestation.
