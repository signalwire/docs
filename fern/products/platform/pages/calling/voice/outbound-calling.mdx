...
description: Learn how REST requests, inbound call forwarding, and SDK requests start outbound calls, with server and browser examples.
max-toc-depth: 3
---
...
An outbound call is a call SignalWire places to a phone number or SIP endpoint. It can start
with a REST API request, an inbound call being forwarded, or a request from an SDK.
...
Use E.164 format for phone numbers, such as `+15551234567`. For Calling API requests, use a
[phone number][phone-numbers] purchased in your project or a [verified caller ID][caller-id] as
the calling number. Forwarding has its own caller ID configuration; the
[forwarding guide][forward-calls] shows how to preserve the original caller's number.
...
For Calling API requests, check `from` and any `caller_id` override against your purchased or
verified numbers. For forwarded calls, check the caller ID configured in your forwarding flow.
