...
The [Calling API][calling-api] also accepts `sips:`, `verto:`, and `client:` addresses in `to`.
For a destination handled by a SWML script, use `to_script` with an inline document or a URL that
returns one; `to` can then be omitted. This is separate from the `url` or `swml` instructions
that handle the call.

SWML's [`connect` method][swml-connect] also supports queues (`queue:support`) and WebSocket
media streams (`stream:wss://example.com/audio`). These route an existing call to a queue or
stream its media; they use the connection-specific options in the `connect` reference.
