...
- If calling from a Web Client, you need a [Subscriber token][subscriber-token] or
...
|---|---|---|---|
| Place the call without holding a connection open | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> |
| Place the call from a web page, with the user speaking on it | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> |
...
| Follow the call's events in your own code, with no public webhook URL | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-check" color="var(--status-success)" /> |
| Receive call progress as HTTP callbacks to a URL you host | <Icon icon="regular circle-check" color="var(--status-success)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> | <Icon icon="regular circle-xmark" color="var(--status-error)" /> |
...
defaults, but options passed to `dial()` always win. To pin the microphone, camera, or speaker across
...
Bind each to a media element's `srcObject`. Give the local element `muted` so the user doesn't hear
their own voice back, leave the remote element unmuted, and give both `playsinline` for mobile Safari.
An audio-only call uses `remoteStream$` the same way.
