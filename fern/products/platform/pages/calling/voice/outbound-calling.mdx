...
Use E.164 format for the caller ID, including `+` and the country code.
...
|---|---|---|
| Phone | Its number in E.164 format | `<YOUR_DESTINATION_NUMBER>` |
...
Subscribers, applications such as AI agents, SWML scripts, or Call Flows, and conference rooms are
[Resources][resources]. Each has a [resource address][resource-addresses] in the form
`/context/name`, where the context is `public` or `private`. The Relay Server SDK supports phone
numbers and SIP endpoints, but not resource addresses.

For this walkthrough, choose a phone number you can answer and replace
`<YOUR_DESTINATION_NUMBER>` with it. Include `+` and the country code.
        ...
        call = await client.dial(
            devices=[[{
                ...
                "params": {
                    ...
                    "to_number": "<YOUR_DESTINATION_NUMBER>",
                    "timeout": 30,
                },
            }]],
        )
...
callButton.onclick = async () => {
  ...
  try {
    const call = await client.dial("<YOUR_DESTINATION_NUMBER>", { audio: true, video: false });
...
This example dials two phones. Keep `<YOUR_DESTINATION_NUMBER>` as the rider's number, and replace
`<YOUR_DRIVER_NUMBER>` with a second number you can answer as the driver.
