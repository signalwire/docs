...
[make-outbound-call]: /docs/platform/voice/make-and-receive-calls#make-an-outbound-call
[answer-inbound-call]: /docs/platform/voice/make-and-receive-calls#answer-an-inbound-call
...
- For the inbound examples, a phone number in your Space whose calls reach your SWML server or
  Relay client. [Answer an inbound call][answer-inbound-call] sets up both.
...
start with [Make an outbound call][make-outbound-call].
...
This guide detects who is on a call. To take messages that callers leave on *your* number, see
the [make and receive calls guide][make-and-receive-calls].
...
<Note title="TypeScript REST signatures depend on the SDK version">
The TypeScript samples on this page pin `@signalwire/sdk@2.0.5`, whose REST `dial()`, `detect()`,
and `detectStop()` each take a single options object after any call ID. The TypeScript
reference documents the positional forms of a newer release, such as
[`dial(from, to, options)`][ts-dial]. Match the form to the version you install.
</Note>
...
you say then is lost. The [make and receive calls guide][make-and-receive-calls] has a shorter version of this flow.
...
or send the [`calling.detect` command][call-commands] with the call's `id`. In
`@signalwire/sdk@2.0.5`, the TypeScript `detect()` and `detectStop()` take every field in one
object after the call ID; the linked reference shows the positional form of a newer release.
The command returns at
...
3000 at that public HTTPS host; [Answer an inbound call][answer-inbound-call] covers pointing your
number at the URL, and [webhook security][swml-webhook-security] covers verifying requests. For a hosted SWML Script, paste the YAML or JSON instead.
