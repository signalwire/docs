...
a [reminder that leaves a voicemail](#send-a-reminder-call-that-leaves-a-voicemail) and a
[business line that forwards callers or takes a message](#forward-callers-to-a-dispatcher-or-take-a-voicemail).
...
The examples later on this page use these additional values:

| Value | Replace with |
|---|---|
| `<YOUR_AGENT_DESTINATION>` | The phone number, in E.164 format, of the person who answers forwarded calls |
...
Continue with [call tracking](#track-the-calls-progress) or the [outbound example](#send-a-reminder-call-that-leaves-a-voicemail).
...
Continue with [call tracking](#track-the-calls-progress) or the [inbound example](#forward-callers-to-a-dispatcher-or-take-a-voicemail).
...
in both directions. Follow the section for the direction and approach of your first call.
...
Each example builds on the first call in its direction. Keep the credentials, Resource, and
address you set up there. Replace the first-run program with the example, or, for a hosted
SWML Script, paste the example's YAML or JSON.

### Send a reminder call that leaves a voicemail
...
### Forward callers to a dispatcher, or take a voicemail

Greet callers on your business number, ring a person, and record a message if nobody picks up.
Here, Bayview Taxi forwards callers to a dispatcher at `<YOUR_AGENT_DESTINATION>`. Keep the ring
timeout shorter than that phone's own voicemail, or its voicemail answers first.
