...
If your number's carrier offers CNAM, set the name in your Dashboard or with [Request a caller ID name](/docs/apis/rest/caller-id-name/request-caller-id-name). Then use the ticket below only for A attestation. If CNAM is not available for the number, include the requested name in the ticket.
