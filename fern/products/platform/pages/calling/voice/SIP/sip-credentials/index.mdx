...
A SIP Address reaches your resource at `{space_name}-{domain}.dapp.signalwire.com` — the host you
give an external carrier. Note the `.dapp.` subdomain: it's a different host from the
`{space_name}.sip.signalwire.com` domain that SIP Credentials register against.

<Note>

A SIP Address and a **domain application** are the same object under two names. Your Dashboard
calls it a SIP Address; the [Domain Applications](/docs/apis/rest/domain-applications/create-domain-application)
REST API calls it a domain application. Either one produces the same `.dapp.signalwire.com` host,
and a domain application created over the API appears in your Dashboard under **My Resources**.
