...
If you would like to do inbound BYOC (i.e. send your carrier traffic to SignalWire), you will need to create a domain application. Domain Apps allow you to send SIP traffic to a custom domain and use SignalWire APIs to manage the incoming request. You can do this under **SIP** -> **Domain Apps** -> **Create a Domain App**, or via the [Domain Apps API](apis/relay-rest/domain-applications/create-domain-application).

For detailed instructions on how to create a domain application, please follow the [SIP Domain Applications](platform/voice/sip/domain-applications) guide.
...
To learn more about what you can do with RELAY, have a look at our [RELAY Realtime SDK documentation](server-sdk/node).

## Outbound BYOC

If you are using BYOC to do outbound calls from SignalWire, you will need a SIP URL from your carrier that we can use to route calls to the right SIP trunk. Once you have that, you can [create a call using the Calling API](apis/calling/calls/call-commands) or [Realtime SDK](server-sdk/node/reference/voice/client#dial) using the SIP URL, SIP username, and SIP password. Let's look at some examples so you can see what we're talking about!
...
To create an outbound call using the Calling API with curl, you can use the [Create a Call endpoint](apis/calling/calls/call-commands) with the following format:
...
[RELAY Realtime SDK](server-sdk/v3/node/reference/voice) is our most recent version of RELAY, and dialing out to a SIP endpoint is simpler than ever:
