...
description: Set up voicemail on a number using SWML or cXML.
---
...
This guide will show how to do exactly that using SWML or cXML.
...
We use the [`play`](/swml/methods/play) method to play the voicemail prompt, then the [`record`](/swml/methods/record) method to record the caller's message.
The `record` method pauses SWML execution until the recording ends, so the final message plays immediately after.

The [`record`](/swml/methods/record) method accepts parameters to configure the beep, terminators, max length, silence timeout, and more.
For additional information, see the `record` [technical documentation](/swml/methods/record).

</Tab>
<Tab title="cXML">

We are going to implement voicemail in a cXML script hosted on SignalWire.
...
Edit the settings of your selected number, and assign the SWML script or cXML script you created to
the Inbound Call Settings.

![Assign Resource to Phone number.](/assets/images/dashboard/phone-numbers/assign-resource-voice.webp)
...
We have seen how to build a basic voicemail using SWML scripts or cXML scripts.
