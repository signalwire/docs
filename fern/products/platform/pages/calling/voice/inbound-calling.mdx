...
Then assign the SWML Script to your number.
...
To do the same with the REST API, [create the SWML Script][rest-create-script] with the document
as `contents`, then [link it to the number][rest-link-number]. The link endpoint is in beta and
requires the number's calling channel to have no Resource assigned yet.
...
Then assign the Relay Application to your number.
...
Assign the number to the Subscriber your token was issued for.
...
Dial your SignalWire number from your phone. With either SWML approach or with Relay, you hear
"Hello, welcome to SignalWire!" and the call ends. With the browser example, the page shows the incoming call.
...
- For the browser, the token is a Subscriber token, not a guest or embed token, and the number is
  assigned to that same Subscriber.
...
document per caller. See the [webhook payload reference][swml-webhook-payload] for every field.
