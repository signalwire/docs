...
calling, run an AI agent, forward the call to a person, or let users answer from your web app.
...
- A voice-capable [phone number in your Space][phone-numbers] and a phone to call it from.
  If you're testing with SIP or another SignalWire client, you can use a SIP address or an alias instead.
...
A [trial project][trial-mode] only receives calls from phone numbers it has verified. Verify the
...
the caller can answer on a phone, on a SIP device, or in a browser.
...
|---|---|---|
| [Server SDK over HTTP (SWML)](#create-your-call-handler) | Your server chooses instructions for each incoming call | SignalWire requests a document from your public HTTPS endpoint and runs the SWML it returns. |
| [Server SDK over WebSocket (Relay)](#create-your-call-handler) | Your server makes decisions as call events arrive | A running Relay client receives calls for its topic and controls them over a persistent connection. |
...
To hear your first call work with the least setup, choose **Hosted SWML**. SWML is the document of
...
either SWML approach or a Relay Application for Relay, or use a Subscriber for the Browser SDK.
...
instead. Every address type works with every handler above.
...
Use the Resource you just created as the address's **call handler**. A phone number's call
handler is separate from its message handler.
...
steps above. Choose the example that matches your handler; assigning another handler replaces
the number's current call route.
...
beta and works only when the number's calling channel has no Resource assigned yet.
...
that requirement. To add the address through the API, use [Create a SIP address][rest-create-sip]
...
with SWML or event subscriptions with Relay and the Browser SDK. Read the section for the
...
object carries the `from` and `to` addresses, the `direction`, and the `call_id`. Your code can
use them to choose the document for each caller. `from` is a phone number, a SIP URI, or an alias,
depending on how the caller reached you. See the [webhook payload reference][swml-webhook-payload]
for every field.
...
Keep the Resource and address from your first call. Change the handler's behavior with one of the
examples below:
...
pick the destination. See the [IVR guide][swml-ivr] for a more complete menu with speech input.
...
The stream runs for as long as the rest of the document does, so follow it with a method that
holds the call, such as `ai` or `connect`.
...
Subscriber directly, or a SWML handler can connect the call to the Subscriber's address, as shown
...
it mirrors the media the caller offered, so pass both explicitly to control what the call carries.
...
To choose the microphone, camera, or speaker for every call, use the
