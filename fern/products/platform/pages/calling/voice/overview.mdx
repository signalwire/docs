...
<CardGroup cols={3}>
  ...
  <Card title="Build with AI" icon="regular bolt" href="/docs/platform/ai/quickstart">
  ...
  <Card title="Place an outbound call" icon="regular phone-arrow-up-right" href="/docs/platform/voice/outbound-calling">
...
<CardGroup cols={2}>
  ...
  <Card title="Receive WhatsApp calls" icon="brands whatsapp" href="/docs/platform/voice/whatsapp/receive-calls">
    Take calls placed from WhatsApp on your business number and route them like any other call.
  </Card>
  <Card title="Record calls" icon="regular microphone" href="/docs/platform/voice/record-calls">
    Capture a whole call, a single message, or a conference, then manage the files.
  </Card>
</CardGroup>
...
Yes, SignalWire will allow you to easily implement Answering Machine Detection (AMD) on your calls! AMD listens to the call to determine if the party that picked up is a real person or a voicemail machine. You can use this information to determine whether to leave a voicemail message or begin interacting with a real person. You can use AMD through [`detect_machine`](/docs/swml/reference/calling/detect-machine) in SWML or `detect_answering_machine` in the [Server SDKs' Relay namespace](/docs/server-sdks/reference/python/relay/call).
</Accordion>

<Accordion title="Do you have call recording capabilities?">
Yes. Record a call from [SWML](/docs/swml/reference/calling/record-call), from a [Relay call handler](/docs/server-sdks/reference/python/relay/call/record) in the Server SDKs, over the REST Calling API, or with a node in Call Flow Builder. The [record calls guide](/docs/platform/voice/record-calls) covers each of them.
</Accordion>

<Accordion title="Do you have transcription capabilities?">
Yes - transcription is possible using the [Server SDKs' Relay namespace](/docs/server-sdks/reference/python/relay/call/transcribe) or through SWML's [AI agent](/docs/swml/reference/calling/ai) capabilities. You can also set a callback in order to do something with your transcriptions when they're received.
</Accordion>
