...
slug: voice/tts/rime
description: Learn how to use Rime's Coda and Mist v2 TTS models with SignalWire AI Voice applications.
---
...
| `arcana` <br/><span class="deprecated-desc">Deprecated</span> | <span class="deprecated-arg">Retired by Rime. Requests for `arcana` are served by `coda` — use `coda` instead.</span> |
...
To use Coda voices, set `model` to `coda` with the
...
<Note>
Portuguese, Hindi, Arabic, and Japanese voices are available with the `coda` model only.
The default `mistv2` model supports English, Spanish, French, and German.
To use the other languages, set `model` to `coda` as shown [above](#voices).
</Note>
...
and transfer to other [Resources](/docs/platform/resources) on SignalWire.
*/}
