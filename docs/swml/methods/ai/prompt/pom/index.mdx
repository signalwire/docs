...
| `body`<span className="conditionally-arg">Conditionally Required</span> | `string` | -       | The body of the section. This will be a paragraph in the rendered prompt. This parameter is required if `bullets` is not present.  |
| `bullets`<span className="conditionally-arg">Conditionally Required</span> | `string[]` | -       | An array of strings that represent the bullets of the section. This will be a list of short statements/rules in the rendered prompt. This parameter is `optional` if `body` is present.  |
