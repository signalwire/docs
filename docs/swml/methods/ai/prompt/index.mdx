...
| [`pom`][pom]<span className="optional-arg">Optional</span>                             | `object[]` | -       | An array of objects that defines the prompt object model (POM) for the AI. The POM is a structured data format for organizing, and rendering prompt instructions for AI agents. If `text` is present while using `pom`, the `pom` prompt will be used instead of `text`. See the [POM technical reference][pom] for more information.                                                                 |
