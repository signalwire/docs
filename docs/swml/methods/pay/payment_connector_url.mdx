...
The `payment_connector_url` is a required URL that processes payment requests during the payment flow. 
This url receives payment details once all required information is gathered from the user. 
The url is responsible for processing the final payment transaction and returning the results through the response.


| Name | Type | Description |
|:-----|:-----|:------------|
| `payment_connector_url` | `string` | The URL to make payment requests to upon completion of all required payment details. |


## Payment connector URL request

SignalWire will send a `POST` request to the URL specified in the `payment_connector_url` parameter.
These details are sent to the connector URL once all required payment details are gathered from the user.
...
The response format varies depending on whether you're processing a charge or generating a token.
...
For a successful charge transaction, return a `200` status code with a JSON response containing:
...
For a successful tokenization, return a `200` status code with a JSON response containing:
...
For failed charge transactions, return a non-200 status code with a JSON response containing:
...
For failed tokenization, return a non-200 status code with a JSON response containing:
