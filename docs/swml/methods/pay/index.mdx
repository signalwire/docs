...
Enable secure payment processing during voice calls. When implemented in your voice application, it handles the complete payment
flow including data collection, validation, and processing through your configured payment gateway.
...
Tokenization allows you to securely store payment information for future use. 
Instead of processing a payment, it generates a secure token that represents the payment method.
You can pass the `charge_amount` as `0` or omit the `charge_amount` attribute entirely to initiate a tokenization transaction.
...
| `currency`<span className="optional-arg">Optional</span>       | `string`           | `usd`            | Takes the currency of the charge amount. Default: usd                                                                                                            |
...
| `language`<span className="optional-arg">Optional</span>       | `string`           | `en-US`          | Language to use for prompts being played to the caller by pay IVR.                                                                                                                                                  |
| `max_attempts`<span className="optional-arg">Optional</span>   | `integer`          | `1`              | Number of times pay IVR should retry when collecting payment card details.                                                                |
| `min_postal_code_length`<span className="optional-arg">Optional</span> | `integer` | `0`              | Callers are expected to enter at least these many digits when prompted for postal code by pay IVR.                                                                                                                                                                      |
...
| `postal_code`<span className="optional-arg">Optional</span>    | `boolean\|string`  | `true`           | Takes true, false or real postcode (if it's known beforehand) to let pay IVR know whether to prompt for postal code.                                                                |
...
| `security_code`<span className="optional-arg">Optional</span>  | `boolean`          | `true`           | Takes true or false to let pay IVR know whether to prompt for security code.                                                         |
| `status_url`<span className="optional-arg">Optional</span>      | `string`           | -                | The URL to make requests to for each status change during the payment process. See the [status_url request body](#status_url_request_body) section for more details.                                                       |
| `timeout`<span className="optional-arg">Optional</span>        | `integer`          | `5`              | Limit in seconds that pay IVR waits for the caller to press another digit before moving on to validate the digits captured.                                                                                                    |
...
| `voice`<span className="optional-arg">Optional</span>          | `string`           | `woman`          | Text-to-speech voice to use.                                                                                                               |
...
The `status_url` parameter is used to make requests to for each status change during the payment process.
...
| `params.status_url` | `string` | The URL to make requests to for each status change during the payment process. |
