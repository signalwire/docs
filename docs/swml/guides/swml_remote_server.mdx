...
title: Handling Incoming Calls from Code
x-custom:
  tags:
    - sdk:swml
    - language:nodejs
...
If you don't have a phone number yet, make sure to [buy one](/platform/phone-numbers/getting-started/buying-a-phone-number). You
will need at least one number to receive calls.

Next, create a new Resource by navigating to the Resources section in the SignalWire Dashboard. Select a Script, and choose type as SWML.
For the primary script, make sure it's set to "External URL" and not "Hosted Script". Input the URL you've hosted your application on. 
