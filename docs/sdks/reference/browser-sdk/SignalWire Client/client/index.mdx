...
[access token]: ../index.mdx#authentication

:::tip

You can manage the DOM yourself by not specifying a `rootElement` here and using the [`buildVideoElement`](../utils.mdx#buildvideoelement) function instead.
...
The `token` is received through one of the [Authentication](../index.mdx#authentication)
...
| `push_notification_key` | `string`                          | SignalWire only sends encrypted push notifications. When you receive them, use this key to decode them ([Details](../push_notifications.mdx)) |
...
method. The [Push Notifications](../push_notifications.mdx) reference goes through this in detail.
...
[push notification payload]: ../push_notifications.mdx#payload
...
[`RoomSession`]: ../../video/video-roomsession.mdx
