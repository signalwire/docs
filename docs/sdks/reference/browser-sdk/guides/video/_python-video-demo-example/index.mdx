...
In the moderator view, we have several functions for controlling other users shown below. These will take a member ID (gathered from `room.params` on room state changes) as the argument and contain the methods shown in the [documentation](/sdks/reference/browser-sdk/video/video-roomsession.mdx#methods) for muting/unmuting/kicking out a user. These will not work if the token does not have the correct permission defined. These will be attached as on click events to buttons that are next to each user on the nav bar **only if** you are in the moderator view.
