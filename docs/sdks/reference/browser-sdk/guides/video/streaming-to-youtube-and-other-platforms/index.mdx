...
slug: /sdks/reference/browser-sdk/guides/video/streaming-to-youtube-and-other-platforms
author: danieleds
x-custom:
  tags:
    - product:video
...
Later, we will put click handlers on the start and stop buttons to call [`startStream`](/docs/sdks/reference/browser-sdk/video/video-roomsession.mdx#startstream) and [`stopStream`](docs/sdks/reference/browser-sdk/video/video-roomsessionstream.mdx#stop) respectively. The [`startStream`](/docs/sdks/reference/browser-sdk/video/video-roomsession.mdx#startstream) function is available on the Room Session object, so first we need to use the [`setupRoomSession`](/video/getting-started/video-conferences#technical-reference) callback function on the PVC to get that object. So, the `VideoConference` constructor at the end of the embed script should look like this:
...
- [SDK Streaming Reference](docs/sdks/reference/browser-sdk/video/video-roomsessionstream.mdx)
- [PVC Reference](/video/getting-started/video-conferences#technical-reference)
