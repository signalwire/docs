...
To create a `Voice` client, you will need to create a [`SignalWire Realtime-Client`](../realtime-client/index.mdx) first.
...
▸ **dial**(`dialer`): `Promise`\<[`Call`](voice-call.mdx)>
...
▸ **dialPhone**(`params`): `Promise`\<[`Call`](voice-call.mdx)>
...
| `params.listen?`            | `Object` | Object that contains Callbacks to listen for events.<br />List of Call events can be found [here](./voice-call.mdx#events).                                                                                                                               |
...
▸ **dialSip**(`params`): `Promise`\<[`Call`](voice-call.mdx)>
...
Emitted when a call is received. Your event handler will be called with a [`Call`](./voice-call.mdx) object.
