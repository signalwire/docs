...
This code introduced you to the [Phone Number Lookup API](https://docs.signalwire.com/topics/relay-rest/#resources-phone-number-lookup-include-carrier-details) , to remove any landlines in your message recipient list. We learned how to make an HTTP request to retrieve a JSON response that will list out information about each number. Through this response, we created a data table using `pandas` and created a formatted list of numbers that are waiting for you to send to!
