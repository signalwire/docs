...
The `<Pay>` verb also has two nouns that be embedded within it:
...
:::note
The actual token is provided by your payment processor and can be used for future transactions without requiring customers
...
| `action`<span className="optional-arg">optional</span> | A absolute URL | Current document URL | HTTPS endpoint that receives POST requests after payment completion. Must be HTTPS. Response determines next CXML instructions. For tokenization, receives PaymentToken/ProfileId. For charges, receives PaymentConfirmationCode. |
| `statusCallback`<span className="optional-arg">optional</span> | A absolute URL | - | HTTPS endpoint that receives real-time payment status updates via POST requests. Includes details about current payment stage, errors, and attempt counts.<br /> See [Status Callback Parameters](#statuscallback-request-parameters) for more details. |
...
| `paymentConnectorUrl`<span className="required-arg">required</span> | A absolute URL | - | URL to POST the encrypted payment data to. |
| `timeout`<span className="optional-arg">optional</span> | positive integer thats above `3` seconds. | `5` | Number of seconds to wait for the next digit input before timing out. Applies to all input collection stages. |
| `maxAttempts`<span className="optional-arg">optional</span> | `1`, `2`, `3` | `1` | Maximum number of retry attempts when timeout occurs or invalid data is received. Payment flow terminates after max attempts reached. |
...
| `postalCode`<span className="optional-arg">optional</span> | `true`, `false` | `true` | Controls postal code collection. When false, skips collection. When String, uses provided value without prompting. |
