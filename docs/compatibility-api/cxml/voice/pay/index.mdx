...
| `statusCallback`<span className="optional-arg">optional</span> | A absolute URL | - | HTTPS endpoint that receives real-time payment status updates via POST requests. Includes details about current payment stage, errors, and attempt counts.<br /> See [Status Callback Parameters](#status-callback-parameters) for more details. |
