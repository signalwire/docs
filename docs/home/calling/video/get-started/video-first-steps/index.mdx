...
- [Realtime SDK](/sdks/reference/realtime-sdk/): receive events from active video rooms and control them server-side (mute users, start recordings, change layouts, and so on).
