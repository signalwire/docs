---
title: Your SignalWire API Space
...
<img
  src={require("./api-credentials.webP").default}
  className="img-800"
  alt="API Credentials Page screenshot"
/>
...
## Generating a new API Token {#generating-a-new-api-token}

To generate a new API token, click the blue "+ New" button. A "New API Token" form will open.

<figure>
  <img src={require("./new-api-token.webP").default} className="img-800" />
  <figcaption style={{ align: "left" }}>
    Give your token a descriptive name to help differentiate it in logs and for debugging.
    You may choose a limited set of RELAY permissions scopes to allow this token to only
    access the scopes necessary for a specific application. When you are finished, click
    "Save" to return to your list of existing tokens.
  </figcaption>
</figure>
...
It is important that the API tokens are kept confidential. They can be used to make API requests on your behalf. Take extreme care to make sure that the tokens don't get pushed to GitHub. Make sure that the tokens aren't publicly accessible. For Node.js backend, you can use [dotenv files](https://www.npmjs.com/package/dotenv) or similar mechanisms paired with a [gitignore file](https://git-scm.com/docs/gitignore) to safely store confidential constants.

:::

All API requests must be made with proper authentication over HTTPS. Calls made over plain HTTP or without auth will fail.

You can find the correct credentials structure for each specific call in the [SDK Reference](/sdks/reference/realtime-sdk), [REST API Reference](/rest), or in a specific [Guide](/guides/) you may wish to follow.
