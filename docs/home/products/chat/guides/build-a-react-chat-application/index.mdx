...
<figure>
  <img className="img-800" src={require("./29dd24a-react-chat.webP").default} alt="A screenshot of a chat application using the SignalWire Chat JS SDK and React. The application has three chat channels: Office, Book Club, and Workshop. Several sample messages from different users are shown. There is a text box and a Send button." />
...
<figure>
  <img
    className="img-800"
    style={{ width: 300 }}
    src={require("./4a5d4ff-react-chat-file-structure.webP").default}
...
With either alternative, you can now `import {Chat} from "@signalwire/js"` and access it in your components. We'll mainly be interested in the `Chat.Client` class for this guide, but if you'd like to explore this API, please browse the [SDK documentation](/sdks/reference/browser-sdk/00-browser-sdk-reference.mdx).
