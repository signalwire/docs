...
learned the basics of using XML bins to define the behavior of phone calls. In
...
We are going to define the forwarding instructions in an XML bin hosted on
...
<figure>
  <img
    ...
    alt="A screenshot of the LāML tab of a project called 'Dan's Space' in SignalWire. Within the selected 'Bins' tab, a table organizes bins by Name, Request URL, Requests, and Last Accessed."
  />
  <figcaption>
...
timeout, then the following instructions keep executing: in our case, we say "no
input detected" and hang up.
...
value, emit a different XML document to execute. You can easily test this with
