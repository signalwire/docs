---
title: SIP BYOC (Bring Your Own Carrier)
...
To address both of the steps above, we will need to [create an XML Bin](/compatibility-api/guides/general/creating-and-using-cxml-applications).
...
so we recommend learning more about the [Verbs you can use](docs/compatibility-api/cxml/index.mdx#verbs).

To learn more about setting up XML bins, have a look at our [guide on creating and using cXML applications](/compatibility-api/guides/general/creating-and-using-cxml-applications).
...
To learn more about what you can do with Relay, have a look at our [Relay Realtime SDK documentation](/sdks/reference/realtime-sdk/relay-v4).

## Outbound BYOC

If you are using BYOC to do outbound calls from SignalWire, you will need a SIP URL from your carrier that we can use to route calls to the right SIP trunk. Once you have that, you can [create a call using the Compatibility API](/rest/compatibility-api/endpoints/create-a-call) or [Relay](https://docs.signalwire.com/reference/relay-sdk-nodejs/v2/#api-reference-relay-calling-methods-dial) using the SIP URL, SIP username, and SIP password. Let's look at some examples so you can see what we're talking about!
...
[RELAY Realtime SDK](/sdks/reference/realtime-sdk/relay-v3/voice/) is our must recent version of RELAY, and dialing out to a SIP endpoint is simpler than ever:
