...
To display a caller ID on SIP to SIP calls, you need to add an additional setting for caller ID in your SIP endpoint. Set the **Caller ID** field within [SIP Endpoint settings](/voice/getting-started/sip/set-up-a-signalwire-phone-number-with-a-sip-endpoint) to display caller ID on SIP to SIP calls. If you choose to set the **Send As** field, that will display one of your purchased or verified numbers, but it will only show for calls to PSTN numbers. **Caller ID** is the only way to display the caller ID for SIP to SIP calls. 
