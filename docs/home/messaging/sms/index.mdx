---
slug: /messaging
sidebar_position: 2
---

import { Card, CardGroup } from "@site/src/components/Extras/Card"
import { MdMessage, MdSync, MdApi, MdSend, MdBook, MdForward, MdLink, MdWeb } from "react-icons/md"
import Subtitle from "@site/src/components/typography/Subtitle";
import UseCase from "./_usecases/_usecases.mdx"
import Admonition from '@theme/Admonition';

# Messaging

<Subtitle>Integrate powerful, programmable, high-throughput SMS and MMS with SignalWire APIs, SDKs, and no-code application builders</Subtitle><br/><br/>

Programmable Messaging is deeply integrated in the SignalWire platform.
Our robust APIs, SDKs, and application builders are backed by first-class support and a team dedicated to compliance and class-leading delivery rates
across all carriers and locations.

SignalWire is your Messaging partner from proof-of-concept to deployment.
Get up and running quickly with our Getting Started guides,
test your application with our [Platform Free Trial][pft],
and get registration right the first time with our comprehensive
[Guide to The Campaign Registry (TCR)][tcr].

## Try it out

<UseCase/>

## Choose an API
<div className="row">
  <div className="col col--6">
SignalWire's many API offerings make it quick and easy to send and receive SMS and MMS within an application through a variety of phone number types. There are a few different ways to utilize these services, so we encourage you to browse the documentation for our APIs.
  </div>
  <div className="col col--6">
  <Admonition type="info" icon="💬" title="looking for Chat or PubSub?">
    SignalWire's Messaging APIs are tailored for SMS and MMS over the PSTN. For non-PSTN applications, check out our [Chat APIs][chat].
  </Admonition>
  </div>
</div>

<CardGroup>
  <Card 
    title="Compatibility APIs" 
    icon={<MdSync/>}
    href="/compatibility-api"
  >
    Easy migration from other providers in our industry with a notoriously higher billio.
  </Card>
  <Card 
    title="Realtime Server SDK" 
    icon={<MdApi/>}
    href="/sdks/relay-realtime-sdk/messaging"
  >
    Build a messaging solution with your own server using web socket technology for low latency operations.
  </Card>
  <Card 
    title="REST APIs" 
    icon={<MdMessage/>}
    href="/rest/signalwire-rest"
  >
    Use classic HTTP calls to manage brands and campaigns, configure phone numbers, and much more.
  </Card>
</CardGroup>

## What Kind of Guide are you Looking for?

<CardGroup>
  <Card 
    title="Send your first SMS" 
    icon={<MdSend/>}
    href="/messaging/getting-started/sending-your-first-sms"
  >
    Focused on installation, authentication, and the most basic examples of SMS Messaging. You will also find all of the vital information on The Campaign Registry in this section.
  </Card>
  <Card 
    title="Guides" 
    icon={<MdBook/>}
    href="/messaging/guides/general/messaging-character-limits"
  >
    Guides on general information and common use cases. General guides apply to all products while CXML API and Realtime API guides are separated by the API product.
  </Card>
  <Card 
    title="Examples" 
    icon={<MdMessage/>}
    href="/compatibility-api/guides/messaging/general/how-to-send-sms-from-google-sheets"
  >
    Full walkthroughs of more specific use cases with either a dedicated GitHub repo or extensive embedded code examples.
  </Card>
</CardGroup>

## Popular Guides

<CardGroup>
  <Card 
    title="Forward your Texts to Email" 
    icon={<MdForward/>}
    href="/sdks/relay-realtime-sdk/guides/messaging/forwarding-texts-to-email"
  >
    Learn how to listen for incoming text messages (SMS) and forward them to an email address.
  </Card>
  <Card 
    title="Create a Private URL Shortener" 
    icon={<MdLink/>}
    href="/messaging/guides/general/how-to-build-a-private-url-shortener"
  >
    Learn how to build an a private URL shortener.
  </Card>
  <Card 
    title="Send SMS from the Browser" 
    icon={<MdWeb/>}
    href="/sdks/relay-realtime-sdk/guides/messaging/send-sms-from-the-browser"
  >
    This guide shows you how to create a web application that allows users to send SMS from the browser.
  </Card>
</CardGroup>

{/* Links */}
[chat]: /chat
[pft]: /messaging/getting-started/platform-free-trial
[tcr]: /messaging/get-started/campaign-registry