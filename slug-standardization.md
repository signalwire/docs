...
python3 scripts/propose-new-slugs.py            # reads frontmatter-export.csv → slug-proposals.csv
```
...
and writes a separate `slug-proposals.csv` (adds a `new_slug` column). Neither
script mutates the other's output. Review `slug-proposals.csv` in a spreadsheet,
edit `new_slug` values as needed, then apply (apply script TBD).
