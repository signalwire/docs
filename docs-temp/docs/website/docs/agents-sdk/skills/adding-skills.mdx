...
> **Note**: Skills like `web_search` and `joke` require additional configuration or API keys. See the [Built-in Skills](/sdks/agents-sdk/skills/builtin-skills) section for details on each skill's requirements.
