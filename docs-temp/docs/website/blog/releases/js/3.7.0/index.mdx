...
For example, let's say we want to [create a new room using the REST APIs](/rest/signalwire-rest/endpoints/video/create-room). To enable room previews, you can add the `enable_room_previews` flag:
...
Or, to obtain it *before* joining a room, simply use the REST APIs to [list room sessions](/rest/signalwire-rest/endpoints/video/list-room-sessions): you'll find the URL in there too.
