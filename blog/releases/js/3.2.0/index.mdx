...
- Room objects now exposes the methods [startRecording](/sdks/relay-browser-sdk/video/room-session#startrecording) and [getRecordings](/sdks/relay-browser-sdk/video/room-session#getrecordings), which you can use for controlling recordings of the current room session.
- Room objects now emit [events associated with recordings](/sdks/relay-browser-sdk/video/room-session#events). These are `recording.started`, `recording.updated`, and `recording.ended`.
