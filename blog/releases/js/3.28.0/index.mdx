...
This release introduces **Call Fabric** (alpha) and **Video SDK** enhancements, including new APIs for room management, 
...
1. Added new types and fixed incorrect type definitions:
    - fixed `ConversationMessageEventParams.type` to be `string` instead of `MessageType`
    - added `ConversationContract`
    - fixed types for `VideoLayout` and other layout related types
2. fix Chat Messages filtering for previous and next pages, and normalize `getAddress` response
3. Fix local `audio_muted` and `video_muted` state for `Call` to reflect the actual tate.



## NOTES (PRIVATE (remove before publishing))

- Wouldn't removing implicit reauth be a breaking change?

- User Params Priority for `client.dial`: is this section accurate. Does it affect more than just the `dial` command?