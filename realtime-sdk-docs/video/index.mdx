...
A [`RoomSession`](./video-roomsession.mdx) can be created through the [Browser SDK](/sdks/reference/browser-sdk/video/video-roomsession#constructors),
...
When a [`RoomSession`](./video-roomsession.mdx) is created, the [`onRoomStarted`](./video-client.mdx#onroomstarted) event is triggered.
We can then subscribe to the [`RoomSession`](./video-roomsession.mdx) events,
