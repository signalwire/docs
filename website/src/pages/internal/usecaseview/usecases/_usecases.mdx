import Compat1 from "./_compat1.mdx"
import Compat2 from "./_compat2.mdx"
import SWMLExt from "./_swml_ext.mdx"

import { UseCaseView } from "@site/src/components/HomepageFeatures/UseCaseView"

<UseCaseView data={{
  "Deploy an AI Agent": {
    "SWML": <Compat1/>,
    "Relay Realtime": <Compat2/>,
    "Compatibility API": <SWMLExt/>,
    "Call Flow Builder": <SWMLExt/>,
  },
  "Launch SMS marketing": {
    "Compatibility API": <Compat2/>,
    SWML: <div style={{height:300, background:"aquamarine"}}></div>,
    "Call Flow Builder": <div>Call Flow Builder 2</div>,
  },
  "Integrate MFA": {
    "Compatibility API": <Compat2/>,
    SWML: <div style={{height:300, background:"aquamarine"}}></div>,
    "Call Flow Builder Call Flow Builder Call Flow Builder Call Flow Builder Call Flow Builder": <div>Call Flow Builder 2</div>,
  },
  "Add Programmable Video Conferencing": {
    "SWML": <Compat1/>,
    "Relay Realtime": <Compat2/>,
    "Compatibility API": <SWMLExt/>,
    "Call Flow Builder": <SWMLExt/>,
  },
  "Build an IVR": {
    "SWML": <Compat1/>,
    "Relay Realtime": <Compat2/>,
    "Compatibility API": <SWMLExt/>,
    "Call Flow Builder": <SWMLExt/>,
  },
  "Stream a Voice Call": {
    "SWML": <Compat1/>,
    "Relay Realtime": <Compat2/>,
    "Compatibility API": <SWMLExt/>,
    "Call Flow Builder": <SWMLExt/>,
  },
}}/>