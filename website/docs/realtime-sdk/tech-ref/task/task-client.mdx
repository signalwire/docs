...
| `params.topics`<span className="required-arg">Required</span> | `string[]` | Topics to listen to for events. Previously known as `"contexts"`.                                                |
| `params.EVENT`<span className="optional-arg">Optional</span>  | `string`   | The event to listen to. List of events can be found [here][link-1].<br />Example event: (E.g: `onTaskReceived`) |
...
| `params.topic`<span className="required-arg">Required</span> \| `params.topics` | `string` \| `string[]`    | Topic to send the task to. Previously known as `"context"` \| `"contexts"`. |
| `params.message`<span className="required-arg">Required</span>                  | `Record<string, unknown>` | Message to send.                                                            |
| `params.project`<span className="optional-arg">Optional</span>                 | `string`                  | SignalWire project id, e.g. `a10d8a9f-2166-4e82-56ff-118bc3a4840f`.         |
| `params.token`<span className="optional-arg">Optional</span>                   | `string`                  | SignalWire project token, e.g. `PT9e5660c101...a360079c9`.                  |
...
| Name      | Type                      | Description          |
|:----------|:--------------------------|:---------------------|
| `payload`<span className="required-arg">Required</span> | `Record<string, unknown>` | The message payload. |
