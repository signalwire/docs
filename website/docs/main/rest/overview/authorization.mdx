...
sidebar_position: 1
title: Authorization
slug: /rest/overview/authorization
---
...
[Basic Authentication](https://swagger.io/docs/specification/v3_0/authentication/basic-authentication/) is the standard method for authenticating with SignalWire REST APIs, using your **Project ID** and **API Token**.
...
Given the Project ID `a1b2c3d4-e5f6-7890-abcd-ef1234567890` and API Token `PT9a8b7c6d5e4f3a2b1c`:

```
a1b2c3d4-e5f6-7890-abcd-ef1234567890:PT9a8b7c6d5e4f3a2b1c
```

Base64 encoded:

```
YTFiMmMzZDQtZTVmNi03ODkwLWFiY2QtZWYxMjM0NTY3ODkwOlBUOWE4YjdjNmQ1ZTRmM2EyYjFj
```

The full header:

```
Authorization: Basic YTFiMmMzZDQtZTVmNi03ODkwLWFiY2QtZWYxMjM0NTY3ODkwOlBUOWE4YjdjNmQ1ZTRmM2EyYjFj
```

### Finding your credentials

Your Project ID and API Tokens are available in the [SignalWire Dashboard](/platform/dashboard/getting-started/your-signalwire-api-space).
...
| Scope | Description |
|-------|-------------|
| Voice | Voice and calling APIs |
| Chat | Chat APIs |
| Fax | Fax APIs |
| Management | Account and project management APIs |
| Messaging | SMS and MMS APIs |
| Numbers | Phone number management APIs |
| Pubsub | Pub/sub APIs |
| Storage | Storage APIs |
| Tasks | Task APIs |
| Video | Video APIs |
| DataSphere | DataSphere APIs |

Getting a `401 Unauthorized`? Check that your token has the required scope. Manage scopes in the [SignalWire Dashboard](/platform/dashboard/getting-started/your-signalwire-api-space).

### cURL example

```bash title="cURL"
curl https://example.signalwire.com/api/video/rooms \
     -H 'Authorization: Basic <Base64(YourProjectID:YourAPIToken)>'
```

With the example credentials above:
...
**How to obtain:** Call the [Create Subscriber Token](/rest/signalwire-rest/endpoints/fabric/subscriber-tokens-create) endpoint using Basic Auth.
...
**Use case:** Client apps connecting to Fabric services, such as listing [Fabric Addresses](/rest/signalwire-rest/endpoints/fabric/fabric-addresses-list).
...
**How to obtain:** Call the [Create Subscriber Guest Token](/rest/signalwire-rest/endpoints/fabric/guest-tokens-create) endpoint using a SAT.
...
To keep a session alive without re-authenticating, call the [Refresh Subscriber Token](/rest/signalwire-rest/endpoints/fabric/subscriber-tokens-refresh) endpoint before expiration.
