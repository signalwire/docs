...
![A diagram explaining the account pane of microsip. The Account Name is the Friendly Name. The SIP Server and Domain both correspond to the SignalWire SIP Domain. The Username and Login both correspond to the SIP Endpoint Username.](@image/external/microsip/sip-settings.webP)
...
![The Encryption settings in microsip. Media Encryption is set to Mandatory SRTP, and Transport is set to TLS.](@image/external/microsip/encryption-setting.webP)
