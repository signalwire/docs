...
We are going to define the forwarding instructions in a SWML script hosted on
SignalWire. To create a new SWML script, navigate to the "Resources" section from your sidebar. There, create
a new script, and select the "SWML" option.

### Creating your call forwarding script

We will start by creating a new SWML script, and pasting the following in it:
...
We used the [`connect`](/swml/methods/connect) method to call the number you
would like the call to be forwarded to. Notice how we used its `from` parameter to ensure that the
number of the original caller (stored in the `call.from` variable) is maintained as caller ID for the forwarded call.
...
Now any calls that go to that number will get forwarded with the caller ID being the original number that called in.
