...
| Model | Description |
|-------|-------------|
| `sonic-english` | Optimized for English-language speech |
| `sonic-multilingual` | Support for multiple languages |
| `sonic-2` | Second-generation model with improved quality |
| `sonic-3` | Third-generation model with enhanced naturalness |
| `sonic-turbo` | Optimized for ultra-low latency |
| `sonic-v3` | **Default** - Latest version, balanced quality and speed |

All Cartesia voices can be used with any model.

## Additional Parameters {#parameters}

Cartesia supports advanced parameters for fine-tuning speech output:

- `speed` - Speech rate adjustment (0.5-2.0, default: 1.0)
- `emotion` - Emotional expressiveness level (0-100)
- `max_emotion` - Maximum emotion intensity cap (default: 30)

## Output Formats {#formats}

Cartesia supports multiple audio output formats:
...
**Parameters:**
- `voice_id` (required): The UUID voice identifier from the table below
- `model` (optional): One of the Sonic models listed above (default: `sonic-v3`)

**Examples:**
```
cartesia.a167e0f3-df7e-4d52-a9c3-f949145efdab
cartesia.694f9389-aac1-45b6-b726-9d9369183238:sonic-v3
...
Cartesia voices are fully multilingual when used with `sonic-multilingual`, `sonic-2`, `sonic-3`, or `sonic-v3` models. The multilingual models automatically adapt to the input text language.
...
For the complete list, consult [Cartesia's documentation](https://docs.cartesia.ai/build-with-cartesia/models/tts).
