...
<Frame caption="Testing the `/api/token` endpoint with Thunder Client">
  ![Thunder Client showing a GET query being used to test the /api/token endpoint.](./images/backendtest.webP)
</Frame>
...
![a video conference, showing controls for video, audio, participants, invitations, and layout.](./images/controls2.webP) 
</Frame>
...
<Frame caption="Sending GET request to '/api/sessions'" >
  ![a GET query in Thunder Client. A GET request is being sent to /api/sessions.](./images/sessionstest.webP)
</Frame>
...
<Frame caption="Thumbnails of ongoing room sessions being displayed.">  
  ![two thumbnail images representing ongoing rooms, with the option to join.](./images/sessions.webP)
</Frame>
