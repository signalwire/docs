...
Provide them with your SIP domain app and explain that you need a password for VAPI integration.
:::
...
Run the API call below to create the outbound trunk, making sure to replace the placeholder values with your actual configuration:
...
If you've already set up inbound calling with the same number, VAPI will indicate this is a duplicate number. This is normal and expected - you're using the same phone number for both inbound and outbound calling with different configurations.
:::
...
Your VAPI assistant can now make outbound calls through SignalWire, and the calls will show your SignalWire phone number as the caller ID.
...
**Authentication failures**: Double-check that you're using the correct SIP domain app password provided by SignalWire Support. The authentication username should be your phone number in E.164 format.
