...
which forwards the call to the destination number. The key configuration is setting the **From** field to `%{call.from}` - this variable contains the original caller's
