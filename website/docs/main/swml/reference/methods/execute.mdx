...
- A URL (http or https) - URL pointing to the document to execute. An HTTP POST request will be sent to the URL. The `params` object is passed, along with the variables and the [`Call`](/swml/guides/deployment#the-call-object) object. Authentication can also be set in the url in the format of `username:password@url`.
...
The `call` object is described in detail in the [introduction](/swml/guides/deployment#the-call-object). All variables created
