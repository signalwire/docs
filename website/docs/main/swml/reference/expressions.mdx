...
Expressions use the `${...}` syntax and support JavaScript for dynamic value construction. Any JavaScript that evaluates to a value can be used inside `${}`.
...
sections:
  main:
    ...
    - set:
        ...
        subtotal: '${params.unit_price * parseInt(prompt_value)}'
        tax: '${subtotal * 0.08}'
        total: '${subtotal + tax}'
...
However, serverless SWML has inherent limitations. You can't access databases or external APIs directly through expressions, and async operations aren't supported.
The expression syntax is also limited to simple JavaScript—no loops, function declarations, or complex control flow.
