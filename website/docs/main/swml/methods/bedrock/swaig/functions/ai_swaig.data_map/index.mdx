...
<APIField
  ...
  required={true}
>
  An array of objects that have pattern matching logic to process the user's input data. A user can define multiple expressions to match against the user's input data. See [`expressions`](./data_map.expressions.mdx) for details.
</APIField>
...
<APIField
  ...
  required={true}
>
  An array of objects that define external API webhooks that can be used to make external API calls. See [`webhooks`](./data_map.webhooks/index.mdx) for details.
</APIField>
...
<APIField
  ...
  required={true}
>
  An object that defines the output of the SWAIG function. Behaves like a `return` statement in traditional programming. See [`output`][output] for details.
</APIField>
