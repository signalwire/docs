...
Place the current call in a queue where it will wait until an agent or resource becomes available.
This method allows you to manage call flow by queuing calls and playing wait music while callers wait for service.
...
<APIField
  ...
  type="string"
>
  HTTP or HTTPS URL to deliver queue status events. See [Queue Status Callbacks](#queue-status-callbacks) for event details.
</APIField>
...
- **queue_result:** (out) The result of the queue operation. **Possible Values**: `connected` (successfully connected to an agent), `timeout` (waited too long and timed out), `hangup` (caller hung up while waiting), `failed` (queue operation failed due to an error).
