...
When attempting to pass a SWML object with a `connect` your script must be JSON, and `"transfer": true,` must be included before the SWML in the same action object.
