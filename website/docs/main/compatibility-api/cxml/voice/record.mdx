...
| `RecordingChannels` <span className="optional-arg">integer</span> | The number of channels in the recording. `<Record>` has one channel present by default and can also accept two channels.                                                  |
