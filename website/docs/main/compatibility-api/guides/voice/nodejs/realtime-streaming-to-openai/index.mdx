...
The webhook automatically sets the correct codec based on your `AUDIO_FORMAT` configuration. No manual codec configuration needed here—see [Configure audio format](#configure-audio-format) for how to set your desired audio quality.
...
This is the core of the integration. The WebSocket server receives audio from SignalWire and forwards it to OpenAI's Realtime API, then sends AI responses back. The `SignalWireCompatibilityTransportLayer` handles all the protocol translation and audio format conversions automatically.
