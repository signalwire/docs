...
The HTTP POST to `/incoming-call` includes metadata about the call:
- `streamSid` - Unique identifier for this audio stream
- `callSid` - Unique identifier for this call
- `mediaFormat` - Audio codec and sample rate (e.g., G.711 μ-law at 8kHz)

Your server uses this information to configure the transport layer with the correct audio format.
:::
...
const { SignalWireCompatibilityTransportLayer } = require('../transports/SignalWireCompatibilityTransportLayer.js');
